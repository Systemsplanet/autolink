...
- **cobsSeq Gap/Stale Detection:** Every data frame carries a 1-byte sequence counter (the `cobsSeq` space is 0..253; 0xFE and 0xFF are reserved wire discriminators). ASYNC delivery is Go-Back-N: the receiver accepts strictly in order and cumulative-ACKs the highest contiguous seq; a frame that arrives ahead of the expected seq is dropped (not buffered) and a NAK fires for the missing seq. The sender's oldest-unacked frame is the only retransmit target — a NAK or RTO on it resends the whole outstanding window verbatim in one shot. Stale duplicates are re-ACKed so a lost ACK never pins a sender slot. maxRetx exhausted on the oldest-unacked frame is an honest link drop and resync, not a slot retirement.
- **ARQ — Per-Message ACK:** Every accepted `send()` is retransmitted on demand (NAK-driven fast retransmit + ACK-timeout retransmit) until the peer acknowledges it. If a slot exceeds its retry budget (`cfg.maxRetx`) it is retired — the usual cause is a delivered frame whose ACK the wire ate — rather than resetting the link. A stalled-window gate rejects new sends when the oldest un-ACKed frame falls ~96 seqs behind, keeping the 8-bit seq space unambiguous. The payload cache is pool-backed (no `malloc` on the TX hot path) — under sustained back-pressure, `send()` returns 0 and the message is dropped (with a loud log line), never silently sent-without-cache. Delivery is best-effort under sustained noise — if your sketch needs hard delivery guarantees, layer them on top.
