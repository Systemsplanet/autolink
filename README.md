...
Sequential mode grows each message from 1 byte up to `cfg.maxMsg` and wraps back to 1 (operators see the message size grow monotonically through the link's MTU). Random mode picks a random size (capped at half the GBN window's worth of chunks, so every random message stays co-admittable with a loaded pipeline) with random data, forcing multi-chunk sends in steady state so the ARQ chunk path is exercised end-to-end without starving the sender window. The default per-transmit delay is 0 ms — full line-rate out of the box (configurable via the dashboard's delay-ms widget or `cfg.txDelayMs`).
...
- **Baud Preference:** Once the link locks, the baud that worked is remembered. A peer BREAK — a clean detach — re-locks straight at that baud. A watchdog drop clears the preference and walks the full sweep again: a baud that just failed is not a good bet.
...
- **Go-Back-N ARQ:** Every data frame carries a 1-byte sequence counter. The receiver accepts strictly in order and cumulative-ACKs the highest contiguous seq; a frame that arrives early is dropped and a NAK fires for the one that's missing. The sender's oldest un-ACKed frame is the only retransmit target, driven by that NAK or by its RTO. Stale duplicates are re-ACKed, so a lost ACK never pins a sender slot. Admission is a single bound — in-flight chunks plus this message's chunks must fit the window — so `send()` returns `0` (normal backpressure) instead of overcommitting, and the payload cache is pool-backed with no `malloc` on the TX hot path. Retransmit storms are damped three ways: a per-RTO burst cap, an exponential inter-round backoff, and a keep-vs-drop check that refuses to tear down a link whose peer is demonstrably still answering. See `docs/Protocol.md`.
- **Idle Keepalive:** A locked but idle link PINGs the peer every `idleTimeoutMs / 2`; the PONG refreshes both ends. A duty-cycled link — a sensor reporting once a minute, an operator-triggered command link, a paused dashboard — stays up instead of being torn down and re-swept every 30 seconds.
- **SYNC and ASYNC:** `cfg.mode = ASYNC` (default) pipelines up to a full GBN window and never blocks. `cfg.mode = SYNC` keeps one frame in flight and blocks `send()` on the ACK. Same wire format either way.
...
- **Smart Framing:** Control commands are wrapped in CRC-8-validated frames behind a `0xAA 0x55` preamble, and the CRC is checked *before* five bytes are consumed as control — so neither electrical noise nor a random payload collision can trigger a false state change.
