...
- **cobsSeq Gap/Stale Detection:** Every data frame carries a 1-byte sequence counter (the `cobsSeq` space is 0..253; 0xFE and 0xFF are reserved wire discriminators). On a gap, the out-of-order frame is held in a bounded reorder buffer and a NAK fires for the first missing seq; the retransmit lands in-order when it arrives. A staleness cap (`cfg.arqReorderHoldMs`, default 1500 ms) drops the hold and counts `lostMsgs++` if the retransmit never succeeds.
