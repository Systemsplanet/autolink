...
void setup() {
    ...
    AutoLinkConfig cfg;
    cfg.allowedBauds = myBauds;
    cfg.errThreshold = 10;
    cfg.delayMs = 100;
    cfg.reliableMode = true; // Use COBS byte stuffing!
    
    link.reset(new AutoLink(UART_NUM_2, 16, 17, true, cfg));
}
...
**v3.0.0 (Security & Performance Hardening)**
+ **Concurrency Fixes:** Removed the application mutex from the hardware transmission sequences, completely eliminating downstream UART deadlocks. Offloaded FreeRTOS Timer Service invocations into custom queue events, ensuring the RTOS daemon never stalls waiting on shared links.
+ **Dynamic RX State Parsing:** Removed stale state caching. The parser now evaluates the volatile state machine instantly on every parsed byte, resolving erratic pointer overwrites when connections drop mid-packet.
+ **COBS Reliable Framing:** Completely ripped out the high-overhead `0xDD` static-byte framing and implemented Consistent Overhead Byte Stuffing (COBS). This mathematically guarantees zero byte-collisions between payload and protocol delimiters while recovering vast amounts of serial bandwidth.

**v2.0.0 (Production-Ready)**
+ API Enhancement: Inherited `AutoLink` from standard Arduino `Stream`.
+ Performance Optimization: Replaced bitwise nested loops in CRC with O(1) LUT.

**v1.0.0 (Initial Prototype)**
+ Initial Master/Slave auto-baud negotiation logic.
+ Basic FreeRTOS stream buffer integration.
