...
- `test/test_desktop/` — host unit tests, subsecond. Mirrors `src/al/<mod>/` one level deeper: `al/link/gbn/`, `al/link/health/` (+ `break/` / `watchdog/` / `admission/`), `al/link/message/` (+ `roundtrip/` / `buf/`), `al/link/frame/`, `al/link/ack/`, `al/link/async/`, `al/link/timer/`, `al/link/misc/`, `al/link/arq/`, `al/link/sweep/` (+ `sweep/guard/` splits into `phase/` / `break/` / `converge/`) group by test concern rather than 1:1 with a source file, since one source concern often has many pins. `al/web/` splits into the root (dashboard/routing tests) and `httpd/` (lifecycle/startup/retry tests); `al/` itself splits into `facade/` (runtime AutoLink-level tests) and `meta/` (structural/source-grep tests); `al/pingpong/` splits into the root, `prod/`, `structure/`, and `ping/`. `dashboard-js/` holds the jsdom-based dashboard JS tests, run via `make test_dashboard_js`.
...
5. Update `docs/Version.md` (newest on top, one-line summary + fix + regression test + limitations). `build/version.py add --version <X.Y.Z> --title "..."` scaffolds; `trim --keep 20` enforces the cap (`DEFAULT_KEEP` in `build/version.py` — AL-C2: this used to say 50 here and 20 both in `build/version.py`'s own default and two lines up in this same file; the code's default is the source of truth). `check` is the pre-zip gate — AL-C1: wired into `build/pre_zip_check.sh`, so a `docs/Version.md` over the cap now blocks a release the same way a missing required file does.
   - Open work that an AI can complete in this environment (source edits, host tests, cross-compile in a network-capable sandbox) belongs in the current `docs/Version.md` entry's "Disclosed limitations" / "Open" section so it ships with the release. Work that requires physical hardware — FireBeetle bench runs, real-UART @ 512000 validation, scope traces — also belongs there, not in a separate todo file.
...
8. The zip is the deliverable. Stage flat at the archive root (`cd staging && zip -r -y /path.zip .`). No wrapper folder. Strip build byproducts:
   - `*.o`, `*.bak`, `*.gcno`/`*.gcda`, `node_modules/`, `compile_commands.json` — build outputs anywhere in the tree
   - Every extensionless file under `test/test_desktop/` or `test/itest/test_desktop/` that is not a `Makefile` or package metadata (`package.json` / `manifest.json`). Catches the legacy `run_test_*` (unit) / `run_loopback*` (itest) shapes plus every other `run_*` shape the suites produce (`run_recovery_*`, `run_app_*`, future shapes). Per-name globs miss new shapes; the directory rule doesn't.
   - `build/verify_build/build` + `build/verify_build/libraries` — ESP32 build caches.
   `build/clean.py` is the single owner of this list (rule 34); the per-name globs in the prior shape were the source of the v6.1.69 stale-itest-binary miss. Keep `.cpp`, `.h`, `.ino`, `.md`, `.properties`, `.txt`, `build/`, **`test/`** (the full test tree: `test/Makefile`, `test/test_desktop/`, `test/itest/`, `test/common/`, `test/scripts/`, `test/test_embedded/`, `test/dashboard-js/`). Every `Pinned by …Test` reference in a source comment must resolve to a file in the zip; an absent test file is a load-bearing gate that the user can no longer run. Normalize perms: dirs 0755, files 0644, scripts 0755, `library.properties` 0644. Smoke-test by extracting into a fresh dir and running `make test && make itest` — only the itest gate catches the stale-binary class.
   **Mandatory:** `bash build/pre_zip_check.sh --zip <path.zip>` after staging. Exit 1 = the zip is rejected; do not deliver. The pre-zip check must also assert the four test entries (`test/Makefile`, `test/test_desktop/Makefile`, `test/common/MockHal.h`, `test/scripts/coverage/coverage_manifest.py`) are present in the archive.
...
20a. Every package (a directory of source or test files) stays at or under 7 files; every `.cpp`/`.h` file stays under 15 KB. When a directory or file would cross either limit, split into a subdirectory by concern — see `src/al/link/` and its test mirror for the reference shape. `test/common/`'s shared fixtures reach a subdirectory (`accessors/`) via an extra `-I` search path in both test Makefiles rather than qualifying every `#include` — prefer that shape when the alternative is rewriting a large number of bare includes across many files. Enforced as a non-regression ratchet by `build/rule20a_check.sh` (wired into `build/pre_zip_check.sh`): `build/rule20a_baseline.txt` lists every pre-existing violation, and a release fails only on a violation NOT in that list. Splitting an existing large file removes its line from the baseline; introducing a new one that grows past the cap without splitting fails the build.
