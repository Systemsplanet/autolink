...
9b. **Prefer composition to inheritance.** When hooking the
    facade into the protocol, default to function-pointer
    callbacks or `unique_ptr<Interface>` rather than virtual
    bases. Reason: function pointers compile to a single
    indirect call and don't pull a vtable into every ALink
    instance (saves ~32B per instance on ESP32, more when the
    derived class has multiple overrides). Inheritance is
    acceptable only when (a) the polymorphic interface is on the
    boundary the user is expected to extend (e.g. `ILink`), or
    (b) the type genuinely needs runtime dispatch. The library
    already uses this style: `setArqCacheHooks`,
    `setFillModeHook`, `setLinkResetHook` — keep adding to that
    pattern, don't introduce a new virtual base per hook.
    **Lesson learned the hard way in v5.1.40**: an early draft
    of `MockHal` used a virtual base for the test-only
    `frameDropPct` knob; this forced every test to inherit from
...
14a. **Never add version numbers to paths inside the zip.** The
     top-level directory inside the zip is always `AutoLink/`,
     never `AutoLink-5.1.45/`. The version is in the zip
     *filename* (`AutoLink-5.1.45.zip`), not in any internal
     path. Reason: ArduinoDroid, Arduino IDE, and PlatformIO all
     expect a stable directory layout to find
     `library.properties` at `<lib-root>/library.properties`.
     A versioned directory forces the user to rename it on every
     update, or the IDE silently fails to find the library.
     The `AUTOLINK_VERSION` macro inside `src/AutoLink.h` is the
     *one* place the version appears in the installed library
     files. Lesson learned the hard way in v5.0.0: shipped zips
     with `AutoLink-5.0.0/` at the top and a dozen user reports
     of "library not detected" — all of them were because the
     IDE didn't recognize the versioned top dir. v5.0.1 fixed
     this; never reintroduce it.
