...
# Phase 2 directly against broken tree, and verifying it returns
...
FS_INCLUDE_OK=1
for f in src/al/web/handlers/AutoLinkWebLifecycle.cpp \
         src/al/web/handlers/AutoLinkWebHttpd.cpp \
         src/al/web/handlers/AutoLinkWebHandlersData.cpp \
         src/al/web/handlers/AutoLinkWebHandlersOta.cpp; do
    if ! grep -qE '^#include <FS\.h>$' "$REPO_DIR/$f"; then
        FS_INCLUDE_OK=0
        echo "  $f:"; grep -n '#include <.*FS\.h>' "$REPO_DIR/$f" || echo "    (no FS include)" >&2
    fi
done
if [ "$FS_INCLUDE_OK" -eq 1 ]; then
    echo "  PASS: web TUs include <FS.h> directly"
else
    echo "FAIL: one or more web TUs are missing the direct #include <FS.h>" >&2
    exit 5
fi
