...
def main() -> int:
    ...
    #    what AutoLinkWeb::handleRoot expects. The chunked-send
    #    loop walks sizeof(DASHBOARD_HTML) - 1, so the contract
    #    is "DASHBOARD_HTML is the concatenation of the five
    #    parts plus the final null terminator". The per-part
    #    decomposition is the structural guarantee: every part
    #    contributes a positive number of bytes (no empty part
    #    silently sneaks in), the runtime equals the
    #    sum-of-parts + 1, and the total is non-trivial.
    #    Asserting against the computed sum keeps the pin
    #    honest as the JS grows further.
    runtime, sizes = _runtime_size_from_header(after, version_str)
    if runtime <= 0:
        sys.exit('test_dashboard_assets: cannot compute runtime size')

    for part_name, part_size in sizes.items():
        if part_size <= 0:
            sys.exit(f'test_dashboard_assets: part {part_name} '
                     f'has zero / negative size ({part_size}); '
                     f'sizes: {sizes}')

    expected = sum(sizes.values()) + 1
    if runtime != expected:
        sys.exit(f'test_dashboard_assets: runtime size {runtime} '
                 f'differs from sum-of-parts + 1 = {expected} '
                 f'(sizes: {sizes})')
