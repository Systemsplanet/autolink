// DiagEcho.ino — raw UART loopback diagnostic using the same ESP-IDF APIs
// as AutoLink (event queue, GPIO pull-up, uart_wait_tx_done, esp_timer LED).
//
// Uses NO AutoLink library code. Flash on BOTH boards — each one sends its
// own messages and echoes what it receives, so you see both wire directions
// from either serial monitor.
//
// LED feedback (blue LED, GPIO 2) — driven by esp_timer one-shots, same as
// AutoLink's EspBlinkHal:
...
// ── LED — non-blocking, esp_timer one-shot (same mechanism as AutoLink) ───────
//
// ledOn() lights the LED and schedules a one-shot timer to turn it off after
// `ms` milliseconds. A new call while the LED is on just restarts the timer
// (the longer flash wins if called twice in quick succession).
...
void setup() {
    ...
    // UART config — identical field layout to EspHal::EspHal()
    uart_config_t cfg = {};
    ...
    // Install driver with event queue — same as AutoLink's EspHal
    esp_err_t err = uart_driver_install(UART_PORT,
                                        RX_BUF, RX_BUF,
                                        QUEUE_DEPTH, &uartQueue, 0);
    if (err != ESP_OK) {
    ...
    // Pull RX high — same as AutoLink's EspHal, prevents noise on floating pin
    gpio_set_pull_mode((gpio_num_t)RX_PIN, GPIO_PULLUP_ONLY);

    Serial.printf("UART%d ready  tx=GPIO%d  rx=GPIO%d  baud=%lu  queue=%d\n",
                  (int)UART_PORT, TX_PIN, RX_PIN,
                  (unsigned long)UART_BAUD, QUEUE_DEPTH);
}
...
void loop() {
    ...
    // --- send ---
    char msg[64];
    snprintf(msg, sizeof(msg), "%s Send: #%lu\n", tsBuf, (unsigned long)++msgNum);
    uart_write_bytes(UART_PORT, msg, strlen(msg));
    uart_wait_tx_done(UART_PORT, pdMS_TO_TICKS(100));
    Serial.printf("> %s", msg);
    ledOn(50);      // short flash = sent

    // --- read via event queue (same path as AutoLink's uart_event_task) ---
    uart_event_t event;
