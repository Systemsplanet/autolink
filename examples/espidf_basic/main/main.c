// Stock-ESP-IDF example: UART-driven echo on UART2 with LED
// heartbeat. C counterpart to the Arduino basic sketch.
#include <stdio.h>
#include <string.h>
#include <inttypes.h>
...
void app_main(void) {
    ...
    if (uart_driver_install(UART_PORT, RX_BUF, RX_BUF,
                            QUEUE_DEPTH, &uart_queue, 0) != ESP_OK) {
    ...
    if (uart_set_pin(UART_PORT, TX_PIN, RX_PIN,
    ...
    while (1) {
        ...
        while ((xTaskGetTickCount() * portTICK_PERIOD_MS) < deadline) {
            uint32_t remaining = deadline -
                                 xTaskGetTickCount() * portTICK_PERIOD_MS + 1;
            ...
            if (xQueueReceive(uart_queue, &event, wait) == pdTRUE) {
                if (event.type == UART_DATA && event.size > 0) {
                    ...
                    int got = uart_read_bytes(UART_PORT, rx_buf,
                                              event.size < (RX_BUF - 1)
                                                  ? event.size
                                                  : (RX_BUF - 1),
                                              pdMS_TO_TICKS(10));
