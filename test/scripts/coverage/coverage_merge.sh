...
#
# The source-to-binary map must not be a hardcoded array in this
# script: a hand-maintained list drifts from TEST_BINS in the test
# Makefile the moment a new suite is added there (see AGENTS.md
# rule 4). Instead, the Makefile generates a manifest
# (coverage/manifest.sh) that lists, for every library source
# basename, the set of run_test_* binaries that link it — derived
# from TEST_BINS and the per-suite build rules, so it cannot drift.
# This script sources the manifest and uses the resulting
# src_for_<basename> variables; the list of test binaries is also
# passed via the manifest (TEST_BINS variable), so the test-file
# branch iterates the same set as the Makefile.
#
...
#                    least one suite.
set -e
# Deliberately no `cd "$(dirname "$0")"` here: every relative path
# below (coverage/, $bin-$src.gcno, etc.) is relative to the
# CALLER's cwd (test_desktop/, per the Makefile's `coverage` recipe
# and this script's own usage comment above), not this script's own
# location. A `cd` into the script's directory here silently
# breaks the manifest lookup and every path after it — caught by
# actually running `make coverage` end-to-end, not just reading it.
...
# Guard: two different library sources sharing one basename produce
# colliding <bin>-<basename>.gcno/.gcda sidecars — the second
# compilation overwrites the first's .gcno and both objects race
# writes into the one .gcda, so gcov silently reports 0% for one of
# them ("stamp mismatch with notes file"). Detect it here and fail
# loudly instead. (Found the hard way: src/al/link/LinkSweep.cpp vs
# src/al/link/sweep/LinkSweep.cpp — see docs/Version.md
# Disclosed limitations / Open section.)
dupes=$(find ../../src -name "*.cpp" | xargs -n1 basename \
        | sort | uniq -d)
if [ -n "$dupes" ]; then
    echo "coverage_merge.sh: WARNING — duplicate source basenames" >&2
    echo "detected; their gcov data is unreliable (sidecar collision):" >&2
    for d in $dupes; do
        echo "  $d:" >&2
        find ../../src -name "$d" -exec echo "    {}" \; >&2
    done
    echo "Coverage numbers for these files will be wrong (likely 0%)." >&2
    echo "Rename one of each pair before trusting this report." >&2
fi
...
for f in coverage/merged/*.gcno; do
    ...
    case "$base" in
        ...
        *) gcov -b -o coverage/merged "$f" > /dev/null 2>&1 \
               || echo "coverage_merge.sh: gcov failed for $f (stamp" \
                       "mismatch? see the duplicate-basename warning" \
                       "above) — continuing" >&2 ;;
    esac
done
