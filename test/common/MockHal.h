...
public:
    ...
    // Real UART hardware/driver has a small, bounded TX queue —
    // the field logs' "EspHal begin: ... txBuf=381" is exactly
    // this. tx() there returns fewer bytes than requested (or 0)
    // once the queue is full; the caller is expected to tolerate
    // a partial/zero write, same as any embedded UART driver.
    // txBuf here was unbounded, so a burst of logical retransmit
    // calls (AutoLink's onNak fast-retransmit is deliberately
    // undamped — see loopback_multichunk_test) could queue
    // unlimited bytes with no real-world analogue, growing to a
    // std::bad_alloc under the fieldsoak's sustained-load
    // scenario. Bounding it makes the sim's backpressure match
    // real hardware instead of hiding it.
    size_t txCap = 65536; // generous headroom; catches runaway growth only, not
                          // realistic FIFO sizing
