// Closed-loop wire simulator:
// two MockHal adapters + shared clock.
#pragma once
...
public:
    WireSim(AutoLinkConfig cfg = AutoLinkConfig())
        : mA_(new MockHal()), mB_(new MockHal()), a_(*mA_, true, cfg),
          b_(*mB_, false, cfg) {
        mA_->peer = mB_.get();
        mB_->peer = mA_.get();
...
private:
    ...
    void drivePing_() {
        ...
        uint8_t buf[AUTOLINK_DEFAULT_MAX_MSG];
        int n;
        ...
        for (auto &s : pendingA_) {
            ...
            if (debug_log_) {
                std::cerr << "[drivePing_] sendMsg(" << len << ") -> "
                          << (ok ? "true" : "false")
                          << " stateA=" << (int)sim_.getStateA()
                          << " facadeCache=" << sim_.pendingCountA()
                          << std::endl;
            }
            if (ok) {
                s.active = true;
    ...
    void drivePong_() {
        ...
        int n;
        int sent = 0;
        while ((n = sim_.linkB().recvMsg(buf, sizeof buf)) > 0 &&
