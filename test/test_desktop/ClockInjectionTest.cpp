// Clock injection: deterministic timer scheduling
// through MockHal::pumpClock.
...
{
    AutoLinkConfig cfg;
    std::cout << "\n=== Test: idle watchdog drops "
                 "link after cfg.idleTimeoutMs ==="
              << std::endl;
    ...
    int dropB =
        (int)(postB.discCount - preB.discCount);
    std::cout << "  drops: A=" << dropA
              << " B=" << dropB
              << " (timerFiredCalls A="
              << mA.timerFiredCalls
              << " B=" << mB.timerFiredCalls << ")"
              << std::endl;
    ...
    assert(mA.timerFiredCalls + mB.timerFiredCalls >
           0);
...
{
    std::cout << "\n=== Test: ACK timeout at "
                 "ACK_RTO_MS triggers retransmit ==="
              << std::endl;
...
{
    std::cout
        << "\n=== Test: SWP/LCK stall forces BREAK ==="
        << std::endl;
    WireSim sim;
    sim.setFrameDropPct(0);
    TwoNodeFixture fix(sim);
    ...
    int breaksAfter =
        mA.sendBreakCalls + mB.sendBreakCalls;
    std::cout << "  breaks before=" << breaksBefore
              << " after=" << breaksAfter << " (delta="
...
{
    std::cout << "\n=== Test: cobsSeq wraps cleanly, "
                 "cache stays bounded (merged) ==="
              << std::endl;
    ...
    std::cout << "  after wrap-driven run: cacheA="
              << cacheA << " cacheB=" << cacheB
              << " stateA=" << (int)fix.getStateA()
              << std::endl;
...
void test_idle_watchdog_is_per_node()
{
    ...
    autolink::MockHal &mARef =
        const_cast<autolink::MockHal &>(mA);
    mARef.runFor(2000);
    int fired = mARef.timerFiredCalls;
    std::cout << "  A timer fired " << fired
              << " times in 2s simulated" << std::endl;
    assert(fired > 0);
    std::cout << "PASS" << std::endl;
}
...
{
    std::cout << "\n=== Test: keepalive fires at "
                 "idleTimeoutMs/3 (the fix) ==="
              << std::endl;
    autolink::WireSim sim;
    sim.setFrameDropPct(0);
    autolink::TwoNodeFixture fix(sim);
    ...
    autolink::MockHal &mA =
        const_cast<autolink::MockHal &>(sim.rawA());
    ...
    std::cout << "  A txBuf delta after 10s simulated="
              << txDelta << " timerFiredCalls="
              << mA.timerFiredCalls << std::endl;
...
{
    std::cout
        << "\n=== Test: forced drop transitions "
           "OK->SWP, timer re-armed (the fix) ==="
        << std::endl;
    autolink::WireSim sim;
    sim.setFrameDropPct(0);
    autolink::TwoNodeFixture fix(sim);
    ...
    std::cout << "  timer before drop=" << timerBefore
              << " after drop=" << timerAfter
              << std::endl;


    for (int i = 0; i < 20; i++)
        mA.pumpClock(60);
    std::cout << "  stateA after pump="
              << (int)fix.getStateA()
              << " timerFiredCalls="
              << mA.timerFiredCalls << std::endl;
    std::cout << "PASS" << std::endl;
...
{
    std::cout << "\n=== Test: idle watchdog combined "
                 "TX+RX check (the fix) ==="
              << std::endl;

    {
        assert(autolink::decideIdleWatchdog(6000, 6000,
                                            5000) ==
               autolink::IdleAction::Drop);

        assert(autolink::decideIdleWatchdog(6000, 0,
                                            5000) ==
               autolink::IdleAction::Hold);

        assert(autolink::decideIdleWatchdog(0, 6000,
                                            5000) ==
               autolink::IdleAction::Hold);

        assert(autolink::decideIdleWatchdog(4000, 4000,
                                            5000) ==
               autolink::IdleAction::Hold);
    ...
    autolink::MockHal &mA =
        const_cast<autolink::MockHal &>(sim.rawA());
    autolink::MockHal &mB =
        const_cast<autolink::MockHal &>(sim.rawB());
    autolink::Stats preA;
    fix.nodeA().getStats(preA);
    autolink::Stats preB;
    fix.nodeB().getStats(preB);
    ...
    assert(dropA + dropB >= 1);
    std::cout
        << "PASS (decision table ok, both-silent "
           "drops, active-TX holds link)"
        << std::endl;
}
