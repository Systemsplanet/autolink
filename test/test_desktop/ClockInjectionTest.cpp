...
void test_ack_timeout_retransmits() {
    ...
    // v5.1.45: okTickMs() returns max(idleTimeoutMs/3, 50, ACK_RTO_MS).
    // With the default idleTimeoutMs=5000 the tick interval is 1666ms,
    // way too slow to retx within 1500ms of pumpClock. Drop
    // idleTimeoutMs to 300 so keep=100 < RTO=100 → okTickMs=100ms.
    ...
    //
    // v5.1.45: also drive some chunks from A in each step so the
    // ARQ cache has unacked entries. Without this, all chunks sent
    // in the pre-loop are ACKed (pre-loop pipes both ways) and the
    // retx loop has nothing to do.
    uint8_t msg[64];
    for (int i = 0; i < 64; i++) msg[i] = (uint8_t)i;
    for (int i = 0; i < 30; i++) {
        // One message per step — 30 messages × ~1 chunk = ~30 chunks
        // sent in this loop. None are ACKed (one-way pipe).
        if (i % 3 == 0) fix.nodeA().sendMsg(msg, 64);
...
// v5.1.45: merged sender_cobsSeq_wraparound + cache_survives_cobsSeq_wraparound.
// Both were closed-loop drivers that ran txSeq past 256 and asserted
// cache stays bounded — the first ran 1500 steps and asserted
// < 200 (loose, to verify drain), the second ran 500 steps and
// asserted <= 240 (cap bound). Both pinned the v5.1.39 one-owner
// design's safety: after wrap, seq=0 is a fresh slot, not stale.
// Combining into one test gives one WireSim run that asserts both
// invariants together.
void test_cobsSeq_wraparound_does_not_pollute_cache() {
    std::cout << "\n=== Test: cobsSeq wraps cleanly, cache stays bounded (v5.1.45 merged) ===" << std::endl;
    autolink::WireSim sim;
    sim.setFrameDropPct(0);
    autolink::TwoNodeFixture fix(sim);
    bringToOk(fix, sim);
    // 1500 steps drives txSeq well past 256 (WINDOW=32 messages
    // per ping burst × 4 messages/loop iteration max).
    for (int i = 0; i < 1500; i++) fix.step(10);
    // Drain in-flight with a few larger steps.
    ...
    assert(cacheA < 200);
    assert(cacheB < 200);
    // Hard cap: even under wire stutter or retransmit storms,
    // the cache must never exceed ARQ_CACHE_CAP=240.
    assert(cacheA <= 240);
    assert(cacheB <= 240);
    std::cout << "PASS" << std::endl;
}
