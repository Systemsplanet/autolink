...
void test_facade_arqcache_rejects_when_full()
{
    std::cout << "\n=== Test: ARQ cache gate (pool-backed, size="
              << AutoLink::test_arqPoolSize() << ") ===" << std::endl;
    AutoLink link(0, 16, 17, true);

    uint8_t payload[32] = { 0 };
    int filled = 0;
    const int POOL_SIZE = AutoLink::test_arqPoolSize();
    for (int i = 0; i < POOL_SIZE; i++) {
        link.test_arqCache_put((uint8_t)i, payload, 32,
                               (uint8_t)chunkCount(32));
        filled++;
    }
    assert(filled == POOL_SIZE);
    assert(link.arqCacheSizeForTest() == POOL_SIZE);

    uint8_t overflowSeq = (uint8_t)POOL_SIZE;
    link.test_arqCache_put(overflowSeq, payload, 32, (uint8_t)chunkCount(32));
    assert(link.arqCacheSizeForTest() == POOL_SIZE);
...
void test_facade_link_reset_clears_cache()
{
    ...
    for (int i = 0; i < overshoot; i++) {
        link.test_arqCache_put((uint8_t)i, payload, 64,
...
void test_facade_repeated_drops_do_not_latch_gate()
{
    ...
    for (int cycle = 0; cycle < 5; cycle++) {
        for (int i = 0; i < overshoot; i++) {
            link.test_arqCache_put((uint8_t)((cycle * 32 + i) & 0xFF), payload,
                                   32, (uint8_t)chunkCount(32));
        }
        int beforeDrop = link.arqCacheSizeForTest();
        if (beforeDrop != POOL_SIZE) {
...
void test_facade_cache_miss_does_not_clear_neighbours()
{
    ...
    auto retxTracer = [](uint8_t, void *) -> bool { return false; };
...
{
    std::cout
        << "\n=== Test: ARQ_CACHE_POOL_SIZE is 64 (v5.3.45 bump from 16) ==="
        << std::endl;
    AutoLink link(0, 16, 17, true);
    uint8_t payload[16] = {};
    const int POOL_SIZE = AutoLink::test_arqPoolSize();
    ...
    for (int i = 0; i < POOL_SIZE; i++) {
        link.test_arqCache_put((uint8_t)i, payload, 16,
                               (uint8_t)chunkCount(16));
    }
    assert(link.arqCacheSizeForTest() == POOL_SIZE);
    assert(!link.test_arqCache_hasRoom());
    ...
    link.test_arqCache_freeBySeq(0);
    assert(link.test_arqCache_hasRoom());
    std::cout << "PASS (pool=" << AutoLink::test_arqPoolSize() << " holds "
              << POOL_SIZE << " messages before back-pressure)" << std::endl;
}
