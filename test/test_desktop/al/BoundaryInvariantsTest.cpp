...
void test_test_forward_resync_evicted_from_user_config() {
    ...
    // Link holds a private member, set via the test accessor —
    // and gated so the ESP32 build carries no test surface.
    auto memberPos = linkH.find("testForwardResync_");
    assert(memberPos != std::string::npos);
    auto guardPos = linkH.rfind("#ifdef AUTOLINK_HOST_TEST", memberPos);
    assert(guardPos != std::string::npos &&
           "testForwardResync_ must sit under AUTOLINK_HOST_TEST");

    // LinkRx reads the Link member, not the removed config
    // field, inside the same host-test gate.
    auto rxPos = linkRx.find("testForwardResync_");
    assert(rxPos != std::string::npos);
    auto rxGuard = linkRx.rfind("#ifdef AUTOLINK_HOST_TEST", rxPos);
    assert(rxGuard != std::string::npos);
...
void test_resweep_logs_free_heap_on_device() {
    ...
    // The fix shape: the count-and-resweep branch
    // (count && state == State::OK) emits a Log
    // call that mentions both "resweep" and
