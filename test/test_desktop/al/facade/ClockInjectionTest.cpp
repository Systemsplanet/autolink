...
#include "../../src/al/link/sweep/LinkDecision.h"
...
void test_idle_timeout_drops_link() {
    ...
    // Drain the post-bringUp PONG / LOCK-CMD TX stamp
    // past the FAST_IDLE_TX_MS=1000ms window so the
    // asymmetric-idle check (rxAge > 300ms && txAge <
    // 1000ms && arq_.pendingCount() > 0) doesn't fire
    // on the first OK tick. The 1500 ms pre-roll
    // pushes lastTxMs past 1000 ms with margin so
    // the test period runs entirely outside the
    // asymmetric window.
    ...
    // Open 5: the idle watchdog must be a no-op on a clean quiet
    // link. Unconditionally dropping on rxAge>idleTimeoutMs &&
    // txAge>idleTimeoutMs fires on a healthy loopback with no
    // pending ARQ and no recent errors; the watchdog must only drop
    // when there's an actual reason to suspect the link
    // (arqPending>0 or frameErrs>0).
    //
    // Pumping 4500 ms (not 5500) keeps rxAge under the new
    // slave's DropPeerReset threshold (2 * campBudgetMs =
    // 2 * 4000 ms = 8000 ms). The branch is slave-only and
    // fires on an empty-window, no-pending-ARQ shape —
    // exactly the "clean quiet link" the test was asserting
    // would not drop. The two assertions are not in
    // conflict; the test was written before the slave wedge
    // was identified as its own fast-exit class. The
    // 4500 ms window still exercises DropIdle / DropDeadLink
    // (both gated on pending>0 || frameErrs>0, both Keep
    // here), which is the test's actual contract.
    assert(dropA + dropB == 0);

    assert(mA.timerFiredCalls + mB.timerFiredCalls > 0);
    std::cout << "  PASS (symmetric idle does NOT drop; both nodes "
                 "stay up across 4500 ms quiet)"
              << std::endl;
}
...
void test_idle_watchdog_is_per_node() {
    ...
    autolink::Link a(mA, cache, true, cfg);
    a.begin();
...
void test_keepalive_emitted_at_half_of_idle_timeout() {
    // The OK-state keepalive (PING_CMD emitted every
    // idleTimeoutMs/2 from onTimerOk_unlocked) closes
    // the silent-peer false-positive on a duty-cycled
    // link. Pin the presence: a quiet link that has
    // been idle for half the idle window MUST emit a
    // PING so the peer's lastRxMs stays fresh. The
    // link is otherwise empty (no app traffic, no
    // pending chunks) — the keepalive is the only
    // source of TX bytes after bringToOk returns.
    std::cout
        << "\n=== Test: OK-state keepalive emits a PING after idleTimeoutMs/2 ==="
        << std::endl;
    autolink::WireSim sim;
    sim.setFrameDropPct(0);
    autolink::TwoNodeFixture fix(sim);
    ...
    std::cout << "  A txBuf delta after 10s simulated=" << txDelta
              << " timerFiredCalls=" << mA.timerFiredCalls << std::endl;
    // OK-state timer re-arms (Link::okTickMs drives
    // onTimerOk_unlocked's repeat-arm) AND the
    // keepalive fires every idleTimeoutMs/2 = 2.5s,
    // so we expect >= 3 PINGs across 10s of idle
    // (the master half + the slave PONG-reply
    // half). Toggle off the keepalive (delete the
    // keepalive from onTimerOk_unlocked) -> this
    // assertion flips red.
    assert(mA.timerFiredCalls > 1);
    assert(txDelta > 0);
    std::cout << "  PASS (keepalive PING on the wire: txBuf delta=" << txDelta
              << " after 10s idle)" << std::endl;
}
...
void test_idle_watchdog_combined_tx_rx_asymmetric_only() {
    // The OK-state timer has no symmetric "both quiet for
    // idleTimeoutMs" drop path: a link quiet in BOTH directions for
    // the full idleTimeoutMs stays up. Only the asymmetric idle
    // check (FAST_IDLE_RX_MS / FAST_IDLE_TX_MS in LinkTimers.cpp,
    // TX active + RX silent) drops.
    std::cout
        << "\n=== Test: symmetric idle does not drop (asymmetric still does) ==="
        << std::endl;
    ...
    // Drain the post-bringUp PONG / LOCK-CMD TX
    // stamp so the asymmetric-idle check
    // (txAge < FAST_IDLE_TX_MS=1000ms) does NOT
    // fire on the first OK-state tick during the
    // 5500 ms measurement window. With the ASYNC
    // tick clamp at reorderHoldMs/2 (=750 ms at the default),
    // the first tick lands 750 ms into the
    // window; if lastTxMs is still inside the
    // 1000 ms window the asymmetric check would
    // sendBreak and bump discCount. The 1500 ms
    // pre-roll pushes lastTxMs past 1000 ms with
    // margin so the test period runs entirely
    // outside the asymmetric window.
    mA.runFor(1500);
    mB.runFor(1500);
    autolink::Stats preA;
    fix.nodeA().getStats(preA);
    ...
    // clean idle link (no pending ARQ, no
    // recent errors). With maxBurstPerLoop=0
    // and no traffic, both nodes' rxAge and txAge exceed
    // idleTimeoutMs=5000 within the 5500 ms window — but the
    // symmetric watchdog requires an actual reason to suspect the
    // link (arqPending>0 OR frameErrs>0). Without those, neither
    // node drops: quiet alone is fine.
    assert(dropA + dropB == 0);
    std::cout << "  PASS (clean idle link: no drops, watchdog is a "
                 "no-op without pending ARQ or recent errors)"
              << std::endl;
}
...
int main() {
    ...
    test_keepalive_emitted_at_half_of_idle_timeout();
    test_forced_drop_transitions_ok_to_swp();
    test_pumpClock_terminates_finitely();
    test_idle_watchdog_combined_tx_rx_asymmetric_only();
