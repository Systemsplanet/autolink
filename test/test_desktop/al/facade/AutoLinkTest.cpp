...
void test_txDelayMs_default_and_setter() {
    ...
    AutoLinkConfig cfg;
    // Default txDelayMs is 0: full line-rate by default. A nonzero
    // default costs a fresh sketch without WiFi (and therefore
    // without the dashboard's poll pressure) ~half the wire's
    // natural throughput for no operator-visible reason. The
    // dashboard dropdown still defaults to 0; operators who want a
    // throttled bench run can pick a value via the widget or set
