...
bool sendSyncDriven(WireSim &sim, AutoLink &node, const uint8_t *payload,
                    int len) {
    AutoLinkTestAccessor t(node);
    // Wait out the post-lock wire-settle window
    // (AUTOLINK_WIRE_SETTLE_MS = 50ms) BEFORE
    // emitting the SYNC frame. The settle gate
    // drops the first wave of post-lock wire
    // frames silently; in SYNC mode, the
    // production ladder's RTO doesn't fire
    // (the per-chunk timer is only armed in
    // SWP state), so a frame dropped by the
    // settle gate is lost forever and the
    // test's stillWaiting() never returns
    // false. 100ms pre-emit wait is 2x the
    // gate, with margin for the test's 1ms
    // per-step clock tick.
    for (int i = 0; i < 100; i++)
        sim.step(1);
    bool beginOk = t.sendMsgBeginForTest(payload, len);
    if (!beginOk)
        return false;
    int budget = t.syncAckTimeoutMsForTest() + 50;
    for (int i = 0; i < budget; i++) {
        sim.step(1);
        if (!t.sendMsgStillWaitingForTest())
            return true;
    }
    return !t.sendMsgStillWaitingForTest();
}
} // namespace
...
void test_sync_mode_clean_wire_zero_pool_use() {
    ...
    // immediately. Pins: SYNC mode is
    // reachable through the runtime API.
...
int main() {
    Log::log().setLevel(Log::Level::WARNING);
    std::cout << "=== SYNC Mode Tests ===" << std::endl;
