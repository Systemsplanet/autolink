...
// Pins the OK-state CTRL scan's CRC8 pre-check: the scan must
// validate the CRC before consuming five bytes as a CTRL frame.
// `0xAA 0x55` is the worst-case fill collision — without the
// pre-check, the framer eats the first 5 bytes of payload as if
// they were a CTRL frame, fails the CRC, and after errThreshold of
// these across the suite drops the link.
//
// Test layout:
...
void test_fill_byte_roundtrip_large() {
    ...
    for (int b = 0; b <= 0xFF; b++) {
        ...
        for (int sz : sizes) {
            ...
            // failure: five back-to-back messages total 16670
            // bytes, which is over one 1 s budget at 115200
            // (11520 B/s). A real app pumps and retries; so do
            // we, bounded, so a genuine refusal still trips the
            // assert.
            bool ok = false;
            for (int attempt = 0; attempt < 400 && !ok; attempt++) {
                ok = ping.sendMsg(tx.data(), sz);
                if (ok)
                    break;
