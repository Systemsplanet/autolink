...
void test_closed_loop_with_forced_drops() {
    std::cout
        << "\n=== Test: closed loop with 2% drop + forced dropLink() every "
           "16000 cycles (killer) ==="
        << std::endl;
    ...
    // dropLink() only resets the caller's own side; the peer reacts to the
    // resulting BREAK on its own clock, and — unlike a HealthWatchdog reset
    // detected symmetrically on both sides — only the peer's reset carries
    // preservePreferredBaud, so it spends up to three ~750 ms camps at the
    // old baud before falling back to a full P1 walk alongside the side
    // that called dropLink(). A single recovery this way costs upward of
    // 10 s of simulated time. Spacing forced drops 800 cycles apart is
    // faster than that recovery can complete, so every drop after the
    // first lands mid-recovery and the link never reconverges — not a
    // wedge, a cadence shorter than the path it drives. 16000 gives one
    // recovery comfortable headroom before the next drop.
    sim.setForcedDropEvery(16000);
    ...
    int dropsSeen = 0;
    const int kCycles = 50000; // three forced drops, each fully recovered
    ...
    for (int cycle = 0; cycle < kCycles; cycle++) {
        ...
        if (sim.dropsInjected() > dropsSeen) {
            ...
            f.maxBurstPerLoop = 0;
            // 15000, not 2000: dropLink() only resets the caller's side,
            // so the peer's camp-at-old-baud-then-fall-back-to-P1
            // reconvergence (see the comment above) has to run its full
            // course before pendingCountB can reach 0 too.
            for (int drain = 0; drain < 15000; drain++) {
                f.step(1);
                if (f.pendingCountA() == 0 && f.pendingCountB() == 0)
                    break;
            }
    ...
    std::cout << "  " << kCycles << " cycles, " << sim.dropsInjected()
              << " drops, " << sim.framesDropped() << " wire bytes dropped, "
              << "total bytes A->B: " << f.totalBytesAtoB() << std::endl;
    if (f.totalBytesAtoB() < 1000) {
