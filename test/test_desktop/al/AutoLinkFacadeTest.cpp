...
void test_facade_link_arqcache_is_facade_cache_by_reference() {
    // AutoLink owns an ArqCache by value;
    // the Link holds a reference to that
    // same cache. Reference semantics make
    // the contract unbreakable at compile
    // time: the Link cannot exist without
    // the cache, and the cache outlives the
    // Link because AutoLink's members are
    // destroyed in reverse declaration
    // order (link first, then arqCache_).
    //
    // If a future refactor moves the
    // ArqCache to unique_ptr or reintroduces
    // a public setArqCache(IArqCache*)
    // post-construction setter, the
    // pointer-equality check below still
    // passes at the time of construction —
    // but a subsequent dtor order swap
    // dangles the pointer silently. The
    // compile-time pin (Link ctor takes
    // IArqCache&) catches that class of
    // bug; this runtime check confirms the
    // wiring is correct today.
    std::cout
        << "\n=== Test: AutoLink facade passes its ArqCache to Link by reference ==="
        << std::endl;
    AutoLink link(0, 16, 17, true);
    AutoLinkTestAccessor t(link);

    ArqCache *facade_cache = t.arqCache();
    IArqCache *link_cache = nullptr;
    {
        LinkTestAccessor lt(*t.link());
        link_cache = &lt.arqCache();
    }
    assert(facade_cache != nullptr);
    assert(link_cache != nullptr);
    assert(static_cast<const void *>(facade_cache) ==
           static_cast<const void *>(link_cache));
    // The same identity, no copy: passing
    // by reference means there's exactly
    // one cache per AutoLink. Reverting
    // the fix to a copy or a default-null
    // pointer would either make the
    // facade's write invisible to the
    // link (cache miss on every retx) or
    // crash on first retx dereference.
    std::cout << "PASS (facade cache and link cache are the same instance)\n";
}
