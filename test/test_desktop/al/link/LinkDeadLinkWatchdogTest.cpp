// Regression: SYNC mode must have a dead-link
// backstop. The bench log (07/07, SYNC, 512000
// baud) showed a link re-lock then go fully silent
// (tx=0/rx=0 on both sides) for 2+ minutes with
// zero recovery events — the earlier removal of the
// symmetric-idle watchdog left a mutually-quiet
// SYNC link with no drop path. The fix: a narrower
// dead-link check that fires on `pending>0 + mutual
// quiet past idleTimeoutMs`. Narrower than the old
// symmetric check (which fired on any mutual quiet,
// including a clean idle link with nothing queued)
// and from DropIdle (which only fires for ASYNC and
// accepts frameErrs as an alternative signal).
//
// Pins:
