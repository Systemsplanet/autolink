...
void test_rearmSlot_math() {
    ...
    // budget-ring slot the original onSent wrote.
...
void test_source_pin() {
    ...
    const std::string fallbackBlock = "P3 preferredBaud_ relock missed -> "
    ...
    // lock across setSpd. The prior shape released and
    // reacquired with portMAX_DELAY inside a timer callback,
    // which deadlocks when the holder of the mutex needs to
    ...
    // fix to the prior shape can't reintroduce a silent
    // dead branch. The actual call is
...
void test_enterphase1_fallback_contract() {
    ...
    // Pin 6b — the deadlock-freeness pin. The prior shape
    // (hw.unlock() / hw.setSpd() / hw.lock() around the
    // P3/resweepPrefPending_ branch) could deadlock on
    // ESP32: onTimer() runs on the FreeRTOS timer-service
    // (daemon) task, and reacquiring the link lock with
    // portMAX_DELAY inside a timer callback blocks forever
    // when the holder of the mutex needs to post a
    // startTimer/stopTimer command serviced by the same
    // daemon task.
    //
    // The new shape holds the link lock the whole window.
    // A hook that flips state/phase under onSetSpd is now
    // a no-op for the link's behavior (the lock is held
