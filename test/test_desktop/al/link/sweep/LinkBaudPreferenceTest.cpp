...
static void test_v53_preferred_baud_recorded_on_lock() {
    ...
    // Sweep locks the master at SOME baud the P2/P3 sweep
    // confirms; preferredBaud_ mirrors that. MockHal's
    // instant wire lets the round-trip dwell formula's
    // rt-clamp-to-50ms dominate, so the first P3 attempt
    // can expire before 2 ACKs land — master falls through
    // to the next baud and locks there. The contract is
    // "preferredBaud_ is recorded"; the specific index
    // depends on dwell timing. Accept any valid index.
    assert(d.preferredBaud < cfg.allowedBaudsCount);
    std::cout << "PASS (locked at preferredBaud=" << (int)d.preferredBaud << ")"
              << std::endl;
}
...
static void test_v53_resweep_starts_at_preferred_baud() {
    ...
    Diag d;
    ping.getDiag(d);
    // Sweep locks at SOME baud (see comment in the test
    // above for why the specific index is timing-dependent).
    assert(d.preferredBaud < cfg.allowedBaudsCount);
    ...
    mHal2.peer = &sHal2;
    sHal2.peer = &mHal2;
    Link ping2(mHal2, cache, true, cfg);
    Link pong2(sHal2, cache, false, cfg);
    ...
    mHal3.peer = &sHal3;
    sHal3.peer = &mHal3;
    Link ping3(mHal3, cache, true, cfg);
    Link pong3(sHal3, cache, false, cfg);
    ping3.begin();
    pong3.begin();
...
static void test_v53_short_message_coalesces() {
    ...
    Link pong(sHal, cache, false, cfg);
    negotiate_to_ok(ping, pong, mHal, sHal);
    // AL90-2/3: open the post-lock evidence gate
    // — the test isn't exercising the gate, it's
    // exercising the coalesce path. Mark peer
    // response seen so the gate's wall-clock hold
    // doesn't block the 5 back-to-back sends.
    LinkTestAccessor accPing(ping);
