...
// this release: decideSwpTick + SwpAction removed
// alongside the LCK state machine (the only callers
// routed the sweep arm through SwpAction::EnterLck
// into onTimerLck_unlocked, which never fired). The
// production sweep now uses direct state-machine branches
// (decidePongPhase1Timeout / decideMasterPhase1Ack /
// decidePongPhase2Ping / decideMasterPhase2Ack /
// decidePongPhase3Ping / decideMasterPhase3Ack) which
// are still pinned by their own tests below. Absence
// pin: a future refactor that re-adds decideSwpTick
// without a production caller is dead code and should
// come with a SourceGrepFor("decideSwpTick") absence
// pin in CompileCheckTest (not asserted here because the
// absence is already implied by the LCK removal).

// this release: decideLckTick + LckAction removed
// alongside the LCK state machine (no production caller
// reached LCK state). Absence pin: a future refactor that
// re-adds decideLckTick outside a (c) host-pinned test
// would be a wire-shape change and needs the wire gate
