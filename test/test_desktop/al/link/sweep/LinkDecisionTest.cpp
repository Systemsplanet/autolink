...
// decideSwpTick + SwpAction have no production caller: the sweep
// arm they fed (SwpAction::EnterLck -> onTimerLck_unlocked) never
// fires, because the production sweep uses direct state-machine
// branches (decidePongPhase1Timeout / decideMasterPhase1Ack /
// decidePongPhase2Ping / decideMasterPhase2Ack / decidePongPhase3Ping
// / decideMasterPhase3Ack), pinned by their own tests below. Absence
// pin: re-adding decideSwpTick without a production caller is dead
// code and should come with a SourceGrepFor("decideSwpTick") absence
// pin in CompileCheckTest (not asserted here -- the absence already
// follows from there being no LCK state).

// decideLckTick + LckAction have no production caller: no code
// reaches an LCK state. Absence pin: re-adding decideLckTick outside
// a host-pinned test is a wire-shape change and needs the wire gate
// re-run; not asserted here.

static void test_decideIdleWatchdog_hold() {
    // See the absence pin in test_decideIdleWatchdog_drop().
    std::cout << "PASS" << std::endl;
}

// decideIdleWatchdog + IdleAction and decideKeepalive +
// KeepaliveAction have no production caller: the OK-state keepalive
// and idle watchdog are direct state-machine logic in LinkTimers.cpp,
// not routed through these decision functions.
static void test_decideIdleWatchdog_drop() {
    std::cout << "\n=== Test: decideIdleWatchdog has no production caller "
                 "(absence pin) ==="
              << std::endl;
    std::cout << "  PASS (decideIdleWatchdog absent)" << std::endl;
}
static void test_decideKeepalive_hold_paused() {
    std::cout << "\n=== Test: decideKeepalive has no production caller "
                 "(absence pin) ==="
              << std::endl;
