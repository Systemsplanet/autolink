...
static void test_onbreak_in_ok_still_resets() {
    ...
    assert(LinkTestAccessor(pong).sweepPhase() == SweepPhase::PHASE1 &&
           "slave falls back to P1 slowest — its preferredBaud_ is "
           "cleared on the BREAK path, matching the kickoff path");
    std::cout << "  PASS (OK onBreak resets: state=SWP, phase=P3 "
                 "= preferredBaud_ fast re-lock)"
              << std::endl;
}
...
static void test_slave_promote_to_phase2_stays_at_slowest() {
    ...
    FILE *f = fopen("../../src/al/link/LinkSweepGlue.cpp", "r");
    assert(f);
    char buf[65536];
