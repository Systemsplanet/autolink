...
void test_attempt_cap_enforced() {
    ...
    acc.lockOkForTest(0);
    int fired = 0;
    for (int i = 0; i < LinkTestAccessor::BAUD_UPGRADE_MAX_ATTEMPTS_FOR_TEST + 3; i++) {
        hal.pumpClock(100);
    ...
    std::cout << "  fired=" << fired << " (cap="
              << LinkTestAccessor::BAUD_UPGRADE_MAX_ATTEMPTS_FOR_TEST << ")" << std::endl;
...
void test_full_renegotiation_clears_upgrade_memory() {
    ...
    assert(acc.bestProvenBaudForTest() == LinkTestAccessor::NO_PREFERRED_BAUD_FOR_TEST &&
