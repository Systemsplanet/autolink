...
static void test_v531_phase1_locks_at_slowest_baud() {
    ...
    if (!mHal.txBuf.empty()) {
        ...
        mHal.clearTx();
        sHal.events()->onRx(b.data(), (int)b.size());
    }
    if (!sHal.txBuf.empty()) {
        std::vector<uint8_t> b = sHal.txBuf;
        sHal.clearTx();
        mHal.events()->onRx(b.data(), (int)b.size());
    }
...
static void test_v531_heartbeat_miss_drops_quickly() {
    ...
    // errors). Pre-this-fix shape dropped
    // here at idleTimeoutMs; post-fix the
    // watchdog stays quiet. To exercise the
    // drop path the test must stage a reason
    // to suspect the link — mark a seq
    // pending so the asymmetric-idle check
    // fires (txAge<FAST_IDLE_TX_MS, rxAge>
    // FAST_IDLE_RX_MS, arqPending>0).
...
// Pong's P2->P3 transition. The prior shape did
// sweep_.setPhase(PHASE3); sweep_.reset();
// sweep_.enterPhase3(*this, b); — three writes
// where one suffices. The new shape calls enterPhase3
// directly. enterPhase3 itself sets phase_=PHASE3
// and resets phase3Acks_, so a prior setPhase+reset
// is a no-op write followed by an override. Pin:
// pong enters P3 with phase3Acks()==0 on a P2 PING,
// and the spdI it locks onto is its current spdI
// (the baud the P2 sweep parked on).
static void test_pong_p2_ping_enters_p3_with_zero_acks() {
    NullArqCache cache;
    std::cout << "\n=== pong P2 PING enters P3 (clean setPhase) ==="
              << std::endl;
    AutoLinkConfig cfg;
    cfg.allowedBauds[0] = 115200;
    cfg.allowedBauds[1] = 9600;
    ...
    // Park pong in P2 at slowest baud so the PING
    // handler sees a P2 PING.
    pongT.setSweepPhase(SweepPhase::PHASE2);
    assert(pongT.sweepPhase() == SweepPhase::PHASE2);
    assert(pong.getCurrentSpdIndex() == 1); // slowest baud

    // Drive a PING from master to pong. Set pong's
    // baud to the master's baud first so the
    // pipe_data baud-filter lets the PING through.
    // Clear master's stale txBuf so exactly one PING
    // reaches pong (the second PING would push pong
    // from P3 to phase3Acks==1 on the P3 PING
    // handler — we want to pin the P2->P3 transition
    // in isolation, with phase3Acks==0 right after
    // enterPhase3).
    sHal.setSpd(cfg.allowedBauds[1]);
    mHal.clearTx();
    // Manually emit one master PING so the test
    // is self-contained (doesn't depend on
    // ping.begin()'s exact sendFrame timing).
    ping.onTimer();
    pipe_data(mHal, sHal);
    assert(pongT.sweepPhase() == SweepPhase::PHASE3);
    assert(pongT.phase3Acks() == 0);
    assert(pongT.phase3Baud() == pong.getCurrentSpdIndex());
    std::cout << "PASS (pong entered P3 cleanly: phase=3 acks=0 baud="
              << pongT.phase3Baud() << ")" << std::endl;
}
...
int main() {
    ...
    test_pong_p2_ping_enters_p3_with_zero_acks();
