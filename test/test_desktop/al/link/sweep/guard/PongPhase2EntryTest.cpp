...
void test_apply_pong_promote_to_phase2_does_work() {
    std::cout
        << "\n=== Pin 2: applyPongSwpAction_unlocked PromoteToPhase2 case "
           "does the work ==="
        << std::endl;
    // applyPongSwpAction_unlocked moved to LinkSweepGlue.cpp
    // after the god-class split (renamed from LinkSweep.cpp
    // to break the basename collision with sweep/LinkSweep.cpp).
    std::string linkCpp =
        readFile(projectRoot() + "/src/al/link/timers/LinkSweepGlue.cpp");
...
// LinkSweepGlue.cpp (so the new path is reachable) and the
