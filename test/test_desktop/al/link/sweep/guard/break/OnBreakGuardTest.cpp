...
// Pin 1: onBreak() in P1 must NOT hard-reset the sweep.
// Unconditionally calling reset_unlocked(true) tears down SWP state
// on every spurious break (post-setSpd glitches in P1 -> pingSample
// resets -> sweep restart -> another spurious break — a livelock).
...
// switch glitches a break, and the old code
...
static void test_onbreak_in_ok_still_resets() {
    ...
    // states — OK keeps earlier behavior
    // modulo the BREAK-interrupt-coalescing
    // pin (a single BREAK in OK is debounced; two
    // BREAKs outside the coalesce window confirm
    // the reset). The P1-walk skip + fast P3
    // re-lock at the proven baud is the same as
    // before.
    // : a single OK-state BREAK is debounced.
    // The first onBreak arms the confirm window
    // (state stays OK); the second, outside the
    // coalesce window, fast-confirms. No time has
    // passed in this synthetic test (now=0), so
    // the second onBreak fires the fast-confirm
    // path. Pinned by BreakConfirmTest.
    LinkTestAccessor pingAcc(ping);
    LinkTestAccessor pongAcc(pong);
    ping.onBreak();
    assert(ping.getState() == State::OK &&
           pingAcc.breakSuspectMsForTest() != 0 &&
           ": a single OK-state BREAK arms the confirm window, "
           "state stays OK until the deadline or a second BREAK");
    pong.onBreak();
    assert(pong.getState() == State::OK &&
           pongAcc.breakSuspectMsForTest() != 0 &&
           ": pong's first OK-state BREAK arms pong's confirm window");
    // Pump past BREAK_CONFIRM_MS (150 ms) on both sides.
    // The confirm deadlines fire and confirm the resets.
    for (int i = 0; i < 10; i++) {
        mHal.pumpClock(20);
        sHal.pumpClock(20);
    }
    assert(ping.getState() == State::SWP && pong.getState() == State::SWP);
    // BREAK-triggered resweep with a recorded preferredBaud_
    // now skips the P1 walk and tries a fast P3 re-lock at
    // the proven baud. The phase at the END of onBreak is P3
    // (master → P3 at preferred; slave → P1 slowest — the master's
    // WireSim/bench baud-match snap helper drives the slave's lock
    // from there).
    assert(LinkTestAccessor(ping).sweepPhase() == SweepPhase::PHASE3 &&
           "master enters P3 at preferredBaud_ via reset_unlocked(..., "
           "true) for BREAK-triggered resweep");
    // : the slave also takes the preserved-baud
    // fast path on a HealthWatchdog-reason reset (the
    // BREAK-driven reset uses HealthWatchdog). The
    // preserved-baud fast path closes the master/slave
    // sweep-phase skew that otherwise lands both sides
    // on the 9600 fallback baud.
    assert(LinkTestAccessor(pong).sweepPhase() == SweepPhase::PHASE3 &&
           ": slave also takes the preserved-baud fast path on "
           "HealthWatchdog-resets (master/slave sweep-phase skew fix)");
    std::cout << " PASS (OK onBreak resets: state=SWP, phase=P3 "
                 "= preferredBaud_ fast re-lock on both sides)"
              << std::endl;
}
...
static void test_slave_promote_to_phase2_stays_at_slowest() {
    std::cout << "\n=== Pin 4: slave PromoteToPhase2 keeps slowest baud "
                 "(source-grep) ==="
              << std::endl;
    // applyPongSwpAction_unlocked lives in LinkSweepGlue.cpp
    // after the god-class split (renamed from LinkSweep.cpp
    // to break the basename collision with sweep/LinkSweep.cpp).
    FILE *f = fopen(
        testRepoPath("src/al/link/timers/LinkSweepGlue.cpp").c_str(), "r");
    assert(f);
    char buf[65536];
...
// loop period (~50-100 ms). The old value of the same
...
static void test_esphal_break_debounce_ms_above_loop_period() {
    std::cout << "\n=== Pin 5: EspHal.h BREAK_DEBOUNCE_MS >= 100 ms ==="
              << std::endl;
    FILE *f = fopen(testRepoPath("src/al/hal/EspHalUartEvent.h").c_str(), "r");
    assert(f);
    char buf[16384];
    size_t n = fread(buf, 1, sizeof(buf) - 1, f);
    buf[n] = 0;
...
static void test_ping_setPaused_stamps_sweep_stall() {
    std::cout << "\n=== Pin 6: Ping::setPaused stamps tSweepStall_ ==="
              << std::endl;
    FILE *f = fopen(testRepoPath("src/al/pingpong/Ping.h").c_str(), "r");
    assert(f);
    char buf[65536];
    size_t n = fread(buf, 1, sizeof(buf) - 1, f);
    buf[n] = 0;
