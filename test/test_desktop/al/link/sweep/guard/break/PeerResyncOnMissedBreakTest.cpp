...
static void test_peer_resyncs_on_missed_break() {
    ...
    // AL-09: kickoff() (called from begin(), above) now bumps
    // sweepEpoch_ once on both master and slave, outside the
    // count && state==OK gate, so a fresh kickoff is observable
    // to the peer even on a first boot (previously the master
    // ran an entire session at epoch=0). A.sweepEpoch_ is
    // therefore 1 after begin(), not 0, before this test's
    // explicit reset_unlocked(true) call below runs.
    uint8_t epochBeforeA = LinkTestAccessor(a).sweepEpochForTest();
    assert(epochBeforeA == 1 &&
           "precondition: A.sweepEpoch_ is 1 after begin()'s kickoff "
           "bump (AL-09)");
    LinkTestAccessor(a).resetLink(true);
    uint8_t epochAfterA = LinkTestAccessor(a).sweepEpochForTest();
    assert(epochAfterA == 2 &&
           "Pin 1 pre: A.sweepEpoch_ must bump from 1 to 2 on "
    ...
    // Epoch state pins: A's epoch is 2 (1 from begin()'s
    // kickoff bump, AL-09; +1 from the reset that started
    // this scenario). B's peer-epoch was latched on the
    // epoch-mismatch PING and matches, also 2. No spurious
    // epoch known/unknown flags.
    assert(LinkTestAccessor(a).sweepEpochForTest() == 2 &&
           "Pin 1: A.sweepEpoch_ must be 2 (1 from begin()'s kickoff "
           "bump, AL-09, +1 from the reset that started the scenario)");
    assert(LinkTestAccessor(b).peerSweepEpochForTest() == 2 &&
           "Pin 1: B.peerSweepEpoch_ must be 2 (latched from the "
...
// bump under the count && state==OK gate must stay in
// lockstep with discCount (both fire together on a genuine
// OK-session disconnect). This is independent of kickoff()'s
// own direct sweepEpoch_++ (AL-09, outside this gate, so a
// fresh kickoff is observable to the peer even on a first
// boot) — this pin only covers the disconnect-triggered bump
// inside reset_unlocked itself.
...
// sweep-adjacent PING must carry sweepEpoch_ in byte[2], including
// the OK-state idle keepalive in onTimerOk_unlocked — every
// production sender of the 5-byte control-frame shape goes
// through sendSweepFrame_unlocked for exactly this reason (see
// CompileCheckTest #16: the alternate, txSeq-in-byte[2] sender was
// dead code and has been removed). If a future call site ever
// bypasses sendSweepFrame_unlocked and writes byte[2] as
// application data sequence instead, the very next routine
// keepalive reads as a false "peer restarted" signal on the
// receiving side and tears down a perfectly healthy, idle link.
// Pin 1 deliberately sets idleTimeoutMs=0 to keep the keepalive
// out of its own scenario, so it can't catch this — this pin
// exists specifically to cover what Pin 1 excludes.
