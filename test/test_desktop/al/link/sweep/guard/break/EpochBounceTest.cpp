...
static void test_peer_epoch_mismatch_reset_does_not_bump_epoch() {
    ...
    // AL-09: begin()'s kickoff() call, above, now bumps
    // sweepEpoch_ once directly (outside this reset's gate), so
    // A.sweepEpoch_ is 1 after bringToOk, not 0.
    ...
    assert(acc.sweepEpochForTest() == 2 &&
...
static void test_epoch_storm_does_not_cascade() {
    ...
    assert(LinkTestAccessor(a).sweepEpochForTest() == 2 &&
           "precondition: A's own break must bump its epoch once more, "
           "from 1 (begin()'s kickoff bump, AL-09) to 2");
    ...
    assert(epochA == 2 &&
           "Pin 3: A's epoch must still read 2 — a further bump means B's "
    ...
    std::cout << " Pin 3 PASS (epoch settled at 2, no cascade, both sides "
                 "OK)"
              << std::endl;
}
