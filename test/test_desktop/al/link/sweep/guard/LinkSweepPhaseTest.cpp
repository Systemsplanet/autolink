...
static void test_v531_heartbeat_miss_drops_quickly() {
    ...
    // link (no pending ARQ, no errors) — it must stay quiet at
    // idleTimeoutMs. To exercise the drop path the test must stage
    // a reason to suspect the link: mark a seq pending so the
    // asymmetric-idle check fires (txAge<FAST_IDLE_TX_MS,
    // rxAge>FAST_IDLE_RX_MS, arqPending>0).
