...
//  must eventually re-converge on the original baud
...
// re-arm is a no-op if the peer missed the original
...
static void test_p3_preferred_baud_retry_resends_ping() {
    ...
    buf[got] = 0;
    fclose(f);
    // The P3 retry branch is gated on
    // resweepPrefAttempts_ < RESWEEP_PREF_MAX_ATTEMPTS
    // (the new shape). Within that branch, the body
    // must call sendSweepFrame_unlocked(PING_CMD).
    // Find the gate, then walk forward to the matching
    // return; assert PING re-send is in the body.
    const char *gate =
        strstr(buf, "resweepPrefAttempts_ < RESWEEP_PREF_MAX_ATTEMPTS");
    assert(gate &&
           "Pin 3: LinkTimersSwp.cpp must keep the "
           "resweepPrefAttempts_ retry branch (the "
           "RESWEEP_PREF_MAX_ATTEMPTS gate is the new "
           "shape).");
    ...
    // a no-op if the peer missed the original PING.
    assert(strstr(bodybuf, "sendSweepFrame_unlocked(PING_CMD)") != NULL &&
           ...
           "no-op if the peer missed the original PING outright "
...
static void test_slave_p3_camp_bounded_by_attempts() {
    ...
    FILE *fOk = fopen(testRepoPath("src/al/link/timers/LinkTimersOk.cpp").c_str(), "r");
    assert(fOk);
