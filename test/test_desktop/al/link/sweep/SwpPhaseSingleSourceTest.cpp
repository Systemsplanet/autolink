...
//      LinkWire.h (single source, no
//      duplicate in LinkSweepGlue.cpp).
//   2. roundTripMs() lives in LinkDecision.h
//      and is called from LinkSweepGlue.cpp's
...
void test_phase3_acks_needed_lives_in_link_context() {
    std::cout << "\n=== PHASE3_ACKS_NEEDED single-source in LinkWire.h ==="
              << std::endl;
    std::string ctxH = readFile(projectRoot() + "/src/al/link/LinkWire.h");
    assert(ctxH.find("constexpr int PHASE3_ACKS_NEEDED") != std::string::npos);
    // Must NOT be defined in LinkSweepGlue.cpp
    std::string sweepCpp =
        readFile(projectRoot() + "/src/al/link/sweep/LinkSweep.cpp");
    assert(sweepCpp.find("PHASE3_ACKS_NEEDED =") == std::string::npos);
    std::cout << "  PASS (defined in LinkWire.h, not in LinkSweepGlue.cpp)"
              << std::endl;
}
void test_round_trip_ms_lives_in_link_decision() {
    ...
    // called from LinkSweepGlue.cpp's applyMasterSwpAction
    // (P3 rearm dwell) after the god-class split.
    std::string linkCpp =
        readFile(projectRoot() + "/src/al/link/LinkSweepGlue.cpp");
...
void test_handle_swp_uses_is_lock_payload() {
    ...
    // handleSwp_unlocked lives in LinkSweepGlue.cpp
