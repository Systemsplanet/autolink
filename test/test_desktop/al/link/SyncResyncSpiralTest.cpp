...
void test_pin_sync_nak_skips_cache_retx() {
    ...
    std::string src;
    for (const char *path :
         { "../../src/al/link/LinkRx.cpp", "src/al/link/LinkRx.cpp",
    ...
    auto naked = src.find("arq_.onNaked(missingCobsSeq");
    assert(naked != std::string::npos);
    // GBN shape: onNak returns early for SYNC before it can ever
    // reach the GBN window-resend call — the SYNC retx ladder
    // (sendMsg's blocking syncAwaitAcked) owns SYNC recovery
    // instead.
    auto guard = src.find("cfg.mode == AutoLinkConfig::Mode::SYNC", naked);
    auto resend = src.find("gbnResendWindow_unlocked(", naked);
