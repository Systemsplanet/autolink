// Regression: ASYNC NAK-driven retransmit must fire immediately,
// inline in onNak — not deferred to the next OK-timer tick. A
// single pendingRetxBase_ slot that the timer drains one frame per
// okTickMs, with each fresh NAK overwriting the prior base, throttles
// recovery to one chunk per tick under continuous line-rate traffic
// with real loss — far slower than the receiver's reorder window
// expiry, so multi-chunk ASYNC messages never reassemble and the
// link churns resweeps (delivery collapses to ~0). This is the
// "random mode is worse than sequential" failure: SYNC self-throttles
// via inline waitForAck and never builds a backlog.
//
// Pins:
