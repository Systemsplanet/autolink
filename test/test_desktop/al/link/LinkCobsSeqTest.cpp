...
void test_lost_msgs_burst_vs_single() {
...

    {
        ...
        Diag d2;
        c.getDiag(d2);
        // Open 6: a gap held past reorderHoldMs whose
        // missing cobsSeq never arrives is abandoned —
        // rxSeq advances past the dropped slot so the
        // contiguous-resume logic can move on. With
        // reorderHoldMs=0 every held frame expires
        // immediately on the next onRx (dropExpired runs
        // at the top of onPayload), and the advance
        // collapses the subsequent gap counter.
        //
        // Sequence (post-fix):
        //   frame 0: dropExpired (empty), deliver,
        //            rxSeq=0, gaps=0
        //   frame 4: dropExpired (empty), gap→hold,
        //            gaps=1
        //   frame 5: dropExpired drops slot 4,
        //            reorderAdvanceRxSeq(4)→rxSeq=4,
        //            classifyGap(5,4,...) = Contiguous,
        //            hold slot 5 (no new gap), gaps=1,
        //            lostMsgs=1
        //   frame 7: dropExpired drops slot 5,
        //            reorderAdvanceRxSeq(5)→rxSeq=5,
        //            classifyGap(7,5,...) = Gap (1),
        //            hold slot 7, gaps=2, lostMsgs=2
        assert(d2.gaps == 2);
        // Lost msgs is 1, not 2: slot 4 expires
        // when frame 5 arrives (Contiguous after
        // the rxSeq advance), but slot 5 is never
        // held (frame 5 is Contiguous, not Gap).
        // The frame 7 gap holds slot 7 instead,
        // but lostMsgs only bumps on EXPIRY of a
        // held slot, not on Gap detection (which
        // is reversible if the missing seq lands
        // within reorderHoldMs).
        assert(d2.lostMsgs == 1);
    }
