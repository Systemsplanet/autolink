// Message reassembly: MSG_HDR coalescing, multi-chunk,
// CRC verify, resync.
...
struct TestCache {
    ...
    void insert(uint8_t, const uint8_t *, int,
                uint8_t chunks)
    {
        count += chunks;
    }
...
void test_message_size_sweep()
{
...

    {
        ...
        std::cout << " [0] no-op (true, no bytes)"
                  << std::endl;
    }
    ...
    for (size_t idx = 0; idx < sizes.size(); idx++) {
        ...
        std::vector<uint8_t> tailTx(tailSz),
            tailRx(tailSz + 32);
        ...
        int gotTail = b.recvMsg(tailRx.data(),
                                (int)tailRx.size());
        if (gotTail != tailSz) {
            std::cerr << " size=" << sz
                      << " tail size " << tailSz
                      << " expected got=" << gotTail
                      << std::endl;
            assert(false);
        }
        for (int i = 0; i < tailSz; i++)
        ...
        expectedRx += (uint64_t)(sz + tailSz) +
            (uint64_t)MSG_HDR * 2;

        std::cout << " [" << sz << " B / tail "
                  << tailSz << " B] ok"
                  << " (txBuf wire bytes so far: "
                  << mHal.txBuf.size() << ")"
                  << std::endl;
    }
...

    {
        ...
        assert(b.recvMsg(rA.data(), (int)rA.size()) ==
               3);
        for (int i = 0; i < 3; i++)
            assert(rA[i] == mA[i]);
        assert(b.recvMsg(rB.data(), (int)rB.size()) ==
               10000);
        for (int i = 0; i < 10000; i++) {
            if (rB[i] != mB[i]) {
                std::cerr
                    << " burst: B mismatch at i=" << i
                    << std::endl;
                assert(false);
            }
        }
        assert(b.recvMsg(rC.data(), (int)rC.size()) ==
               7);
        for (int i = 0; i < 7; i++)
            assert(rC[i] == mC[i]);
        assert(b.recvMsg(rA.data(), (int)rA.size()) ==
               0);
        std::cout << " [burst: 3 + 10000 + 7] ok"
                  << std::endl;
    }

    expectedTx += (uint64_t)(3 + 10000 + 7);
    expectedRx += (uint64_t)(3 + 10000 + 7) +
        (uint64_t)MSG_HDR * 3;
    Stats as, bs;
    a.getStats(as);
    b.getStats(bs);
    ...
    std::cout << " [stats] sender tx=" << as.tx
              << " (payload) receiver rx=" << bs.rx
              << " (payload+MSG_HDR)" << std::endl;
...
void test_app_buffer_null_simulates_disconnect()
{
    ...
    std::cout << "PASS (recv returned 0, gaps=0 (flow "
                 "control, not wire error))"
              << std::endl;
}
...
{
    AutoLinkConfig cfg;
    std::cout << "\n=== Test: Corrupt MSG_HDR Drops "
                 "Single Frame, Not Whole Buffer ==="
              << std::endl;
    MockHal mHal, sHal;
    mHal.peer = &sHal;
    sHal.peer = &mHal;
    ...
    assert(b.read(scratch.data(), availBefore) ==
           availBefore);
    assert(b.available() == 0);
    for (int i = 0; i < 6; i++)
        sHal.appBuf.push(0);
    ...
    std::cout << "PASS (corrupt header dropped, m1 + "
                 "m2 preserved)"
              << std::endl;
}
...
void test_send_rejections_log_errors()
{
    ...
    a.dropLink();
    b.dropLink();
    assert(a.sendMsg((const uint8_t *)"z", 1) ==
           false);
...
{
    std::cout << "\n=== Test: 240-chunk carries, "
                 "241st rejected (ARQ cap) ==="
              << std::endl;
    ...
    a.setArqCacheHooks(
        ...
        [](void *ctx) -> bool {
            return ((test_internal::TestCache *)ctx)
                ->hasRoom();
        },
        [](uint8_t baseSeq, const uint8_t *b, int len,
           uint8_t chunks, void *ctx) {
            ((test_internal::TestCache *)ctx)
                ->insert(baseSeq, b, len, chunks);
        },
        [](void *ctx) {
            ((test_internal::TestCache *)ctx)
                ->clearAll();
    ...
    assert(b.recvMsg(rx.data(), (int)rx.size()) ==
           (int)big.size());
    for (size_t i = 0; i < big.size(); i++) {
        assert(rx[i] == big[i]);
    }
    ...
    uint8_t base2 = 0;
    assert(a.sendMsg(oneMore.data(),
                     (int)oneMore.size(),
                     &base2) == false);

    std::cout << "PASS (240 chunks carry, 241st "
                 "rejected by ARQ cap gate)"
              << std::endl;
}
...
{
    AutoLinkConfig cfg;
    std::cout << "\n=== Test: Corrupt MSG_HDR with "
                 "L>maxMsg Resyncs Forward ==="
              << std::endl;
    MockHal mHal, sHal;
    mHal.peer = &sHal;
    sHal.peer = &mHal;
    ...
    for (int i = 0; i < 10; i++)
        assert(rx[i] == m1[i]);
    std::cout << "PASS (oversize-L header resynced, "
                 "m1 preserved)"
              << std::endl;
}
...
void test_corrupt_msg_header_resync_drops_bytes()
{
    ...
    for (int i = 0; i < 10; i++)
        assert(rx[i] == m2[i]);
    std::cout << "PASS (resync dropped 6 leading "
                 "corrupt bytes, m2 preserved)"
              << std::endl;
}
...
void test_corrupt_msg_header_resync_rejects_false_boundary_v5_1_54()
{
    AutoLinkConfig cfg;
    std::cout
        << "\n=== Test: Resync rejects false boundary "
           "(payload CRC mismatch, fix) ==="
        << std::endl;
    MockHal mHal, sHal;
    mHal.peer = &sHal;
    sHal.peer = &mHal;
    ...
    assert(got == 0);
    std::cout
        << "PASS (false-boundary candidate rejected, "
           "buffer cleared, no wrong-length delivery)"
        << std::endl;
}
...
{
    AutoLinkConfig cfg;
    std::cout << "\n=== Test: recvMsg Buffer Too "
                 "Small Drains Payload ==="
              << std::endl;
    MockHal mHal, sHal;
    mHal.peer = &sHal;
    sHal.peer = &mHal;
    ...
    std::cout << "PASS (buffer too small -> -1, "
                 "payload drained, link OK)"
              << std::endl;
}
...
{
    AutoLinkConfig cfg;
    std::cout << "\n=== Test: Corrupt MSG_HDR With No "
                 "Resync Clears Buffer ==="
              << std::endl;
    MockHal mHal, sHal;
    mHal.peer = &sHal;
    sHal.peer = &mHal;
    ...
    uint8_t ok[8] = { 0xDE, 0xAD, 0xBE, 0xEF,
                      1,    2,    3,    4 };
    ...
    for (int i = 0; i < 8; i++)
        assert(rx[i] == ok[i]);
    std::cout << "PASS (no-resync path cleared 200 "
                 "bytes, next msg OK)"
              << std::endl;
}
...
void test_corrupt_payload_byte_crc_reject()
{
    ...
    assert(b.getState() == State::OK);
    std::cout << "PASS (payload bit-flip caught by "
                 "CRC, no leakage)"
              << std::endl;
}
...
void test_recvMsg_empty_buffer()
{
    ...
    assert(b.recvMsg(rx, sizeof(rx)) == 0);
    assert(b.getErrCount() == 0);
    std::cout << "PASS (empty recvMsg returns 0)"
              << std::endl;
}
...
void test_zero_byte_send_silent_noop()
{
    ...
    assert(b.available() == 0);
    std::cout << "PASS (sendMsg(0)=true silent, "
                 "write(0)=0 silent, no wire bytes)"
              << std::endl;
}
...
void test_resetDiag_zeros_cobsseq_counters()
{
    ...
    for (int i = 0; i < 4; i++)
        assert(rx[i] == m[i]);
    std::cout << "PASS (resetDiag zeros "
                 "gaps/stale/lostMsgs, idempotent, "
                 "link stays OK)"
              << std::endl;
}
...
{
    std::cout
        << "\n=== Test: Multi-chunk loss — recvMsg "
           "returns -1, lostMsgs bumps ==="
        << std::endl;
    AutoLinkConfig cfg;
    cfg.streamBufferSize = 70000;
    cfg.maxMsg = 65535;
    ...
    std::cout << "PASS (multi-chunk loss surface "
                 "documented; "
                 "full baseSeq-tied resync deferred "
                 "to a future release)"
              << std::endl;
}
...
int main()
{
    std::cout << "=== Running ALinkMessage Tests ==="
              << std::endl;
