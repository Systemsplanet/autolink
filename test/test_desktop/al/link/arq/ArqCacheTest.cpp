...
void test_hasRoom_is_O1_after_pool_drain() {
    std::cout
        ...
        << std::endl;
    // hasRoom() must be O(1) — a POOL_SIZE-entry scan is tolerable
    // at WINDOW=32 but a real bottleneck the moment POOL_SIZE grows.
    // A poolFree_ counter keeps it O(1) at any pool occupancy.
    //
...
void test_pool_size_covers_pipeline_window() {
    ...
    // The window is now injected at
    // construction (default = the
    // autolink-wide pipeline value);
    // dropping it below POOL_SIZE/2 trips
    // the ctor assert in the host build
    // and an error log + silent drop in
    // production. This test pins the
    // runtime behaviour: build a cache
    // with a 1-window budget and assert
    // the ctor accepts it AND POOL_SIZE
    // can absorb 2*window worth of
    // inserts before hasRoom() flips.
    if (ArqCache::POOL_SIZE < 2 * AUTOLINK_ARQ_PIPELINE_WINDOW) {
        std::cerr << "\nFAIL: ArqCache::POOL_SIZE=" << ArqCache::POOL_SIZE
                  << " must be >= 2*AUTOLINK_ARQ_PIPELINE_WINDOW="
                  << (2 * AUTOLINK_ARQ_PIPELINE_WINDOW)
    ...
    // the same window retx-held
    // neighbours before any insert
    // silently drops. The (2*window + 1)-th
    // insert is the boundary that should
    // fail.
    ArqCache c;
    int w = c.window();
    assert(w == AUTOLINK_ARQ_PIPELINE_WINDOW);
    uint8_t payload[16] = {};
    for (int i = 0; i < 2 * w; i++)
        c.insert((uint8_t)i, payload, 16);
    assert(c.size() == 2 * w);
    ...
    std::cout << "PASS (POOL_SIZE=" << ArqCache::POOL_SIZE
              << " >= 2*window=" << (2 * w) << ")" << std::endl;
}

void test_freeRoom_returns_min_of_slots_and_pool() {
    std::cout
        << "\n=== Test: freeRoom() returns min(SLOTS-pending, poolFree) ==="
        << std::endl;
    // Real impl path: min(SLOTS - pendingCount_, poolFree_).
    // Cover the three shape boundaries:
    //   1. fresh cache: pool full, 0 pending -> SLOTS
    //   2. half-pool + half-slots: returns the smaller (slots)
    //   3. pool drained (testFillPool): returns 0
    ArqCache c;
    uint8_t buf[16];
    for (int i = 0; i < 16; i++)
        buf[i] = (uint8_t)i;

    // 1. fresh: SLOTS=256, POOL_SIZE=64 -> min = POOL_SIZE
    int r = c.freeRoom();
    assert(r == ArqCache::POOL_SIZE);
    assert(r > 0);
    std::cout << "  fresh cache freeRoom=" << r << std::endl;

    // 2. half-slots used, pool still mostly free.
    // Pick n < POOL_SIZE so inserts actually claim pool
    // buffers (otherwise halfSlots inserts would log
    // pool-exhausted errors). 16 inserts: pendingCount_=16,
    // poolFree_=POOL_SIZE-16=48; freeRoom() = min(240, 48) = 48.
    int n = 16;
    for (int i = 0; i < n; i++)
        c.insert((uint8_t)i, buf, 16);
    r = c.freeRoom();
    int want = (ArqCache::SLOTS - n < ArqCache::POOL_SIZE - n)
        ? ArqCache::SLOTS - n
        : ArqCache::POOL_SIZE - n;
    assert(r == want);
    std::cout << "  16 inserts freeRoom=" << r << " (min(SLOTS-" << n << "="
              << (ArqCache::SLOTS - n) << ", POOL_SIZE-" << n << "="
              << (ArqCache::POOL_SIZE - n) << "))" << std::endl;

    // 3. pool drained -> freeRoom() is 0
    c.testFillPool();
    r = c.freeRoom();
    assert(r == 0);
    std::cout << "  pool drained freeRoom=" << r << std::endl;

    std::cout << "PASS (freeRoom is the min of slots-pending and poolFree)"
              << std::endl;
}
...
int main() {
    ...
    test_freeRoom_returns_min_of_slots_and_pool();
