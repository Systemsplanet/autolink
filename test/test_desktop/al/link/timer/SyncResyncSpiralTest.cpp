...
void test_pin_sync_retx_ladder_resends_before_drop() {
    ...
    // AL87-11: the give-up decision is now time-based
    // (elapsed-since-first-step >= maxRetx * syncAckTimeoutMs)
    // instead of attempt-count-based, so the clock must actually
    // advance by syncAckTimeoutMs between steps to exercise the
    // same "three real RTO expiries" this test's comment always
    // claimed to simulate — a real waitForAck timeout always
    // burns one full syncAckTimeoutMs, so this is a more faithful
    // simulation, not a behavior change for this path.
    assert(t.syncRtoStep() && "attempt 1 must resend");
    assert(t.syncAttempt() == 1);
    hal.now += (uint32_t)cfg.syncAckTimeoutMs;
    assert(t.syncRtoStep() && "attempt 2 must resend");
    hal.now += (uint32_t)cfg.syncAckTimeoutMs;
    assert(t.syncRtoStep() && "attempt 3 must resend");
    assert(t.syncAttempt() == 3);
    hal.now += (uint32_t)cfg.syncAckTimeoutMs;
    // Ladder exhausted at maxRetx=3 * syncAckTimeoutMs=50 -> the
    // next step declines (caller then drops + BREAKs).
    assert(!t.syncRtoStep() &&
           "ladder must stop at maxRetx * syncAckTimeoutMs");
    assert(t.syncAttempt() == 3);
...
// Pin 5 (AL87-11/AL87-24) — a NAK storm arriving well inside a
// single syncAckTimeoutMs interval must NOT exhaust the ladder.
// Before AL87-11 the give-up check was `attempt >= maxRetx`, so
// calling syncRtoStep() repeatedly with the clock barely moving
// (as a real NAK-driven wake does — waitForAck returns in a few ms,
// not after the full RTO) burned all maxRetx attempts in
// milliseconds and declared mid-message desync while the peer was
// alive and answering. The fix makes the give-up time-based: the
// ladder must keep resending until real wall-clock time
// (maxRetx * syncAckTimeoutMs) has elapsed, no matter how many
// times it's been stepped.
void test_pin_nak_storm_does_not_exhaust_ladder_early() {
    std::cout << "\n=== Pin 5: a NAK storm inside one RTO does not exhaust "
                 "the ladder ==="
              << std::endl;
    ArqCache arq{ AUTOLINK_ARQ_PIPELINE_WINDOW };
    AutoLinkConfig cfg;
    ...
    // Field shape: NAKs land ~6ms apart. Step the ladder 5 times
    // (== maxRetx) advancing the clock by only 6ms each time —
    // total elapsed 30ms, nowhere near the 2500ms budget.
    for (int i = 0; i < 5; i++) {
        bool r = t.syncRtoStep();
        assert(r &&
               "a NAK-storm-fast step must still resend — real elapsed "
               "time is far under the ladder's budget");
        hal.now += 6;
    }
    assert(t.syncAttempt() == 5);
    // Even though attempt has now reached the OLD cap (maxRetx=5),
    // the ladder must keep going because only ~30ms of the 2500ms
    // budget has elapsed.
    bool r6 = t.syncRtoStep();
    assert(r6 &&
           "attempt count alone must no longer end the ladder — only "
           "real elapsed time may");
    std::cout << "  PASS (ladder survived " << (t.syncAttempt())
              << " fast steps in ~36ms; still had budget)" << std::endl;
}
...
void test_pin_sync_nak_skips_cache_retx() {
    ...
    // AL90-9 / AL92-6 / AL92-17: onNak's
    // ASYNC-side suppression checks (base
    // stuck, same-event dedup) must bump
    // the NAK counter WITHOUT reseating
    // the RTO clock — that's
    // noteSuppressedNak. But the NAK that
    // is actually acted on (the one that
    // reaches the resend call) MUST
    // reseat the RTO the same as any
    // other event that puts a fresh
    // frame on the wire — that's
    // onNaked(). A whole-body "onNaked
    // must never appear" grep (the
    // AL90-9..AL92-17 shape) is wrong:
    // it silently halved ASYNC delivery
    // in the app_gap_stop_soak itest
    // (46/31 vs the pre-regression
    // baseline of 85/58) because it
    // removed the RTO reseat from the
    // accepted resend path too. Assert the split
    // precisely instead: at least two
    // noteSuppressedNak calls exist
    // between the SYNC guard and the
    // resend call (the base-stuck and
    // dedup branches), and exactly one
    // onNaked(missingCobsSeq call exists,
    // positioned immediately before the
    // resend call (the accepted path).
    auto nakOnlyFirst = src.find("arq_.noteSuppressedNak(missingCobsSeq", nak);
    assert(nakOnlyFirst != std::string::npos && nakOnlyFirst > guard &&
           nakOnlyFirst < resend &&
           "noteSuppressedNak (the counter-only bump) must sit on "
           "the ASYNC side of the SYNC guard, before the resend");
    auto nakOnlySecond =
        src.find("arq_.noteSuppressedNak(missingCobsSeq", nakOnlyFirst + 1);
    assert(nakOnlySecond != std::string::npos && nakOnlySecond < resend &&
           "both the base-stuck AND same-event-dedup branches must "
           "call noteSuppressedNak — a suppressed NAK of either "
           "kind must not defer the RTO ladder");
    auto nakedPos = src.find("arq_.onNaked(missingCobsSeq", nak);
    assert(nakedPos != std::string::npos && nakedPos > nakOnlySecond &&
           nakedPos < resend &&
           "the accepted NAK (the one that reaches the resend call) "
           "must call onNaked() to reseat the RTO clock — a real "
           "resend just went on the wire, the same as any other "
           "TX event");
    auto secondNaked = src.find("arq_.onNaked(missingCobsSeq", nakedPos + 1);
    assert((secondNaked == std::string::npos || secondNaked > resend) &&
           "onNaked (RTO reseat) must not appear on either "
           "suppressed branch — only on the accepted resend path");
...
int main() {
    ...
    test_pin_nak_storm_does_not_exhaust_ladder_early();
