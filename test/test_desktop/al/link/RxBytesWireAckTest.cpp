// Runtime + structural pin for the inbound wire
// ACK/NAK/CTRL byte counter.
//
// Pre-this-fix shape: rxBytes was only bumped on
// the app-payload path (onPayload→pushAppBuf→
// rxBytes += n). In the PingPong protocol Pong
// does NOT echo the payload back — its reply is
// a wire-level ACK frame, dispatched to onAck
// (not onPayload). The sender's UART was
// continuously receiving ACK frames but its RX
// total stayed 0 forever, while the dashboard's
// Pong side showed a healthy non-zero TX total.
// The fix: bump rxBytes by the inbound wire-frame
// size in onAck and onNak (RX_ACK_WIRE_BYTES=8
// and RX_NAK_WIRE_BYTES=6 are the COBS+framing
// totals for the 5-byte ACK and 3-byte NAK raw
// frames respectively; declared in
// LinkContext.h alongside MAX_CHUNK).
//
// Pin set:
//
...
//      constants live in LinkContext.h (single
...
// scenario after a successful send) — pre-fix
// shape didn't bump at all.
void test_on_ack_bumps_rx_bytes() {
    ...
    assert(delta == RX_ACK_WIRE_BYTES &&
           "onAck must bump rxBytes by RX_ACK_WIRE_BYTES; the pre-fix "
...
void test_constants_defined_and_referenced() {
    ...
    std::ifstream f(root + "/src/al/link/LinkRx.cpp");
    assert(f.good());
    std::stringstream ss;
    ss << f.rdbuf();
    ...
    std::cout << "  PASS (constants defined in LinkContext.h; "
...
void test_wire_ack_only_path_bumps_rx() {
    ...
    assert(delta > 0 &&
           ...
           "the pre-fix shape saw ping.rxBytes stuck at 0 forever "
           "because the wire-ACK reply path didn't contribute to the "
           "rx counter (only the app-payload path did)");
