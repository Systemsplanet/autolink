...
//      for both modes).
//   3. okTickMs() clamps at
//      reorderHoldMs/2 in ASYNC mode so
//      the OK-state timer tick (which
//      drives the ARQ retransmit loop)
//      always fires a retx before the
//      receiver's reorder buffer expires
//      the held frames. SYNC mode is
...
void test_pin_uart_rx_buffer_scales_by_mode() {
    ...
    cfg.mode = autolink::AutoLinkConfig::Mode::ASYNC;
    ...
    cfg.mode = autolink::AutoLinkConfig::Mode::SYNC;
...
void test_pin_uart_floors_derive_perChunk_from_MAX_CHUNK() {
    ...
    // al/link/LinkContext.h, included
    ...
    assert(src.find("kChunkCap = 250") == std::string::npos &&
           ...
           "moment the chunk cap is bumped in LinkContext.h");
    ...
    assert(rxBody.find("MAX_CHUNK") != std::string::npos &&
           ...
           "(from al/link/LinkContext.h) so a chunk-cap bump can't "
...
void test_pin_okTickMs_clamps_to_reorder_hold_half() {
    std::cout << "\n=== Pin 3: okTickMs clamps at reorderHoldMs/2 in ASYNC ==="
              << std::endl;
    // The pre-this-release shape capped the
    // tick at max(idleTimeoutMs/3, 50,
    // syncAckTimeoutMs) = 3333 ms at the
    // defaults, while reorderHoldMs=1500 ms.
    // A single ARQ loss became unrecoverable
    // because the receiver's reorder buffer
    // expired the held frames before the
    // sender's OK-state tick could fire a
    // retx. The fix clamps the tick at
    // reorderHoldMs/2 in ASYNC mode so the
    // retx always lands before the receiver
    // gives up.
    std::string src = readFile(projectRoot() + "/src/al/link/LinkSweep.cpp");
    assert(!src.empty());
    (void)src;
    // Read the function body.
    auto p = src.find("int Link::okTickMs() const");
    assert(p != std::string::npos);
    ...
    // The clamp branch references both
    // cfg.mode == ASYNC and reorderHoldMs/2.
    assert(body.find("cfg.mode == AutoLinkConfig::Mode::ASYNC") !=
               std::string::npos &&
           "okTickMs must gate the reorderHoldMs clamp on ASYNC mode "
           "so SYNC (which never walks the ARQ table) keeps its old "
           "behavior");
    assert(body.find("cfg.reorderHoldMs / 2") != std::string::npos &&
           "okTickMs must clamp at reorderHoldMs/2 in ASYNC mode so "
           "the ARQ retx tick always fires before the receiver's "
           "reorder buffer expires the held frames");
    std::cout << "  PASS (okTickMs clamps at reorderHoldMs/2 in ASYNC)"
              << std::endl;
}
...
void test_pin_sendAckFrame_bumps_txBytes() {
    std::cout << "\n=== Pin 4b: sendAckFrame_unlocked bumps txBytes ==="
              << std::endl;
    std::string src = readFile(projectRoot() + "/src/al/link/LinkTx.cpp");
    assert(!src.empty());
    auto p = src.find("void Link::sendAckFrame_unlocked");
    assert(p != std::string::npos);
    auto bodyStart = src.find('{', p);
    int depth = 0;
    size_t bodyEnd = bodyStart;
    ...
    std::string body = src.substr(p, bodyEnd + 1 - p);
    assert(body.find("txBytes +=") != std::string::npos &&
           "sendAckFrame_unlocked must bump txBytes by the encoded "
           "frame length — Pong's wire output is dominated by ACKs, "
           "and the pre-fix shape reported 0 B/sec");
    std::cout << "  PASS (ACK frame bytes counted in txBytes)" << std::endl;
}

void test_pin_sendNakFrame_bumps_txBytes() {
    std::cout << "\n=== Pin 4c: sendCtrlCobsFrame_unlocked (NAK) bumps txBytes "
                 "==="
              << std::endl;
    std::string src = readFile(projectRoot() + "/src/al/link/LinkTx.cpp");
    assert(!src.empty());
    auto p = src.find("void Link::sendCtrlCobsFrame_unlocked");
    assert(p != std::string::npos);
    auto bodyStart = src.find('{', p);
    int depth = 0;
    size_t bodyEnd = bodyStart;
    ...
    std::string body = src.substr(p, bodyEnd + 1 - p);
    assert(body.find("txBytes +=") != std::string::npos &&
           "sendCtrlCobsFrame_unlocked must bump txBytes (NAK frames "
           "are wire output too). Pre-fix shape didn't count them, so "
           "the dashboard under-reported on gap-stop runs");
    std::cout << "  PASS (NAK frame bytes counted in txBytes)" << std::endl;
}
...
// cfg.txDelayMs. The pre-v6.0.11 shape honored
...
void test_pin_ping_cooldown_gate_independent_of_txDelayMs() {
    ...
    assert(src.find("backpressureCoolUntilMs_") != std::string::npos &&
           ...
           "send-loop's cooldown gate reads from; the pre-v6.0.11 shape "
    ...
    assert(gateHeader.find("txDelayMs") == std::string::npos &&
           ...
           "pre-v6.0.11 shape conditioned on "
...
int main() {
    ...
    test_pin_sendAckFrame_bumps_txBytes();
    test_pin_sendNakFrame_bumps_txBytes();
