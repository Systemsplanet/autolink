...
void test_error_counter_link_failures() {
...

    {
        ...
        // Open 5: idle watchdog is a no-op on
        // a clean idle link (no pending ARQ,
        // no recent errors). The pre-this-fix
        // shape dropped the link here;
        // post-fix the link stays OK across
        // the idle window. To exercise the
        // watchdog's drop path we have to
        // create a reason to suspect the link
        // — set a pending ARQ slot via the
        // test hook, then run past
        // idleTimeoutMs.
