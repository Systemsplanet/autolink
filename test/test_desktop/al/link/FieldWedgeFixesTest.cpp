...
void test_pin_2b_waitforack_event_driven_source() {
    ...
    // The body should still have a clearAll bump
    // (the generation_ wake path) so a reset
    // unblocks the wait.
    assert(code.find("generation_") != std::string::npos &&
           "waitForAck must consult generation_ to surface a "
           "mid-wait reset_unlocked() call as a false return "
           "(the current fix uses clearAll's xSemaphoreGive to "
           "wake the waiter; the gen mismatch closes the ABA race).");
    std::cout << "  PASS (xSemaphoreTake inside waitForAck; "
                 "generation guard present)"
              << std::endl;
}

// Pin 3: AutoLink::begin() forwards setMode(cfg.mode)
// to the HAL before hal->begin() so the txBufferFloor
// picks up the right branch. Pinned at the source
// level (the call is unconditional in begin()) and
// behaviourally (a facade constructed with SYNC mode
// ends up with a SYNC-sized tx buffer on the HAL).
void test_pin_3_setmode_forwarded_to_hal_before_begin() {
    std::cout << "\n=== Pin 3: AutoLink::begin() forwards setMode to the "
                 "HAL before hal->begin() ==="
              << std::endl;
    std::string src = readFile(projectRoot() + "/include/AutoLink.h");
    assert(!src.empty());
    auto p = src.find("void begin()");
    assert(p != std::string::npos);
    ...
    std::string code = stripComments(body);
    // Must call hal->setMode(cfg_.mode) before
    // hal->begin() (or any equivalent sync — the
    // field log showed path was
    // forwardable but the begin() block never
    // re-asserted, so a custom IHal that didn't
    // honour setMode was free to drift).
    auto setModePos = code.find("hal->setMode(");
    auto halBeginPos = code.find("hal->begin()");
    assert(setModePos != std::string::npos &&
           "AutoLink::begin() must call hal->setMode(cfg_.mode) — the "
           "the current shape fix is to forward explicitly in begin() so a "
           "custom IHal can't drift out of sync with the facade. The "
           "shape field log showed facade=mode=SYNC HAL=mode=ASYNC "
           "with the 381 B ASYNC tx floor under a 32x254 B SYNC "
           "window — uart_write_bytes silently blocks the loop the "
           "moment >1.5 chunks are queued.");
    assert(halBeginPos != std::string::npos);
    assert(setModePos < halBeginPos &&
           "AutoLink::begin() must call hal->setMode BEFORE hal->begin() "
           "so the txBufferFloor/rxBufferFloor pick up the right mode");
    std::cout << "  PASS (hal->setMode(cfg_.mode) appears before "
                 "hal->begin())"
              << std::endl;
}

// Pin 4: AutoLink::begin() logs an error if the
// HAL's mode disagrees with the facade's at
// post-begin time. Pinned at the source level.
void test_pin_4_begin_logs_mode_mismatch() {
    std::cout << "\n=== Pin 4: AutoLink::begin() logs a mode-mismatch "
                 "error if HAL.mode != facade.mode ==="
              << std::endl;
    std::string src = readFile(projectRoot() + "/include/AutoLink.h");
    assert(!src.empty());
    auto p = src.find("void begin()");
    ...
    assert(!ihal.empty());
    auto stripComments2 = stripComments;
    std::string ihalCode = stripComments2(ihal);
    assert(ihalCode.find("virtual AutoLinkConfig::Mode getMode()") !=
               std::string::npos &&
           "IHal must declare virtual AutoLinkConfig::Mode getMode() "
           "const so AutoLink::begin() can detect a HAL/facade mode "
           "divergence post-begin()");
    // AutoLink::begin() must compare hal->getMode()
    // vs cfg_.mode and log an error on disagreement.
    assert(code.find("hal->getMode()") != std::string::npos &&
           "AutoLink::begin() must call hal->getMode() to compare "
           "against cfg_.mode");
    assert(code.find("cfg_.mode") != std::string::npos);
    assert(code.find("mode mismatch at begin") != std::string::npos &&
           "AutoLink::begin() must log a \"mode mismatch at begin\" "
           "error if hal->getMode() != cfg_.mode — the current fix "
           "adds this so a custom IHal that didn't honour setMode() "
           "is caught before the link starts dropping frames.");
    std::cout << "  PASS (IHal::getMode declared; AutoLink::begin logs "
                 "mismatch error)"
              << std::endl;
}
...
void test_pin_8_peer_stalled_baud_derived() {
    ...
    int w512 = healthPeerStalledMs(512000, 32);
    ...
    // = 5461.
    int w96 = healthPeerStalledMs(9600, 32);
...
void test_pin_10_p3_entry_break_on_every_reset() {
    ...
    assert(p != std::string::npos);
    auto end = src.find("void Link::retxSeq_unlocked", p);
    assert(end != std::string::npos);
    std::string body = src.substr(p, end - p);
    auto stripComments = [](std::string s) {
        std::string out;
