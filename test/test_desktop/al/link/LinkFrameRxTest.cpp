// UtilFrameRx COBS decoder + frame-type routing
// (data/ACK/NAK).
...
public:
    ...
    {
        errors++;
        return dropAfterErrors >= 0 &&
            errors >= dropAfterErrors;
    }
};

static std::vector<uint8_t>
wireFrame(const std::vector<uint8_t> &payload,
          uint8_t cobsSeq = 0)
{
    std::vector<uint8_t> raw;
    raw.push_back(cobsSeq);
    raw.insert(raw.end(), payload.begin(),
               payload.end());
    raw.push_back(
...
{
    std::cout << "\n=== Test: Single Frame Delivered "
                 "with cobsSeq ==="
              << std::endl;
    MockListener lis;
    UtilFrameRx rx(lis);
    std::vector<uint8_t> p = { 0x10, 0x00, 0x20,
                               0xFF };
    auto w = wireFrame(p, 42);
    assert(rx.feed(w.data(), (int)w.size()) ==
           (int)w.size());
...
{
    std::cout << "\n=== Test: 0x00 0x00 Keepalive "
                 "Atom Skipped ==="
              << std::endl;
    MockListener lis;
    UtilFrameRx rx(lis);
    uint8_t atom[] = { 0x00, 0x00, 0x00,
                       0x00, 0x00, 0x00 };
...
{
    std::cout << "\n=== Test: 0x00 0x00 After Partial "
                 "Frame ==="
              << std::endl;
    MockListener lis;
    UtilFrameRx rx(lis);
    uint8_t partial_then_atom[] = { 0x05, 0x11, 0x22,
                                    0x00, 0x00 };
    rx.feed(partial_then_atom,
            sizeof(partial_then_atom));
    assert(lis.payloads.empty() && lis.errors == 1);
    std::cout << "PASS" << std::endl;
}
...
void test_drop_stops_feed_early()
{
    ...
    std::vector<uint8_t> ev = { 0x00, 0x02, 0xAB, 0x00,
                                0x02, 0xCD, 0x00, 0xAA,
                                0x55, 0x22, 0x99 };
...
{
    std::cout << "\n=== Test: Zero-Byte Payload "
                 "Delivered with cobsSeq ==="
              << std::endl;
    MockListener lis;
    UtilFrameRx rx(lis);
    auto w = wireFrame({}, 5);
...
void test_max_cobsSeq()
{
    ...
    assert(lisNak.naks.size() == 1 &&
           lisNak.naks[0] == 0xAB);

    std::cout << "\n=== Test: cobsSeq=0xFF (ACK_TYPE) "
                 "Reserved ==="
              << std::endl;
    MockListener lisAck;
...
int main()
{
    std::cout << "=== Running UtilFrameRx Tests ==="
              << std::endl;
    test_single_frame();
