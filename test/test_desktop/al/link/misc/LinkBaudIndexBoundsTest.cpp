...
void test_post_construction_count_write_is_bounded() {
    ...
    // The OOB trigger: write a count above the array size. Without
    // a clamp, the link layer indexes cfg.allowedBauds[19]
    // (spdI = count-1) and returns garbage; the choke point must
    // clamp the count first.
