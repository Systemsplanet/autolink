...
static void test_sweep_drives_linkcontext() {
    ...
    assert(ctx.log.sendSweepFrame == 1);
    ...
    assert(ctx.log.setBaud == 115200);
    assert(ctx.log.sendSweepFrame == 3); // P1, P2, P3 each fire one
    assert(ctx.log.lastFrame == autolink::PING_CMD);
    std::cout << "PASS (3 phase entries → 3 setSpd + 3 sendSweepFrame)"
              << std::endl;
}
