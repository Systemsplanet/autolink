...
void test_readme_usage() {
    ...
    // (AUTOLINK_WIRE_SETTLE_MS, currently 50ms)
    // BEFORE writing the payload. The settle gate
    // drops the first wave of wire frames after
    // lock; if the master writes immediately, the
    // slave's settle drops the data and the master
    // (NullArqCache, no retx) never sees an ACK.
    // A 100ms pre-write settle wait guarantees the
    // gate is closed before the data is emitted.
    for (int i = 0; i < 5; i++) {
        txHal.pumpClock(20);
        rxHal.pumpClock(20);
        txNode.onTimer();
