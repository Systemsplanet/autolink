...
static void test_unconditional_err_source_grep() {
    ...
    // a local; earlier `if (drop > 0)` guard is
    // removed). With the FrameErrCause tagging, the
    // unconditional call is now `err(FrameErrCause::BadHeader);`
    // — assert the cause tag is wired to the
    // BadHeader variant, not CrcFail or OverLen
    // (those would be the bug-class shape of
    // mis-tagging the resync-scan err fire).
    assert(strstr(bodybuf, "err(FrameErrCause::BadHeader)") != NULL &&
           "Pin 2: recvMsg's beginMsg-failure branch must call "
           "err(FrameErrCause::BadHeader) unconditionally — "
           "not `err();` (the cause tag is now mandatory) and "
           "not a different FrameErrCause (which would silently "
           "mis-class the resync-scan fire)");
    std::cout << "  Pin 2 PASS (err(FrameErrCause::BadHeader) "
                 "unconditional, no `if (drop > 0)` guard)"
              << std::endl;
    free(buf);
}
