...
static void test_pool_exhaustion_drop_in_async_mode() {
    ...
    // onTimerOk_unlocked snapshots pool room +
    // pending into HealthState, and a non-Keep
    // decideHealth verdict resets the link and
    // reports break-needed to onTimer.
    ...
    const char *fn = strstr(buf, "bool Link::onTimerOk_unlocked()");
    assert(fn);
    ...
    const char *reset = strstr(health, "reset_unlocked(true)");
    assert(reset);
    const char *brk = strstr(reset, "return true");
    assert(brk);
