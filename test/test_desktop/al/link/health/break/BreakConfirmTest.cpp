...
//   Pin 3: two BREAKs in a row outside the coalesce window defer
//   the reset itself to the next onTimer() tick — same deadlock
//   class as BreakStormDefersToOnTimerTest's onBreakStorm() fix,
//   applied to onBreak()'s fast-confirm branch — but do NOT wait
//   out the BREAK_CONFIRM_MS window: onBreak() arms a near-
//   immediate (1 ms) timer so the deferred confirm still lands on
//   the next tick, not whenever the link's normal cadence next
//   happened to fire.
...
static void test_double_break_outside_coalesce_confirms_immediately() {
    ...
    sHal.deliver_break_to_self();
    assert(b.getState() == State::OK &&
           "Pin 3: still OK immediately after the second BREAK — the "
           "reset itself must run on the next onTimer() tick (timer-"
           "daemon context), not inline on the UART-event-task callback "
           "that fired onBreak()");
    // onBreak() armed a 1 ms deferred-confirm timer; pump just
    // that far and no more, to prove this doesn't wait out the
    // full BREAK_CONFIRM_MS window.
    sHal.pumpClock(1);
    assert(b.getState() == State::SWP &&
           "Pin 3: a second BREAK outside the coalesce window confirms "
           "on the very next tick — no BREAK_CONFIRM_MS window wait");
    assert(LinkTestAccessor(b).lastResetReasonForTest() ==
           ResetReason::HealthWatchdog);
    std::cout << "  Pin 3 PASS (second BREAK deferred to onTimer, "
                 "confirmed on the next tick)"
              << std::endl;
}
