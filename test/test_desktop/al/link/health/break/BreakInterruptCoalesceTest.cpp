...
static void test_coalesce_gate_source_grep() {
    std::cout << "\n=== Pin 2: onBreak() has the BREAK_COALESCE_MS "
              ...
              << std::endl;
    FILE *f = fopen(
        testRepoPath("src/al/link/timers/LinkTimerBreak.cpp").c_str(), "r");
    assert(f);
    fseek(f, 0, SEEK_END);
    long sz = ftell(f);
    fseek(f, 0, SEEK_SET);
    ...
    // The shape with the gate closes the field-log bug
    // class where two sub-ms-spaced BREAKs from one
    // glitch fast-confirmed the reset without coalescing.
    assert(strstr(bodybuf, "BREAK_COALESCE_MS") != NULL &&
           "Pin 2: onBreak() must reference BREAK_COALESCE_MS — the "
           "fix coalesces BREAKs within BREAK_COALESCE_MS into one "
           "event.");
