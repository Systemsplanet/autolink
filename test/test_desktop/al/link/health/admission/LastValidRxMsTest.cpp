...
static void test_lastValidRxMs_source_grep() {
    ...
    {
        FILE *f2 =
            fopen(testRepoPath("src/al/link/io/LinkRx.cpp").c_str(), "r");
        assert(f2);
        fseek(f2, 0, SEEK_END);
        long sz2 = ftell(f2);
        fseek(f2, 0, SEEK_SET);
...
static void test_ack_nak_stamp_lastValidRxMs() {
    ...
    // AL97-7 grew onAck's body (per-ACK trace lines moved behind
    // AUTOLINK_TRACE_WIRE, plus the wire-agg counter accumulation)
    // past the previous 4096-byte cap, silently truncating the
    // stamp this pin checks for out of the copied buffer — a
    // truncation bug in the harness, not a regression in onAck
    // itself. Sized generously above onAck's current ~11.8 KB body
    // with an explicit bounds assert so a future truncation fails
    // loudly here instead of producing a misleading "stamp missing"
    // failure below.
    static char onAckBuf[16384];
    assert(onAckLen < (int)sizeof(onAckBuf) &&
           "Link::onAck's body no longer fits onAckBuf — bump the "
           "buffer size, this assert exists so growth is caught "
           "here rather than silently truncating the copy below");
    ...
    // Same truncation-harness fix as onAckBuf above — onNak's body
    // is also well past the old 4096-byte cap.
    static char onNakBuf[16384];
    assert(onNakLen < (int)sizeof(onNakBuf) &&
           "Link::onNak's body no longer fits onNakBuf — bump the "
           "buffer size, this assert exists so growth is caught "
           "here rather than silently truncating the copy below");
...
static void test_lockOk_resets_lastValidRxMs() {
    ...
    assert(end);
    int len = (int)(end - fn);
    // Pre-existing truncation fragility, same class as the
    // onAck/onNak instances above (this file) — lockOk_unlocked's
    // body (~7 KB) already exceeded the old 4096-byte cap
    // independently of this session's changes. Fixed alongside
    // while touching this file rather than left as a landmine that
    // silently truncates the moment the checked stamp shifts a few
    // lines further into the function.
    static char bodybuf[16384];
    assert(len < (int)sizeof(bodybuf) &&
           "Link::lockOk_unlocked's body no longer fits bodybuf — "
           "bump the buffer size, this assert exists so growth is "
           "caught here rather than silently truncating the copy "
           "below");
