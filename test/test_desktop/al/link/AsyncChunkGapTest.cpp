// Regression pins for todo.md item 1 — UART overrun under
// multi-chunk ASYNC bursts. The FireBeetle bench logs at
// 512000 baud showed `errs` climbing during the RANDOM-fill
// window even before the drops, and Pong's rx-rate collapsing
// while Ping flooded retransmits. HW UART RX-FIFO overrun is
// the proposed root cause: chunks emitted back-to-back at
// 512000 baud outrun Pong's `uart_event_task`, the in-order
// receiver drops the out-of-order tail, NAKs the base, and
// the sender replays the whole window (amplifying the
// overrun). The library fix inserts a small inter-chunk gap
// (cfg.asyncChunkGapMs) via IHal::delayUs — a microsecond-
// resolution primitive so it stays sub-tick on FreeRTOS.
//
// Pins:
