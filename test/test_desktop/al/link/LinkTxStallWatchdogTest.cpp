...
// forever. Three pins:
//
//   1. The wedge recovers: full ARQ pool +
//      empty pending table, sendMsg rejected
//      repeatedly past idleTimeoutMs → the
...
// doesn't tear OK down to SWP. Real ArqCache so
// testFillPool() can drive hasRoom() false with
// zero pending bits — the exact wedge shape.
struct Driver {
    MockHal hal;
    ArqCache cache;
    AutoLinkConfig cfg;
...
void test_wedge_drops_and_resweeps() {
    std::cout << "\n=== Pin 1: sustained reject streak -> drop + resweep ==="
              << std::endl;
    Driver d(IDLE_MS);
    d.cache.testFillPool();
    assert(!d.cache.hasRoom());
    assert(d.link.arqPendingCount() == 0);
...
void test_success_clears_streak() {
    ...
    d.cache.testFillPool();
    assert(rejectedSend(d.link));
    d.cache.testEmptyPool();
    assert(!rejectedSend(d.link) && "send must succeed with room");
    // Retire the successful send's pending slot
    // (as a peer ACK would) so the asymmetric
    // stuck-send check stays quiet — this pin
    // isolates the streak-clear path.
