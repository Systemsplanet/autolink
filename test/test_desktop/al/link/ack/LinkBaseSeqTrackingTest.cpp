...
static void test_async_22chunk_baseseq_message_sum() {
    ...
    uint8_t baseSeq = 0;
    Link::SendResult res{};
    bool ok = ping.sendMsg(tx.data(), msgLen, &baseSeq, &res);
    uint8_t baseLap = res.baseLap;
    assert(ok);
    ...
    // = 1 + 21 = 22.)
    // E8: query the bytesRecvdForMessage
    // with the lap the sender reported, so
    // the seq+lap pair is unambiguous.
    assert(t.bytesRecvdForMessage(baseSeq, baseLap) == 5126);
...
// A fixed-size fread buffer silently truncates as the source file
// grows past it — exactly the failure class documented in this
// project's own history (four pins truncated at a 16 KB buffer
// once). Read the whole file dynamically instead so this pin keeps
// testing real content regardless of file size.
...
static void test_bytes_recvd_for_message_body() {
    std::cout << "\n=== Pin 4: bytesRecvdForMessage delegates to LinkArq, "
                 "which owns the walk ==="
              << std::endl;
    std::string linkH = readWholeFile(testRepoPath("src/al/link/Link.h"));
    assert(!linkH.empty());
    const char *buf = linkH.c_str();
    ...
    std::string arqH = readWholeFile(testRepoPath("src/al/link/arq/LinkArq.h"));
    assert(!arqH.empty());
    const char *abuf = arqH.c_str();
...
static void test_onSent_stamps_baseseq_with_no_base_default() {
    std::cout << "\n=== Pin 5: LinkArq::onSent stamps baseSeq_[] with the "
              ...
              << std::endl;
    std::string arqCpp =
        readWholeFile(testRepoPath("src/al/link/arq/LinkArq.cpp"));
    assert(!arqCpp.empty());
    const char *buf = arqCpp.c_str();
