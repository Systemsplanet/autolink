...
void test_droplink_no_op_when_already_sweeping() {
    // Regression pin: a late dropLink on a link that
    // has already left OK (mid-sweep, mid-recovery)
    // used to reset_unlocked + sendBreak twice. The
    // second reset kicked a fresh sweep on top of
    // the in-progress one, producing the back-to-back
    // P1→P2→P3 cycles visible in the field logs at
    // ~28.7s and ~35.7s. The fix: dropLink() is a
    // no-op unless state == OK.
    std::cout << "\n=== Test: Link::dropLink no-op when state != OK ==="
              << std::endl;
    std::string linkApi = slurp("../../src/al/link/LinkApi.cpp");
    assert(!linkApi.empty());

    auto fnPos = linkApi.find("void Link::dropLink()");
    assert(fnPos != std::string::npos);
    auto bodyStart = linkApi.find('{', fnPos);
    assert(bodyStart != std::string::npos);
    int depth = 0;
    size_t bodyEnd = linkApi.size();
    for (size_t i = bodyStart; i < linkApi.size(); i++) {
        if (linkApi[i] == '{')
            depth++;
        else if (linkApi[i] == '}') {
            depth--;
    ...
    std::string body = linkApi.substr(bodyStart, bodyEnd - bodyStart);

    // The guard: `if (state != State::OK) { hw.unlock(); return; }`
    // must appear before the reset_unlocked call.
    assert(body.find("state != State::OK") != std::string::npos &&
           "dropLink() must guard on state != State::OK");
    auto guardPos = body.find("state != State::OK");
    auto resetPos = body.find("reset_unlocked(true)");
    assert(guardPos < resetPos &&
           "the state guard must precede the reset_unlocked call");
    std::cout << "  PASS (state guard precedes reset_unlocked in dropLink)"
              << std::endl;
}


// Pin: interface segregation of the old fat context (retired).
// The reorder buffer this pin guarded (LinkReorder /
// IReorderSink) was deleted in the GBN rewrite — the receiver
// no longer holds out-of-order frames, so there's no delivery
// sink interface left to keep segregated from ISweepCtx.
...
int main() {
    ...
    test_droplink_no_op_when_already_sweeping();
