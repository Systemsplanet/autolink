...
//      would exceed COBS_SEQ_SPACE. Runtime: drive the
//      arq_ pending table to near-full, attempt a large
//      send, assert it returns false and emits the
//      seq-space log line. Toggle off (revert LinkApi.cpp's
//      guard) → red.
...
//      the ctor log; pre-fix the send path would silently
//      fail on every send. Runtime + log-content pin.
//   5. Static_asserts in AutoLinkConfig.h tie arqChunkBudget,
//      maxMsg (default 1024), and MAX_CHUNK together.
//      Source-grep: chunksForMsgLen(1024) appears in
//      AutoLinkConfig.h with the bound assertion.
//
...
void test_seq_space_guard_rejects_when_inflight_full() {
    std::cout
        << "\n=== Pin 2: sendMsg rejects when inflight+chunks > seq space ==="
        << std::endl;
    Driver d;
    LinkTestAccessor acc(d.link);

    // Drive the ARQ pending table to near-full so a
    // large message would alias. COBS_SEQ_SPACE = 254;
    // a 500-byte message is 3 chunks (1 hdr + 2 data),
    // so we need inflight >= 252 to trip the guard
    // (252 + 3 = 255 > 254).
    constexpr int kInflight = 252;
    ...
    assert(!ok && "sendMsg must reject when seq space is exhausted");
    ...
    std::cout << "  PASS (inflight=" << kInflight << " + chunks=3 > "
              << COBS_SEQ_SPACE << "; sendMsg rejected)" << std::endl;
}
...
void test_seq_space_guard_accepts_when_headroom() {
    ...
    // Same payload (3 chunks) but inflight=200, so
    // 200+3=203 ≤ 254: the guard lets it through.
    // sendMsg() runs the build path which would
    // try to actually transmit; MockHal::tx just
    // appends to a buffer with no peer, so the
    // chunks go out and the ARQ cache stays empty
    // (we used NullArqCache so insert is a no-op).
    // The guard returning true is the signal we
    // care about.
    ...
    uint8_t seq = 0xFF;
    bool ok = d.link.sendMsg(buf, sizeof buf, &seq);
    assert(ok && "sendMsg must accept when inflight + chunks <= seq space");
    std::cout << "  PASS (inflight=" << kInflight
              << " + chunks=3 <= " << COBS_SEQ_SPACE << "; sendMsg accepted)"
              << std::endl;
}

void test_chunksForMsgLen_default_maxMsg_fits_seq_space() {
    std::cout
        << "\n=== Pin 4: chunksForMsgLen(default maxMsg=1024) <= seq space ==="
        << std::endl;
    int n = chunksForMsgLen(1024);
    assert(n == 1 + (1024 + 250 - 1) / 250);
...
void test_static_asserts_present_in_config_header() {
    ...
    bool hasDefaultMaxMsgSanity =
        src.find("chunksForMsgLen(1024)") != std::string::npos &&
        src.find("default maxMsg=1024 must fit the seq space alone") !=
            std::string::npos;
    bool hasBudgetVsMsgInvariant =
        src.find("ARQ_CHUNK_BUDGET >= chunksForMsgLen") != std::string::npos;
