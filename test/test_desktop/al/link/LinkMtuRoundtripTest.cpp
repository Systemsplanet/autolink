...
void test_async_32kb_with_real_arq_cache() {
    ...
    // chunk 65. The pre-fix shape logged "pool
    // exhausted" for the remaining 68 chunks —
    // those chunks landed on the wire WITHOUT
    // a retx slot reserved, so any early loss
    // on those chunks was unrecoverable (the
    // link's NAK handler does a cache lookup
    // and finds nothing → LinkArq marks Drop →
    // link reset, discCount climbs). The fix:
    // each chunk's emit checks hasRoom() under
    // the lock; on a full pool, sendMsg returns
    // false so the caller's backpressure
    // cooldown absorbs the partial. No chunk
    // goes on the wire without a retx slot.
