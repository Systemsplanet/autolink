...
// needed by the test).
//
// Why this suite: the 6.0.0 series had a 32 KB end-to-end
// that proved the wire format survives at MTU; that test
// was dropped in 6.0.1 and the standard loopback only
...
void test_async_32kb_with_real_arq_cache() {
    std::cout << "\n=== Pin 4: ASYNC 32 KB with production ArqCache "
                 "round-trips (pool churn at the boundary) ==="
              << std::endl;
    // Production ArqCache (not the NullArqCache
    // stub). Pool size = 64; 32 KB message
    // needs 133 chunks; the pool fills at
    // chunk 65 and the remaining 68 chunks
    // log "pool exhausted" — those chunks
    // are NOT in the ARQ cache, so if any
    // are dropped on the wire the retransmit
    // path can't recover (peekForRetx
    // returns false → LinkArq marks Drop
    // → link reset). At 0% drop the chunks
    // all land on the wire regardless of
    // cache state, so the round-trip still
    // succeeds. This pins that the
    // multi-chunk send path doesn't depend
    // on the cache containing every chunk;
    // it only needs the cache to be big
    // enough to hold the in-flight chunk
    // burst (== the steady-state window
    ...
    int got = pong.recvMsg(rx.data(), (int)rx.size());
    if (got != msgLen) {
        std::cerr << " ASYNC 32 KB (real ARQ cache) got " << got
                  << " bytes, expected " << msgLen << std::endl;
        assert(false);
    }
    int bad = -1;
    for (int i = 0; i < msgLen; i++) {
    ...
    if (bad >= 0) {
        std::cerr << " ASYNC 32 KB (real ARQ cache) byte mismatch at i=" << bad
                  << std::endl;
        assert(false);
    }
    std::cout << "  PASS (133 chunks, real ARQ pool, " << msgLen
              << " B byte-for-byte; pool churn at chunks 64..132 logs "
                 "but doesn't fail the round-trip)"
              << std::endl;
}
...
int main() {
    ...
    // Pin 4 fires ~70 expected "pool exhausted" warnings
    // when the 133-chunk MTU send overflows the 64-slot
    // ARQ pool. The round-trip still succeeds (the chunks
    // land on the wire regardless of cache state). Drop
    // the log level so the test output reads clean.
