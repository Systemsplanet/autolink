...
int main() {
    ...
    // GBN sweep-retx: split out of LinkTimersOk.cpp for the
    // 15 KB cap. The keep-rearm info line is the state
    // transition; the honest-drop/storm-stuck lines are
    // warnings (not counted here, same as LinkTimersOk).
    test_subsystem_logging_pin(
        { "LinkTimersGbn", "/src/al/link/timers/gbn/LinkTimersGbn.cpp", 1, 0 });
    ...
    // path".
    {
        ...
        // Floor is 2 (SWEEP / NAK): COBS and ACK are
        // verbose (compiled out by default, see
        // WireTraceOffByDefaultTest); the CTRL sender
        // (ISweepCtx::sendFrame) was dead code with no
        // production caller and is gone (CompileCheckTest
        // #16).
        assert(nDebug >= 2 &&
               "LinkTx must log at least two debug-level wire-send "
               "shapes (SWEEP / NAK) for the deep-trace operator; "
               "info is forbidden because the wire-send rate is the "
               "ASYNC pipeline rate");
        ...
        std::cout << "  PASS (debug=" << nDebug
                  << " >= 2, no info in hot send functions)" << std::endl;
    }
