// Boundary invariants from the god-class split. One pin per
// invariant; each reads the source and asserts the required shape
// is intact. Toggle off (revert the invariant) and the matching
// pin flips red.
//
...
void test_ihal_no_link_pointer_and_dispatches_via_ilinkevents() {
    ...
    assert(espHal.find("events()->onRx") != std::string::npos ||
           espHalUartEvent.find("events()->onRx") != std::string::npos);
    assert(espHal.find("events()->onBreak") != std::string::npos ||
           espHalUartEvent.find("events()->onBreak") != std::string::npos);
    assert(espHal.find("events()->onTimer") != std::string::npos ||
           espHalUartEvent.find("events()->onTimer") != std::string::npos);
...
void test_chunk_pool_static_assert_uses_named_constants() {
    std::cout
        ...
        << std::endl;
    // The named-constant form lives in every TU that includes
    // LinkWire.h and ArqCache.h. Check the TUs that assert on the
    // chunk/pool coupling.
    const char *tus[] = {
    ...
    for (const char *rel : tus) {
        ...
        // Must reference both MAX_CHUNK + MSG_HDR and
        // ArqCache::POOL_BUF_MAX by name, not a magic literal.
...
// Pin 7: this-release resweep heap log. LinkCore.cpp
// logs `resweep: disc=%lu freeHeap=%u` from
// reset_unlocked's count-and-resweep branch (when
// discCount bumps) on device builds. The log is the
// triage hook for the disc-counter-climb / CRC-
// desync symptom: a stable freeHeap across cycles
// confirms Fixes 1-3 (state-machine race) are
// sufficient; a monotonic freeHeap drop points at
// POOL_SIZE starvation during uart_driver_install
// (a separate fix). Wrapped in #ifdef ARDUINO so
// the host build's reset_unlocked stays unchanged.
void test_resweep_logs_free_heap_on_device() {
    std::cout << "\n=== Pin 7: resweep logs freeHeap on device builds ==="
              << std::endl;
    std::string linkCore =
        readFile(projectRoot() + "/src/al/link/LinkCore.cpp");
    assert(!linkCore.empty());

    // Locate reset_unlocked body. Match the signature
    // regardless of any second `preservePreferredBaud` flag
    // the BREAK-triggered resweep path added — the
    // disambiguation test only cares about the body shape,
    // not the parameter list.
    auto fnPos = linkCore.find("void Link::reset_unlocked(");
    assert(fnPos != std::string::npos);
    auto bodyStart = linkCore.find('{', fnPos);
    assert(bodyStart != std::string::npos);
    int depth = 0;
    size_t bodyEnd = linkCore.size();
    for (size_t i = bodyStart; i < linkCore.size(); i++) {
        if (linkCore[i] == '{')
            depth++;
        else if (linkCore[i] == '}') {
            depth--;
    ...
    std::string body = linkCore.substr(bodyStart, bodyEnd - bodyStart);

    // The count-and-resweep branch (count && state == State::OK)
    // must emit a Log call that mentions both "resweep" and
    // "freeHeap" inside an #ifdef ARDUINO guard.
    auto resweepPos = body.find("resweep");
    auto heapPos = body.find("freeHeap");
    auto arduinoGuardPos = body.find("#ifdef ARDUINO");
    auto espEndPos = body.find("#endif");
    assert(resweepPos != std::string::npos &&
           "reset_unlocked must log 'resweep' on disc-count bump");
    assert(heapPos != std::string::npos &&
           "reset_unlocked must log freeHeap for heap-pressure triage");
    assert(arduinoGuardPos != std::string::npos &&
           espEndPos != std::string::npos &&
           "the resweep/freeHeap log must be wrapped in #ifdef ARDUINO");
    // The heap log must be inside the ARDUINO guard
    // so the host build doesn't pull in esp_system.h.
    assert(resweepPos > arduinoGuardPos && resweepPos < espEndPos &&
           "resweep log must sit inside the ARDUINO guard");
    assert(heapPos > arduinoGuardPos && heapPos < espEndPos &&
           "freeHeap log must sit inside the ARDUINO guard");

    // The include guard must add esp_system.h on
    // device builds. The include is indented under
    // #ifdef ARDUINO so the literal search uses the
    // preprocessor-form include (#    include) too.
    assert(linkCore.find("include <esp_system.h>") != std::string::npos &&
           "LinkCore.cpp must include <esp_system.h> for "
           "esp_get_free_heap_size() on device builds");
    std::cout << "  PASS (resweep/freeHeap log inside #ifdef ARDUINO; "
                 "esp_system.h include present)"
              << std::endl;
}
...
int main() {
    ...
    test_resweep_logs_free_heap_on_device();
