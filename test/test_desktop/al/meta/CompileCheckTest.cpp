...
const std::vector<std::tuple<std::string, std::string, std::string>>
    ARDUINO_GUARDED_FILES = {
        ...
        { "src/al/hal/EspHal.cpp", "c++14", "" },
        // AutoLinkWeb is split into four ARDUINO-gated TUs, each
        // must parse cleanly under the host stubs.
        { "src/al/web/handlers/AutoLinkWebLifecycle.cpp", "c++14", "" },
        { "src/al/web/handlers/AutoLinkWebHttpd.cpp", "c++14", "" },
        { "src/al/web/handlers/AutoLinkWebHandlersData.cpp", "c++14", "" },
        { "src/al/web/handlers/AutoLinkWebHandlersOta.cpp", "c++14", "" },
...
const std::vector<std::string> EXPECTED_STUB_SYMBOLS = {
    ...
    "uart_set_baudrate", "uart_driver_delete", "gpio_set_pull_mode",
    "esp_timer_start_periodic", "httpd_register_uri_handler", "httpd_start",
    "httpd_stop", "esp_err_to_name", "xTaskCreatePinnedToCore",
    "xStreamBufferCreate", "xStreamBufferSpacesAvailable",
    "xStreamBufferBytesAvailable", "esp_random", "esp_restart",
    ...
    "ets_delay_us",
    // TX-ring free-size query (EspHal::txAvail / gbnResendWindow
    // bounded burst).
...
void test_linkarq_linksweep_present() {
    std::cout << "\n=== Test: LinkArq / LinkSweep present ===" << std::endl;
    std::string root = projectRoot();
    int missing = 0;
    ...
    if (!contains("src/al/link/arq/ArqCache.h", "class ArqCache"))
        missing++;
    assert(missing == 0);
    std::cout << "  PASS (all four classes present in their headers)\n";
}
...
void test_dead_code_boundary() {
    ...
    auto contains = [&](const std::string &rel, const std::string &needle) {
        ...
        if (!f.is_open()) {
            std::cout << "  FAIL: cannot open " << rel << "\n";
            assert(false && "dead-code boundary pin: file path is stale");
        }
        std::stringstream ss;
        ss << f.rdbuf();
    ...
    // The split refactor moved Link.cpp's method bodies across
    // LinkCore / LinkTx / LinkRx / LinkSweepGlue / LinkTimers (now
    // split further into LinkTimersOk + LinkTimersSwp) / LinkApi —
    // so dead-code searches must cover all seven files at their
    // current paths.
    static const std::vector<std::string> LINK_TUS = {
    ...
    for (const std::string &tu : LINK_TUS) {
        chk(tu, "retxNeeded_");
    }

    // #3: no out-of-line `void Link::sendFrame(`
    // definition (non-underscore-suffixed) in any
    // link TU. Historically a lock-wrapper/helper
    // split; the whole symbol is gone now (#16), so
    // this and #16 together cover both spellings.
    for (const std::string &tu : LINK_TUS) {
        chk(tu, "void Link::sendFrame(");
    }
    ...
    // #9: attemptPreferredRelock_unlocked.
    // Removed in the dead-code cleanup: never called
    // (the BREAK path folds the preferredBaud_
    // re-lock into reset_unlocked(..., true) and
    // resweepPrefPending_).
    chk("src/al/link/Link.h", "attemptPreferredRelock");
    for (const std::string &tu : LINK_TUS) {
        chk(tu, "attemptPreferredRelock");
    }

    // #10: reorderAdvanceRxSeq. Removed in
    // the dead-code cleanup: leftover from the
    // GBN reorder-buffer removal; the strict
    // in-order-accept path never advances rxSeq
    // outside the receive path.
    chk("src/al/link/Link.h", "reorderAdvanceRxSeq");
    for (const std::string &tu : LINK_TUS) {
        chk(tu, "reorderAdvanceRxSeq");
    }

    // #11: ArqCache::testEmptyPool /
    // slotPeek. Removed in the dead-code
    // cleanup: never called by any test
    // (testFillPool / testFillSlots cover the
    // pool/stickiness tests).
    chk("src/al/link/arq/ArqCache.h", "testEmptyPool");
    chk("src/al/link/arq/ArqCache.cpp", "testEmptyPool");
    chk("src/al/link/arq/ArqCache.h", "slotPeek");
    chk("src/al/link/arq/ArqCache.cpp", "slotPeek");

    // #12: LinkArq::baseSeqFor. Removed
    // in the dead-code cleanup: no production
    // code reads baseSeq_ through a public
    // LinkArq accessor — Link's
    // bytesRecvdForMessage uses
    // LinkArq::bytesForMessage internally,
    // which walks the ring without exposing
    // baseSeqFor.
    chk("src/al/link/arq/LinkArq.h", "baseSeqFor");
    chk("src/al/link/arq/LinkArq.cpp", "baseSeqFor");

    // #13: LCK state machine. Removed in
    // the dead-state cleanup: State::LCK
    // was unreachable from production
    // code (no transition into it), so
    // onTimerLck_unlocked + handleLck_unlocked
    // + decideLckTick + LckAction were
    // dead surface.
    chk("src/al/link/Link.h", "State::LCK");
    chk("src/al/link/Link.h", "onTimerLck_unlocked");
    chk("src/al/link/Link.h", "handleLck_unlocked");
    chk("src/al/link/timers/LinkTimersOk.cpp", "onTimerLck_unlocked");
    chk("src/al/link/timers/LinkTimersSwp.cpp", "onTimerLck_unlocked");
    chk("src/al/link/timers/LinkSweepGlue.cpp", "handleLck_unlocked");
    chk("src/al/link/sweep/LinkDecision.h", "LckAction");
    chk("src/al/link/sweep/LinkDecision.h", "decideLckTick");
    // LinkDecisionTest no longer has the LCK
    // suite (test_decideLckTick_*); absence is
    // implicitly asserted by the unit test
    // count (76 -> 77 reflects the new
    // SweepTimeoutArmsTest but LinkDecisionTest's
    // own row shrinks by 2 pins). The macro
    // test here covers the public surface.

    // #14: SwpAction + decideSwpTick. Same
    // dead-state story — SwpAction::EnterLck
    // was only consumer in the sweep
    // state machine; routed through deleted
    // onTimerSwp_unlocked path that's now gone.
    // Use "enum class SwpAction" as the
    // needle (matches the exact enum-class
    // declaration; substring "SwpAction"
    // alone would also match the live
    // SwpPhaseAction class).
    chk("src/al/link/sweep/LinkDecision.h", "enum class SwpAction");
    chk("src/al/link/sweep/LinkDecision.h", "decideSwpTick");

    // #15: lckRetries field. No longer
    // tracked after the LCK state machine
    // was deleted; the master/slave reset
    // path is the only thing that ever
    // zeroed it.
    chk("src/al/link/Link.h", "lckRetries_");
    chk("src/al/link/Link.h", "lckRetries");
    chk("src/al/link/LinkCore.cpp", "lckRetries");

    // #16: ISweepCtx::sendFrame / Link::sendFrame_unlocked.
    // Removed in the dead-code cleanup: the only sender
    // of the 5-byte control-frame shape reachable from
    // production code was sendSweepFrame_unlocked
    // (sweepEpoch_ in byte[2]); sendFrame_unlocked's
    // txSeq-in-byte[2] shape had zero callers through
    // the ISweepCtx interface. Keeping an unreachable
    // sender with a different byte[2] meaning than the
    // one every receiver path assumes was a latent
    // collision waiting for a future caller.
    chk("src/al/link/ISweepCtx.h", "sendFrame(uint8_t payload) = 0");
    chk("src/al/link/Link.h", "sendFrame_unlocked");
    for (const std::string &tu : LINK_TUS) {
        chk(tu, "sendFrame_unlocked");
    }

    assert(hits == 0);
    std::cout << "  PASS (all 16 dead-code removals pinned)\n";
}
...
int main() {
    ...
    test_linkarq_linksweep_present();
