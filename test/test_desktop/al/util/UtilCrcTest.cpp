// CRC-8/CRC-16 known-answer tests, lookup-table
// sanity.
...
{
    std::cout
        << "\n=== Test: CRC Known-Answer Vectors ==="
        << std::endl;
    const uint8_t check[] = { '1', '2', '3', '4', '5',
                              '6', '7', '8', '9' };
...
{
    std::cout << "\n=== Test: LUT Agrees With Bitwise "
                 "Reference ==="
              << std::endl;
    for (int v = 0; v < 256; v++) {
        uint8_t b = (uint8_t)v;
        assert(UtilCrc::crc8(&b, 1) ==
               ref_crc8(&b, 1));
        assert(UtilCrc::crc16(&b, 1) ==
               ref_crc16(&b, 1));
    }
    uint8_t buf[4096];
    ...
    assert(UtilCrc::crc8(buf, sizeof(buf)) ==
           ref_crc8(buf, sizeof(buf)));
    assert(UtilCrc::crc16(buf, sizeof(buf)) ==
           ref_crc16(buf, sizeof(buf)));
    std::cout << "PASS" << std::endl;
}
...
{
    std::cout
        << "\n=== Test: Single-Bit Errors Detected ==="
        << std::endl;
    uint8_t buf[64];
    for (int i = 0; i < 64; i++)
        buf[i] = (uint8_t)(i * 7 + 3);
...
int main()
{
    std::cout << "=== Running UtilCrc Tests ==="
              << std::endl;
    test_known_answers();
    test_edges();
    test_lut_vs_reference();
    test_error_detection();
    std::cout << "\n=== UtilCrc Tests Completed "
                 "Successfully ==="
              << std::endl;
    return 0;
}
