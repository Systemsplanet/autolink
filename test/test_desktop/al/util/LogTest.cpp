...
void test_long_message_truncated_at_buffer() {
    ...
    // AL-01 sized the buffer from LogRingBuffer::Entry::msg (400
    // bytes) instead of a hardcoded 384, so the truncated content
    // is now up to 399 chars, not <= 384. AL-01b additionally
    // routes the one-shot truncation warning through the sink
    // (previously fprintf(stderr), a no-op on-device per AGENTS
    // rule 10) — so a truncating call now produces TWO sink
    // calls: the warning line first, then the truncated content
    // itself, in emit()'s own line order.
    //
    // AL90-6 shrunk the buffer to 160 bytes
    // (the field-capture's longest real line
    // is 96 chars; 160 gives 64 chars of
    // headroom). The truncation invariant
    // is now between 144 and 160 chars.
    assert(g_captured.size() == 2);
    assert(g_captured[0].msg.find("line truncated") != std::string::npos);
    assert(g_captured[1].msg.size() <= 160);
    assert(g_captured[1].msg.size() > 144);
    L.clearSink();
    std::cout << "PASS" << std::endl;
}
