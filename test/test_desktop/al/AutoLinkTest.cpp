...
void test_config_defaults_centralized() {
    ...
    assert(cfg.maxMsg == AUTOLINK_DEFAULT_MAX_MSG);
...
void test_txDelayMs_default_and_setter() {
    ...
    AutoLinkConfig cfg;
    // this release: default txDelayMs lowered 50 -> 0 ms. The
    // 50 ms default (set in 6.0.1) gave the /stats + /logs
    // poll enough headroom when the dashboard was the
    // bottleneck, but a fresh sketch that boots without
    // WiFi (and therefore without the dashboard's poll
    // pressure) ships at ~half the wire's natural throughput
    // for no operator-visible reason. 0 ms restores full
    // line-rate by default; the dashboard dropdown still
    // defaults to 0 and operators who want a throttled
    // bench run can pick a value via the widget or set
    // cfg.txDelayMs directly.
