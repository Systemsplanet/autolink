...
void test_txDelayMs_default_and_setter() {
    ...
    AutoLinkConfig cfg;
    // this release: default txDelayMs lowered 100 -> 50 ms. The
    // previous 100 ms default was too aggressive a throttle
    // for the higher baud table — operators reported the
    // dashboard dropping below 10 msg/s, which masks
    // transport errors under too much air-time. 50 ms hits
    // a sweet spot for the 5-baud sweep and matches the HTML
    // GUI's default delay-ms widget selection.
    assert(cfg.txDelayMs == 50);
    AutoLink link(0, 16, 17, true, cfg);
    AutoLinkTestAccessor t(link);
    assert(link.txDelayMs() == 50);
