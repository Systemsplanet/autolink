...
const std::vector<CallSite> REQUIRED_SITES = {
    ...
    // AutoLinkWeb's httpd / esp_timer call sites live
    // in AutoLinkWeb.cpp (the lifecycle TU) after the
    // recent god-class split. The handler TU
    // (AutoLinkWebHandlers.cpp) has no ESP-IDF calls
    // that return esp_err_t, so it's not in this list.
