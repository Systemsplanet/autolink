...
void test_ping_echo_seq_bytes_pending_is_info() {
    ...
    std::cout << "  PASS (3-arg echo line is at info level)" << std::endl;
}
...
void test_pong_echo_seq_bytes_is_info() {
    std::cout << "\n=== Pin 3: Pong per-echo 'echo %u %d' is info ==="
              << std::endl;
    std::string root = projectRoot();
    std::string src = readFile(root + "/src/al/pingpong/Pong.h");
    assert(!src.empty());

    auto pos = src.find("\"echo %u %d\"");
    assert(pos != std::string::npos);
    auto call = src.rfind("base_.log_.", pos);
    assert(call != std::string::npos);
    std::string callLine = src.substr(call, pos - call);
    assert(callLine.find("info") != std::string::npos &&
           "Pong's per-echo 'echo <seq> <bytes>' log line must be at info "
           "level, not verbose — same field-log requirement as Ping");
    assert(callLine.find("verbose") == std::string::npos);
    assert(callLine.find("debug") == std::string::npos);
    std::cout << "  PASS (Pong echo line is at info level)" << std::endl;
}
