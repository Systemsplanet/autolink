...
// Pin 7: this-release send-not-OK split. The
// send-loop's sendMsg-failure branch must
// distinguish "link not OK" (transient, the
// link is self-recovering via the sweep) from
// real backpressure (ARQ cache full / seq-space
// exhausted). Counting the former toward
// MAX_SEND_FAIL makes Ping dropLink a link that
// is already resweeping, doubling the resweep
// cycle and starving the wire. The fix: when
// sendMsg returns false AND link is not OK,
// break out of the send loop without bumping
// consecSendFail_. Real backpressure still
// counts.
void test_ping_send_fail_splits_link_not_ok_from_backpressure() {
    std::cout << "\n=== Pin 7: send-failure split (link not OK vs "
                 "backpressure) ==="
              << std::endl;
    std::string root = projectRoot();
    std::string src = readFile(root + "/src/al/pingpong/Ping.h");
    ...
    // The split shape: an `if (!base_.comm_.ready()) { break; }`
    // guard must precede the consecSendFail_++ bump. The
    // ready() check gates the early-return; the bump
    // counts only real backpressure.
    auto readyPos = branch.find("!base_.comm_.ready()");
    auto bumpPos = branch.find("consecSendFail_++");
    assert(readyPos != std::string::npos &&
           "send-failure branch must check !base_.comm_.ready() to "
           "distinguish self-recovering from backpressure");
    assert(bumpPos != std::string::npos &&
           "send-failure branch must still bump consecSendFail_ on "
           "real backpressure");
    assert(readyPos < bumpPos &&
           "the ready() check must come BEFORE the consecSendFail_ "
           "bump so the link-not-OK case skips the counter");

    std::cout << "  PASS (link-not-OK early-return precedes "
                 "consecSendFail_++ bump)"
              << std::endl;
}
...
int main() {
    ...
    test_ping_send_fail_splits_link_not_ok_from_backpressure();
