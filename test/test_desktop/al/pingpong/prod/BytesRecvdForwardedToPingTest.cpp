// Source-level regression pin for the bytes-recvd facet wire-up.
// Link::bytesRecvdForMessage(baseSeq) must have a production
// consumer, not just accessor plumbing with nothing reading it: the
// dashboard JSON path reads `bytesRecvdForMessage(baseSeq)`
// directly, and the facade forwarder must stay in sync with it.
//
// Pins:
...
void test_autolink_facade_forwards_bytes_recvd_for_message() {
    std::cout
        << "\n=== AutoLink facade exposes bytesRecvdForMessage(baseSeq, baseLap) ==="
        << std::endl;
    ...
    // production single-arg form was
    // deleted (the un-lapped walk can
    // alias a re-stamped seq across a
    // 254-lap wrap). The forwarder
    // signature must match exactly:
    //     uint16_t bytesRecvdForMessage(uint8_t baseSeq, uint8_t baseLap) const
    // A forwarder that takes one arg,
    // or drops the const, trips this
    // pin.
    assert(
        src.find(
            "uint16_t bytesRecvdForMessage(uint8_t baseSeq, uint8_t baseLap) const") !=
    ...
    // The forwarder body must delegate to
    // the link's 2-arg bytesRecvdForMessage
    // (the production accessor that walks
    // the baseSeq_ + baseLap_ tables). A
    // body that calls the single-arg form,
    // or returns a constant, leaves the
    // accessor latent again.
    std::string fwdBody = extractFnBody(
        src,
        "uint16_t bytesRecvdForMessage(uint8_t baseSeq, uint8_t baseLap) const");
    assert(!fwdBody.empty());
    assert(fwdBody.find("link->bytesRecvdForMessage(baseSeq, baseLap)") !=
           std::string::npos);

    std::cout
        << "  PASS (facade forwarder delegates to link->bytesRecvdForMessage(baseSeq, baseLap))\n";
}
...
void test_ping_wire_log_removed() {
    ...
    // in dead-code branches. The log was
    ...
    // wire-ACK sum.
    assert(body.find("echo#=%llu seq=%u msgBytes=%u pending=%d") !=
           std::string::npos);
