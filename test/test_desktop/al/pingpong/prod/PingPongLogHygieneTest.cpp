...
void test_pong_send_failed_demoted_to_warning() {
    ...
    // Pong is ack-only: no send path, and no per-message echo log. Its
    // operator signal is the periodic acks_sent line.
    assert(src.find("acks_sent=%lu") != std::string::npos);
    std::cout << "  PASS (Pong is ack-only; no echo path; "
                 "acks_sent diagnostic)\n";
}
...
void test_link_wiring_spam_ratelimits() {
    ...
    // a fixed boundary (== 15), not
    // a `>` (which combined with the
    // reset-to-5 below re-fires
    // forever). Threshold bumped from
    // 11 to 15 ticks (see
    // WiringStaggeredBootThresholdTest
    // in LinkTimersSwp.cpp) to cover a
    // staggered-boot skew the field
    // capture showed firing 47 s
    // before the peer's begin().
    assert(wiringIf.find("emptySweeps == 15") != std::string::npos);
