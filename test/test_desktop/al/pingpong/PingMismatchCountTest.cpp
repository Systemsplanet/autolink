// Regression pin for todo item 1 (Ping-side echo
...
// Previously the field was declared, passed to logStats,
// and never touched — so it always read zero in the bench
// log. Also pins that the labeled-companion echo log line
// ("echo#=%llu seq=%u msgBytes=%u pending=%d") exists at
// both ack sites so bench operators can read the field
// semantics at a glance (sequence counter vs message
// length, which is random in RANDOM-fill mode).
...
// of Ping::loop. The field was declared and passed to
// logStats() but never incremented (todo item 1).
...
void test_labeled_echo_companion_at_both_sites() {
    ...
    // Cross-check with the legacy format — both must be at
    // 2 sites so the bench log gets one labeled line per
    // legacy echo.
    int echoSites = 0;
    searchFrom = 0;
    while (true) {
