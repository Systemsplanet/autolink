...
// chunks belonging to one queued message. Prior shape:
// `linkDelta != appDelta` fired every reset because a
// multi-chunk message produces N chunks for 1 app
// drop. New shape: directional only — warn if exactly
// one side moves and the other stays at zero.
//
...
int main() {
    ...
    {
        FILE *f = fopen(testRepoPath("src/al/pingpong/PingPongBase.h").c_str(), "r");
        assert(f);
        char buf[8192];
