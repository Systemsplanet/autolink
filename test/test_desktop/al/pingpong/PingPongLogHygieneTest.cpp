...
void test_pong_post_settle_drain_and_frame_errs_in_log() {
    ...
    // Pin 5a: the drain happens AFTER the
    // settling blink. Pre-fix shape ran the
    // drain BEFORE blinkWait(4); the baud
    // switch from the SWP→OK transition
    // pushed line garbage onto the rx
    // stream AFTER the drain (the drain
    // saw nothing — the bench log shows
    // "drained 0 stale bytes pre-blink"),
    // so the next recv() failed CRC and
    // the bench log saw 16 Pong-app rejects
    // per relock. Post-fix shape: blink
    // first, drain on the NEXT loop
    // iteration after SETTLE_MS, by which
    // time the line garbage has landed.
    ...
    assert(body.find("drained %d stale bytes post-settle") !=
           ...
           "the pre-fix pre-blink drain saw 'drained 0' on every relock "
           "because the line garbage hadn't arrived yet");

    // Pin 5b: the recv-rejected log line
    // includes the link-layer's frameErrs
    // counter so operators can correlate
    // the app-level reject (one per recv
    // returning -1) with the wire-level
    // error count (one per actual CRC
    // failure in the framer). The 16-vs-3
    // mismatch the bench log exposed
    // dissolves once both numbers appear
    ...
    assert(body.find("frameErrs=%lu") != std::string::npos &&
           ...
           "frameErrs counter; pre-fix shape only logged ackCount, "
           "forcing operators to correlate 16 app-level rejects with "
           "3 wire-level frameErrs by hand");
