...
void test_dashboard_default_delayMs_is_50() {
    std::cout << "\n=== Pin 3: HTML GUI delay-ms default is 50 ==="
              << std::endl;
    std::string src =
        readFile(projectRoot() + "/src/al/web/dashboard_html_part_b.html");
    assert(!src.empty());
    // The select must have an option with value="50" and
    // the `selected` attribute, and the previous default
    // (value="100" selected) must be gone.
    ...
    assert(slice.find("value=\"50\" selected") != std::string::npos &&
           "value=50 must be the selected option in the delay-ms dropdown");
    assert(slice.find("value=\"100\" selected") == std::string::npos &&
           "value=100 must NOT be the selected option");
    std::cout << "  PASS (HTML default delay-ms is 50)" << std::endl;
}

void test_config_default_txDelayMs_is_50() {
    std::cout << "\n=== Pin 4: AutoLinkConfig::txDelayMs default == 50 ==="
              << std::endl;
    ...
    assert(slice.find("50") != std::string::npos &&
           "txDelayMs default must be 50");
    assert(slice.find("100") == std::string::npos &&
           "txDelayMs default must NOT be 100");
    std::cout << "  PASS (firmware default txDelayMs is 50)" << std::endl;
}
...
int main() {
    ...
    test_dashboard_default_delayMs_is_50();
    test_config_default_txDelayMs_is_50();
