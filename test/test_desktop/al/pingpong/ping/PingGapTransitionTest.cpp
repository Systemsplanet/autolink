...
// broken entry guard is a regression
// signature.
struct Row {
    const char *name;
    uint8_t currentGap;
...
// Bridges this file's original lastAck-based rows to the pending
// predicate decideGapTransition now takes. The candidate seq is
// lastNak before a gap is latched and currentGap after, matching
// Ping::loop; "acked" means lastAck names that candidate.
static bool pendingFrom(uint8_t currentGap, uint8_t lastNak, uint8_t lastAck) {
    uint8_t cand = (currentGap == PING_GAP_NO_GAP) ? lastNak : currentGap;
    if (cand == PING_GAP_NO_GAP)
        return false;
    return lastAck != cand;
}

// Same bridge for the call sites that pass a real lastAck value.
static GapAction decideGapTransitionAckShim(uint8_t currentGap, uint8_t lastNak,
                                            uint8_t lastAck, uint8_t &nextGap) {
    return decideGapTransition(currentGap, lastNak,
                               pendingFrom(currentGap, lastNak, lastAck),
                               nextGap);
}
...
void test_transition_table() {
    ...
    for (const auto &r : rows) {
        uint8_t next = 0;
        GapAction a = decideGapTransition(
            r.currentGap, r.lastNak,
            pendingFrom(r.currentGap, r.lastNak, r.lastAck), next);
...
void test_runtime_send_pause_on_nak() {
    ...
    // here gap stays NO_GAP, so sends proceed.
    {
        ...
        uint8_t next = gap;
        GapAction a = decideGapTransition(
            gap, lastNak, pendingFrom(gap, lastNak, lastAck), next);
        (void)a;
        gap = next;
...
// send loop MUST proceed. The old code gated
...
void test_caller_sends_in_no_gap_steady_state() {
    ...
    std::ifstream ping(testRepoPath("src/al/pingpong/Ping.h").c_str());
    assert(ping.good());
    ...
    for (int i = 0; i < 5; ++i) {
        ...
        GapAction a = decideGapTransition(
            gap, PING_GAP_NO_GAP,
            pendingFrom(gap, PING_GAP_NO_GAP, PING_GAP_NO_GAP), next);
        gap = next;
...
void test_link_layer_nak_signal() {
    ...
    uint8_t next = 0;
    GapAction a = decideGapTransitionAckShim(PING_GAP_NO_GAP, acc.lastNakSeq(),
    ...
    acc.onNak(seq + 1);
    a = decideGapTransitionAckShim(seq, acc.lastNakSeq(), seq, next);
    ...
    // currentGap = seq+1.
    a = decideGapTransitionAckShim(seq + 1, acc.lastNakSeq(), seq + 1, next);
    assert(a == GapAction::Resume);
...
// on the GapAction enum. Earlier bug was
...
//     action enum).
//
// escape-pins (the user-reported wedge):
//   - gap state clears on link-lost
//     (`if (base_.wasReady_) { ... gapSeq_ = NO_GAP;
//     ... }`).
//   - gap state clears on kickoff
//     (`setPaused(false)` runs `gapSeq_ = NO_GAP`).
//   - acked-seq dedupe: a NAK for a seq that
//     `isAcked()` is suppressed, so a re-arm
//     from a stale wire NAK doesn't trap the
//     app in gap-stop.
//   - force-resume timeout: a gap-stop that
//     has been running for > GAP_STOP_FORCE_RESUME_MS
//     force-resumes and resumes sends. The
//     literal log line "gap-stop force-resume:
//     missing seq=%u stalled %lu ms — resuming
//     sends" must exist in Ping.h so a field
//     operator can find the cause from the log.
//   - unconditional stats while gap-stopped:
//     the gap-stop early-return path calls
//     logStats(...) with an AppStateLog that
//     reports `gapStopped=true` and the missing
//     seq, so a wedge self-identifies in the
//     next periodic stats line.
void test_unconditional_read_pin() {
    std::cout << "\n=== Test: gap-stop suppression gate branches on gapSeq_, "
                 "not action ==="
              << std::endl;
    std::ifstream pingGap(testRepoPath("src/al/pingpong/PingGap.h").c_str());
    assert(pingGap.good());
    ...
    // escape pins.
    // 1. gap state clears on link-lost.
    // 2. gap state clears on kickoff / setPaused(false).
    // 3. acked-seq dedupe suppresses re-arms of a
    //    gap for a seq that's already ACKed locally.
    // 4. force-resume timeout.
    // 5. unconditional stats while gap-stopped.

    // Pin 1: link-lost publishes the expected-next echo
    // seq via gapSeq_ so the field log carries the
    // concrete discontinuity instead of the 0xFF placeholder.
    {
        auto lr = pingSrc.find("if (base_.wasReady_)");
        assert(lr != std::string::npos);
        // Walk the wasReady_ branch up to the
        // matching closing brace. Simpler: assert
        // the substring "gapSeq_ = NO_GAP" appears
        // within the next 400 chars after the
        // wasReady_ marker (the link-lost branch is
        // short).
        std::string window = pingSrc.substr(lr, 600);
        assert(window.find("gapSeq_ = (uint8_t)(lastSeq_ + 1u)") !=
                   std::string::npos &&
               "link-lost branch must publish (lastSeq_ + 1) as "
               "the expected-next echo seq");
        std::cout
            << "  Pin 1: link-lost publishes (lastSeq_+1) as gapSeq_ \u2713"
            << std::endl;
    }

    // Pin 2: setPaused(false) clears gapSeq_.
    {
        auto sp = pingSrc.find("void setPaused(bool p)");
        assert(sp != std::string::npos);
        std::string window = pingSrc.substr(sp, 800);
        // The if (!p) branch must contain
        // `gapSeq_ = NO_GAP`.
        assert(window.find("if (!p)") != std::string::npos);
        std::string ifNotP = window.substr(window.find("if (!p)"), 600);
        assert(ifNotP.find("gapSeq_ = NO_GAP") != std::string::npos);
        std::cout
            << "  Pin 2: gapSeq_ = NO_GAP in setPaused(false) (kickoff) \u2713"
            << std::endl;
    }

    // Pin 3: acked-seq dedupe. The buggy-original gap
    // block re-armed on a NAK for a seq that was
    // already ACKed. The fix is a `isAcked(nextGap)`
    // guard that suppresses Enter/Update. Pin the
    // literal log line "gap-stop suppressed" so
    // a regression that removes the dedupe fails
    // loudly.
    {
        assert(pingSrc.find("gap-stop suppressed") != std::string::npos);
        assert(pingSrc.find("isAcked(nextGap)") != std::string::npos);
        std::cout << "  Pin 3: acked-seq dedupe (`gap-stop suppressed` log "
                     "line + isAcked(nextGap) guard) \u2713"
                  << std::endl;
    }

    // Pin 4: force-resume timeout. The
    // GAP_STOP_FORCE_RESUME_MS constant must exist,
    // and the loop body must contain the
    // "force-resume" log line that fires when
    // the timeout elapses.
    {
        assert(pingSrc.find("GAP_STOP_FORCE_RESUME_MS") != std::string::npos);
        assert(pingSrc.find("gap-stop force-resume") != std::string::npos);
        std::cout
            << "  Pin 4: GAP_STOP_FORCE_RESUME_MS + force-resume log line \u2713"
            << std::endl;
    }

    // Pin 5: unconditional stats while
    // gap-stopped. The gap-stop early-return
    // path must build an AppStateLog and call
    // logStats(...). The buggy-original shape returned
    // without ever calling logStats, so a wedge
    // produced no operator-visible signal.
    {
        // The gap-stop branch must contain a
        // `logStats(` call site. Walk the
        // gap-stop block: it's the
        // `if (gapSeq_ != NO_GAP)` block inside
        // the gap-decision scope. Find that
        // `if` and assert logStats( appears
        // before the matching `return;`.
        auto gapIf = pingSrc.find("if (gapSeq_ != NO_GAP)");
        assert(gapIf != std::string::npos);
        auto gapIfEnd = pingSrc.find("return;", gapIf);
        assert(gapIfEnd != std::string::npos);
        std::string block = pingSrc.substr(gapIf, gapIfEnd - gapIf);
        assert(block.find("logStats(") != std::string::npos);
        assert(block.find("app.gapStopped = true") != std::string::npos);
        std::cout
            << "  Pin 5: gap-stop block calls logStats with AppStateLog \u2713"
            << std::endl;
    }

    // Pin 6: tGapStopMs_ cleared on Resume, on
    // link-lost, on kickoff, and on clearQueue_.
    // The buggy-original shape had no tGapStopMs_ and
    // the gap-stopped state could persist
    // across link-lost / kickoff for the
    // entire app lifetime (the user's run B
    // silent wedge).
    {
        // tGapStopMs_ must be declared as a
        // member of Ping.
        assert(pingSrc.find("uint32_t tGapStopMs_ = 0;") != std::string::npos);
        // Must be cleared (= 0) in setPaused(false)
        // and in clearQueue_().
        auto sp = pingSrc.find("void setPaused(bool p)");
        std::string spWindow = pingSrc.substr(sp, 800);
        assert(spWindow.find("tGapStopMs_ = 0;") != std::string::npos);
        auto cq = pingSrc.find("void clearQueue_()");
        std::string cqWindow = pingSrc.substr(cq, 500);
        assert(cqWindow.find("tGapStopMs_ = 0;") != std::string::npos);
        std::cout
            << "  Pin 6: tGapStopMs_ declared, cleared in setPaused(false) "
               "and clearQueue_ \u2713"
            << std::endl;
    }
