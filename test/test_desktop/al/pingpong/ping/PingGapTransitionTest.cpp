...
void test_transition_table() {
    ...
    for (const auto &r : rows) {
        uint8_t next = 0;
        GapAction a =
            decideGapTransition(r.currentGap, r.lastNak,
                                pendingFrom(r.currentGap, r.lastNak, r.lastAck),
                                next);
...
void test_runtime_send_pause_on_nak() {
    ...
    // here gap stays NO_GAP, so sends proceed.
    {
        ...
        uint8_t next = gap;
        GapAction a = decideGapTransition(gap, lastNak,
                                    pendingFrom(gap, lastNak, lastAck), next);
        (void)a;
        gap = next;
...
void test_caller_sends_in_no_gap_steady_state() {
    ...
    for (int i = 0; i < 5; ++i) {
        uint8_t next = gap;
        GapAction a =
            decideGapTransition(gap, PING_GAP_NO_GAP,
                                pendingFrom(gap, PING_GAP_NO_GAP,
                                            PING_GAP_NO_GAP),
                                next);
        gap = next;
