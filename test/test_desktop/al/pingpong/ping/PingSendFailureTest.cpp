...
//     dead-peer signal, and a counter-then-drop
//     shape turns a saturated ASYNC flood into a
//     resweep loop (disc climbs 1→N on both peers,
//     link never recovers). The cooldown stamp on a
//     saturated cache is the throttle; the link's
//     own resweep is the recovery loop.
...
// recv-driven loop only counts; the wire-level ACK
// is the entire Pong-side response. The `echo %u %d`
// per-ack diagnostic was removed : Pong's
// recv path runs at the ASYNC pipeline rate and the
// per-chunk log flooded the wire log. ackCount_++
// + blinkWait is the on-the-wire side-effect; the
// periodic logStats line carries the running total.
void test_pong_no_payload_echo() {
    ...
    // The per-ack 'echo <seq> <bytes>' line was removed
    // : ASYNC pipeline rate flooded the log.
    ...
    std::cout << "  PASS (Pong recv-only; per-ack echo log removed; "
                 "no payload-echo send)"
              << std::endl;
}

// Pin 2: Ping's slot-completion log line is the
// labeled `echo#=... seq=... msgBytes=... pending=...`
// form. The bytes value is the message size from the
// local slot (queue_[head_].len), NOT the per-frame
// ACK's bytes-recvd — the ACK reports the merged-chunk
...
// not the user-visible message size. The unlabeled
// `echo <seq> <bytes> <pending>` line was removed
// in(see test_labeled_echo_companion_at_both_sites
// in PingMismatchCountTest).
void test_ping_echo_log_format() {
    ...
    // The labeled format string must be the 4-arg echo
    // with named fields — the unlabeled `echo %u %u %d`
    // form is gone.
    assert(src.find("echo %u %u %d") == std::string::npos);
    assert(src.find("echo#=%llu seq=%u msgBytes=%u pending=%d") !=
           std::string::npos);
    // The bytes value is queue_[head_].len, NOT
    // bytesRecvdFor() — that was bug. Pin both
    ...
    // calls base_.log_.info("Ping", "echo#=... seq=...
    // msgBytes=... pending=...", ...)). The msgBytes
    // arg must be queue_[head_].len in both, and
    // bytesRecvdFor(...) must NOT appear as the bytes
    // arg in either. We do this by checking the
    // substring between the labeled format string and
    // the head_++ advance.
    size_t searchFrom = 0;
    int lblSites = 0;
    int lenSites = 0;
    while (true) {
        auto fmtPos =
            body.find("echo#=%llu seq=%u msgBytes=%u pending=%d", searchFrom);
        if (fmtPos == std::string::npos)
            break;
        lblSites++;
        // Read the next 1500 chars after the format
        // string — covers the 4-arg labeled call and
        // the head_++ slot advance.
        ...
        // The msgBytes arg (third %u) must be the local
    ...
    assert(lblSites == 2);
    assert(lenSites == 2);
    std::cout << "  PASS (2 labeled echo sites; both read queue_[head_].len; "
                 "no bytesRecvdFor() in the echo log path)"
              << std::endl;
}

// Pin 3: backpressure retries must NOT escalate to dropLink.
// Bumping a consecSendFail_ counter on every backpressure
// send-failure and dropping at some threshold is a self-inflicted
// spiral under a legitimate ASYNC flood: the ARQ cache saturates,
// sendMsg returns false every iteration, the counter trips, the
// drop forces a resweep, the relock re-floods, the cache saturates
// again — the link never recovers. Backpressure is the link's own
// self-throttle; Ping's job is to pace sends (cooldown, not drop)
// and let the link layer's ARQ retransmit loop drain the cache.
...
void test_ping_msg_size_policy() {
    ...
    // the old 1024 collapsed the range to one value).
...
void test_ping_gap_stop_resume() {
    ...
    // The gap-stop / gap-resume branch reads lastNakSeq() and a
    // liveness predicate for the gapped seq, then updates gapSeq_.
    //
    // lastAckSeq() is not a usable predicate here: compared for exact
    // equality against the gapped seq. It names only the most
    // recently acked seq and advances thousands of times a second,
    // so a loop-rate sampler essentially never caught the instant
    // it matched and Resume never fired (a 23 s field stall on
    // seq=189 logged zero "gap resumed" lines). isAcked() answers
    // the same question edge-free, so this pin now requires it.
    assert(body.find("base_.comm_.lastNakSeq()") != std::string::npos);
    assert(body.find("base_.comm_.isAcked(") != std::string::npos &&
           "the gap branch must test the gapped seq's liveness "
           "directly, not infer it from a lastAckSeq() sample");
    // Pause log line.
    assert(body.find("gap stop: missing seq=") != std::string::npos);
    assert(body.find("gap resumed: seq=") != std::string::npos);
    std::cout << "  PASS (gapSeq_ + lastNakSeq()/isAcked() + "
                 "pause/resume logs)"
              << std::endl;
}
...
// flushRx() (matches the log-hygiene pin,
...
void test_ping_flushrx_at_swp_to_ok() {
    ...
    // base_.comm_.flushRx() must appear before the recv() drain
    // loop in the !wasReady_ block — draining via recv() alone
    // leaves a half-received frame in the rx buffer to fail the
    // next CRC.
