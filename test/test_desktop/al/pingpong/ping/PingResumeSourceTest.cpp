...
void test_setPaused_false_avoids_reactive_mismatch_path() {
    ...
    // matchEcho_() keeps the reactive "mismatch -> clear"
    // branch as a safety net, but the resume path must not
    // depend on it: setPaused(false) clears proactively.
    auto body = extractSetPausedBody(src);
    assert(body.find("clearQueue_()") != std::string::npos);
    assert(src.find("mismatchCount_") != std::string::npos);

    std::cout << "  PASS (resume clears proactively in setPaused)" << std::endl;
}
