...
// loop's tail queue drain). The unlabeled "echo %u %u %d"
// line was removed : a random draw's middle
// field was ambiguous between msgBytes and a sequence
// counter, so the unlabeled form was misread in field
// logs. Only the labeled `echo#=... seq=... msgBytes=...
// pending=...` line remains. Pinned by
...
void test_labeled_echo_companion_at_both_sites() {
    ...
    // call. We expect exactly 2 sites (gap-stop + tail drain).
    auto loopPos = src.find("void loop()");
    assert(loopPos != std::string::npos);
    ...
    // The unlabeled "echo %u %u %d" line is gone :
    // operator readability — the labeled line covers both
    // sites, and the unlabeled form's middle field was
    // ambiguous between msgBytes and a sequence counter.
    int echoSites = 0;
    searchFrom = 0;
    while (true) {
    ...
    std::cout << "  PASS (2 labeled echo sites; legacy 'echo %u %u %d' removed)"
              << std::endl;
}
