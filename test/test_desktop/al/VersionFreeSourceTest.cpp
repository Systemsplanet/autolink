...
// non-identifier, non-numeric boundaries so IPs and
// third-party pins (127.0.0.1, esp32@3.3.5) don't trip.
// Skips lines referencing AUTOLINK_VERSION (the macro is
// the only legitimate version token). The prior shape
// matched major 5 only, so 6.x.x refs accreted unseen.
static const std::regex kVersionPattern(R"((?:^|[^A-Za-z_0-9.])(v?[5-9]\.\d+\.\d+))");
