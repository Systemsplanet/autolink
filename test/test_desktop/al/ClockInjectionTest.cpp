...
void test_idle_timeout_drops_link() {
    ...
    // this release: idle watchdog fires on a
    // symmetrically quiet link past
    // cfg.idleTimeoutMs. The pre-this-release
    // shape had decideIdleWatchdog removed; this
    // release adds it back as the "general idle
    // watchdog" alongside the asymmetric-idle
    // check. With idleTimeoutMs=5000 and a 5500
    // ms quiet window, both nodes see rxAge>5000
    // and txAge>5000 and the watchdog drops
    // each side. The asymmetric check (TX active,
    // RX silent) still works for the stuck-send
    // case but is gated on arq_.pendingCount()>0
    // so an ACK-receiver doesn't trigger it.
    assert(dropA + dropB == 2);
...
void test_idle_watchdog_combined_tx_rx_v5_1_54() {
    ...
    // this release: idle watchdog fires on a
    // symmetrically quiet link past
    // cfg.idleTimeoutMs. With maxBurstPerLoop=0
    // and no traffic, both nodes' rxAge and
    // txAge exceed idleTimeoutMs=5000 within
    // the 5500 ms window, and the general
    // watchdog drops each side. The pre-fix
    // shape had dropA+dropB >= 1
    // (decideIdleWatchdog firing on the
    // symmetric condition); this release's
    // watchdog is the same shape with
    // cleaner integration into onTimerOk.
    assert(dropA + dropB >= 1);
    std::cout << "  PASS (both-silent drops via the general idle watchdog)"
              << std::endl;
}
