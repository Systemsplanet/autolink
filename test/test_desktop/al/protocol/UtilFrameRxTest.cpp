...
// v5.1.45: cobsSeq=0xFF is RESERVED for ACK_TYPE. The highest legal
// data-frame cobsSeq is 0xFE (COBS_SEQ_MAX).
void test_max_cobsSeq() {
 std::cout << "\n=== Test: cobsSeq=0xFE (max data) Passes Through ===" << std::endl;
 MockListener lis;
 UtilFrameRx rx(lis);
 std::vector<uint8_t> p = {0xAB};
 auto w = wireFrame(p, /*cobsSeq=*/0xFE);
 rx.feed(w.data(), (int)w.size());
 assert(lis.payloads.size() == 1 && lis.payloads[0] == p);
 assert(lis.seqs[0] == 0xFE);
 // And: cobsSeq=0xFF (ACK_TYPE) is NOT accepted as data — it's
 // routed to onAck.
 std::cout << "PASS" << std::endl;
}
