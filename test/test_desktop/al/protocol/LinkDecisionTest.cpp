...
static void test_classifyGap_stale_wraparound() {
    // v5.1.45: LD_SEQ_WRAP=255 (cobsSeq skips ACK_TYPE=0xFF), so half
    // is 127. diff > 127 → Stale. Boundary diff=127 → NOT stale.
    std::cout << "\n=== Test: classifyGap stale wraparound (diff > 127) ===" << std::endl;
    int d;
    // rxSeq=10, cobsSeq=200: diff = (200-10) = 190 > 127 -> Stale (wraparound).
    assert(classifyGap(200, 10, true, &d) == GapClass::Stale);
    assert(d == 190);
    // rxSeq=0, cobsSeq=200: diff=200 > 127 -> Stale.
    assert(classifyGap(200, 0, true, &d) == GapClass::Stale);
    assert(d == 200);
    // Boundary: diff exactly 127 -> NOT stale (> not >=), so Gap.
...
static void test_classifyGap_gap_small() {
    ...
    assert(d == 3);
    // Wrap-around: rxSeq=254 (max emitted), cobsSeq=0 (next after wrap).
    // v5.1.45: cobsSeq counter skips 0xFF, effective wrap=255. The
    // expected-next calculation returns 0 when rxSeq=254, so this is
    // the cobsSeq==expected path (diff=0 out, not the mod-diff path).
    assert(classifyGap(0, 254, true, &d) == GapClass::Forward);
    assert(d == 0);
    std::cout << "PASS" << std::endl;
}
