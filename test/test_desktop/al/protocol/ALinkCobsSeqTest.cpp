...
// cobsSeq wraparound. seq=253 -> seq=254 -> seq=0 must NOT be gaps
// (it wraps cleanly), and seq=1 after that must not be a gap (it's 2
// ahead of lastRxCobsSeq_=0, but expected is 1 so it IS a gap).
// v5.1.45: cobsSeq 0xFF is RESERVED (ACK_TYPE). Highest data seq
// is 0xFE (COBS_SEQ_MAX). The sender's counter increments
// 0,1,...,254,0,1,... (skips 255).
void test_wraparound_continuity() {
 ...
 // Send seq=253, then seq=254 (max), then seq=0 (wraparound).
...
void test_app_buffer_full_does_not_drop_link() {
 ...
 // v5.1.37: app-buffer-full does NOT bump `gaps`. Counters that
 // drive link-quality decisions stay at zero. The wire is fine —
 // the receiver just can't drain its buffer fast enough.
 // gaps are zero (NOT a wire error per v5.1.37 fix).
 b.getDiag(d);
