...
void test_httpd_stack_size_in_8192_to_16384() {
    std::cout << "\n=== httpd task stack size is heap-aware (8 KB "
                 "default) ==="
              << std::endl;
    std::string root = projectRoot();
    // cfg.stack_size lives in the httpd config block in
    // AutoLinkWebHttpd.cpp's setupHttpAndLogging_, not in
    // the handler TU.
    std::string src =
    ...
    // The heap-aware default: the 16384 setting was larger
    // than the dashboard needs (the chunked send's
    // per-iteration state is ~256 B; the 28 KB payload
    // lives in the heap, not the stack). 8192 hands 8 KB
    // back to the buffer floors — meaningful on a 41 KB
    // post-alloc free heap. Pin the value in
    // [8192..16384] so a regression to either extreme
    // trips here.
    if (stackSize < 8192 || stackSize > 16384) {
        std::cout << "  FAIL: cfg.stack_size = " << stackSize
                  << " (need in [8192..16384]).\n";
    }
    assert(stackSize >= 8192);
...
int main() {
    ...
    test_httpd_stack_size_in_8192_to_16384();
