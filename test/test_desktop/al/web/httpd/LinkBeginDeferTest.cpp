...
void test_link_begin_defers_kickoff_when_paused() {
    ...
    // kickoff when paused. H2: begin() now
    // returns bool (was void) — match
    // either signature.
    auto beginBody = extractFnBody(cSrc, "bool Link::begin()");
    if (beginBody.empty())
        beginBody = extractFnBody(cSrc, "void Link::begin()");
    ...
    // H2: begin() returns bool (was
    // void) — the early-return is
    // "return true;" now.
    ...
    // (between pausedCheck and kickoffCall). Match
    // either "return;" or "return true;" (H2).
    auto earlyReturn = beginBody.find("return", pausedCheck);
    assert(earlyReturn != std::string::npos);
    assert(earlyReturn < kickoffCall);
    // Confirm it's an actual return
    // (not a substring of e.g.
    // "returnValue").
    char afterReturn = beginBody[earlyReturn + 6];
    assert(afterReturn == ';' || afterReturn == ' ');
