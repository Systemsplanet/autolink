// Auto-generated split of earlier
...
// Pin: "Web monitor at http://..." must be logged exactly once per
// httpd startup path, not once inside setupHttpAndLogging_()
// (AutoLinkWebHttpd.cpp, right where enabled_ is set and httpd is
// confirmed running) AND again by every caller on a successful
// return. The field log showed the line twice, back-to-back, from
// the caller-side duplicate in AutoLinkWebLifecycle.cpp's WiFi
// background task retry loop.
void test_web_monitor_at_logged_exactly_once() {
    std::cout << "\n=== \"Web monitor at\" is logged exactly once ===\n";
    std::string lifecycleSrc = readFile(
        projectRoot() + "/src/al/web/handlers/AutoLinkWebLifecycle.cpp");
    std::string httpdSrc =
        readFile(projectRoot() + "/src/al/web/handlers/AutoLinkWebHttpd.cpp");
    assert(!lifecycleSrc.empty() && !httpdSrc.empty());
    ...
    const std::string needle = "\"Web monitor at http://%s:%u\"";
    int lifecycleCount = countOccurrences(lifecycleSrc, needle);
    int httpdCount = countOccurrences(httpdSrc, needle);
    assert(lifecycleCount == 0 &&
           "AutoLinkWebLifecycle.cpp must not log \"Web monitor at\" — "
           "setupHttpAndLogging_() (AutoLinkWebHttpd.cpp) already logs "
           "it on every successful start, regardless of which caller "
           "triggered it. A caller-side log duplicates the line for "
           "every caller.");
    assert(httpdCount == 1 &&
           "\"Web monitor at\" must be logged exactly once, from "
           "setupHttpAndLogging_() in AutoLinkWebHttpd.cpp");
    std::cout << "  PASS (logged once, from setupHttpAndLogging_() only)"
              << std::endl;
}
...
int main() {
    ...
    test_web_monitor_at_logged_exactly_once();
