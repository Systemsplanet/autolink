...
void test_max_uri_handlers_is_9() {
    std::cout
        << "\n=== AutoLinkWeb sets cfg.max_uri_handlers = 9 (HANDLERS_FULL guard) ==="
        << std::endl;
    ...
    // Pin: cfg.max_uri_handlers = 9;
    // Drop this line and 9 handlers will fight for 8
    // slots; the last handler (currently /reboot) silently
    // fails with HANDLERS_FULL and 404s at runtime.
    ...
    std::cout << "  PASS (cfg.max_uri_handlers = 9 in httpd config block)\n";
}
...
void test_uri_handler_count_matches_capacity() {
    ...
    // 9 handler slots, 9 declarations. Drift either side
    // (drop one r<N>, or add a tenth without bumping
    // max_uri_handlers) re-introduces the silent
    // HANDLERS_FULL failure mode.
    assert(n == 9);
    std::cout << "  PASS (9 r<N> handler declarations; matches "
              << "max_uri_handlers = 9)\n";
}
...
void test_paths_array_contains_all_nine_routes() {
    std::cout
    ...
    auto paths = parsePathsOrder(src);
    // 9 unique routes. A duplicate in PATHS[] (e.g.
    ...
    static const char *const mustHave[] = {
        ...
        "/level", "/mode",  "/pausemsg", "/delay", nullptr,
    };
    for (int i = 0; mustHave[i]; i++) {
        assert(seen.count(mustHave[i]) == 1);
    }
    std::cout << "  PASS (9 unique routes, every required route present)\n";
}
