...
//      AutoLinkWeb registers 12 handlers (r0..r11). The
...
//
// The fix is mechanical: cfg.max_uri_handlers = 12 right
...
// match earlier /level /reboot swap) makes the test
// fail loudly under `make test_uri_handler_alignment`.
// r10/r11 are the OTA upload routes (stream-to-flash +
// LittleFS; decision core host-pinned by OtaCoreTest).
#ifndef ARDUINO
...
void test_max_uri_handlers_is_12() {
    std::cout
        << "\n=== AutoLinkWeb sets cfg.max_uri_handlers = 12 (HANDLERS_FULL guard) ==="
        << std::endl;
    ...
    // Pin: cfg.max_uri_handlers = 12;
    // Drop this line and 12 handlers will fight for 11
    // slots; the last handler (currently /ota/gui)
    ...
    std::cout << "  PASS (cfg.max_uri_handlers = 12 in httpd config block)\n";
}
...
void test_uri_handler_count_matches_capacity() {
    ...
    // 12 handler slots, 12 declarations. Drift either side
    // (drop one r<N>, or add a thirteenth without bumping
    // max_uri_handlers) re-introduces the silent
    // HANDLERS_FULL failure mode.
    assert(n == 12);
    std::cout << "  PASS (12 r<N> handler declarations; matches "
              << "max_uri_handlers = 12)\n";
}
...
void test_paths_array_contains_all_twelve_routes() {
    std::cout
    ...
    auto paths = parsePathsOrder(src);
    // 12 unique routes. A duplicate in PATHS[] (e.g.
    ...
    static const char *const mustHave[] = {
        ...
        "/ota/fw", "/ota/gui", nullptr,
    };
    for (int i = 0; mustHave[i]; i++) {
        assert(seen.count(mustHave[i]) == 1);
    }
    std::cout << "  PASS (12 unique routes, every required route present)\n";
}
...
int main() {
    std::cout << "=== Running AutoLinkWeb URI Handler Alignment Tests ==="
              << std::endl;
    test_max_uri_handlers_is_12();
    test_uri_handler_count_matches_capacity();
    test_paths_matches_uris_order();
    test_paths_array_contains_all_twelve_routes();
