...
void test_dashboard_html_has_delay_widget() {
    std::cout << "\n=== Test: dashboard HTML has delay-ms widget ==="
              << std::endl;
    // The seven generated headers under src/al/web/generated/ are
    // unconditionally included by AutoLinkWebCore.cpp, so
    // DASHBOARD_HTML_DEFINED and real content are available on host
    // and ESP32 alike -- there is no stub path left to fall back to.
...
// AL87-02 regression: the sink-side drop counter (mutex-take
// failures inside logSinkCb) must be visible via /stats even
// though the dropped line itself is, by definition, never logged.
// Distinct from the /logs "dropped" field above (ring overrun).
void test_stats_json_has_web_log_dropped_field() {
    std::cout << "\n=== Test: /stats JSON carries webLogDropped ==="
              << std::endl;
    WebSnapshot s = {};
    std::strcpy(s.state, "OK");
    s.webLogDropped = 7;
    ...
    int len = formatStatsJson(&s, 3, AUTOLINK_VERSION, buf, sizeof(buf));
    assert(len > 0);
    assert(std::strstr(buf, "\"webLogDropped\":7") != nullptr);
    std::cout << "PASS" << std::endl;
}
...
// AL87-01 regression: formatLogsJson must report how many lines
// were lost to ring overrun instead of silently starting from the
// ring's true window. The buggy-original shape clamped `start` to
// the ring boundary with no signal in the response — a client
// polling with a stale `since` got a seamless-looking but
// incomplete log, which is exactly what made every field capture
// in this project's history undiagnosable (see docs/Version.md).
// Pinned by WebLogDropVisibleTest.
//
// formatLogsJson indexes with the compile-time WEB_RING_CAP, not
// the caller's array length, so the ring here must be sized
// WEB_RING_CAP (matching production's fixed-size ring) rather than
// a small stand-in — a smaller array would read out of bounds.
void test_logs_json_reports_dropped_on_overrun() {
    std::cout
        << "\n=== Test: formatLogsJson reports dropped on ring overrun ==="
        << std::endl;
    static WebLogEntry ring[WEB_RING_CAP];
    // head=WEB_RING_CAP+15: only the most recent WEB_RING_CAP
    // entries (seq 15..head-1) are still live. A client asking
    // since=0 has lost seq 0..14 (15 lines) with no way to know
    // unless the response says so.
    const uint32_t head = (uint32_t)WEB_RING_CAP + 15;
    for (uint32_t i = 15; i < head; i++) {
        WebLogEntry &e = ring[i % WEB_RING_CAP];
        e.seq = i;
        e.sev = 'I';
        std::snprintf(e.line, sizeof(e.line), "line%u", (unsigned)i);
    }
    char buf[8192];
    int len = formatLogsJson(ring, head, 0, buf, sizeof(buf));
    assert(len > 0);
    assert(std::strstr(buf, "\"dropped\":15") != nullptr);
    assert(std::strstr(buf, "\"seq\":15,") != nullptr);
    // Exact-match check (not a bare substring search): "seq":14
    // would also match inside "seq":140..149, which legitimately
    // exist in this range. The trailing comma pins it to the
    // literal value 14.
    assert(std::strstr(buf, "\"seq\":14,") == nullptr);
    std::cout << "PASS" << std::endl;
}

// A `since` that's already inside the still-live window (the
// common case — client polling faster than the ring fills) must
// report zero dropped, not a negative/wrapped count.
void test_logs_json_no_dropped_when_since_current() {
    std::cout
        << "\n=== Test: formatLogsJson reports 0 dropped when since is current ==="
        << std::endl;
    WebLogEntry ring[10] = {};
    for (uint32_t i = 0; i < 5; i++) {
        ring[i].seq = i;
        ring[i].sev = 'I';
        std::strcpy(ring[i].line, "x");
    }
    char buf[512];
    int len = formatLogsJson(ring, 5, 3, buf, sizeof(buf));
    assert(len > 0);
    assert(std::strstr(buf, "\"dropped\":0") != nullptr);
    std::cout << "PASS" << std::endl;
}
...
int main() {
    ...
    test_logs_json_reports_dropped_on_overrun();
    test_logs_json_no_dropped_when_since_current();
    test_stats_json_has_web_log_dropped_field();
