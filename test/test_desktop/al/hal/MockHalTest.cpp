...
void test_send_break_self_delivery_is_noop_without_peer() {
    ...
    MockHal hal;
    NullArqCache cache;
    // Link::Link calls hal.setEvents(*this) so the
    // HAL dispatches onBreak / onRx / onTimer without
    // ever seeing the concrete Link. Tests keep a
    // local Link* for state inspection.
    Link *link = new Link(hal, cache, true, AutoLinkConfig());
    hal.sendBreak();
...
void test_send_break_delivers_to_peer() {
    ...
    a.peer = &b;
    Link *aLink = new Link(a, cacheA, true, AutoLinkConfig());
    Link *bLink = new Link(b, cacheB, false, AutoLinkConfig());

    assert(aLink->getState() == State::OK);
    assert(bLink->getState() == State::OK);
    a.sendBreak();
    assert(a.sendBreakCalls == 1);
    // : a single BREAK in OK is debounced —
    // the the prior release shape reset immediately. The peer
    // link's breakSuspectMs_ must now be non-zero
    // (the confirm window is armed). The link state
    // stays OK until the confirm window expires or
    // a second BREAK arrives outside the coalesce
    // window. Pinned by BreakConfirmTest.
    LinkTestAccessor bAcc(*bLink);
    assert(bAcc.breakSuspectMsForTest() != 0 &&
           "Pin: peer link's onBreak must arm the confirm window "
           "after sendBreak");
    assert(bLink->getState() == State::OK &&
           "Pin: peer link stays OK during the confirm window");
    assert(aLink->getState() == State::OK);
    delete aLink;
    delete bLink;
    std::cout << "PASS" << std::endl;
