...
void test_esphal_begin_does_not_retry_on_esp_fail_and_logs_oom() {
    ...
    auto beginPos = src.find("EspHal::begin(");
...
void test_esphal_derives_stream_buffer_size_from_maxmsg() {
    ...
    // from cfg.maxMsg internally. The old facade
    ...
    assert(floorWindow.find("slots = 16") == std::string::npos &&
           "the old slot count constant must be gone from the HAL");
    assert(floorWindow.find("32 * (cfg.maxMsg") == std::string::npos &&
           "the old 32x factor must be gone from the HAL");
    ...
    assert(cfgFloorSlice.find("multiples = 2") != std::string::npos &&
           ...
           "constant (2x maxMsg + hdr padding, not the old 32x "
           "pipeline factor)");
...
void test_autolink_config_default_tx_buffer_size_is_256() {
    ...
    // Sanity: the field must NOT be zero (the old default).
    assert(init != "0");
...
// Pin 7: EspHal::begin() must log AUTOLINK_VERSION in
// the begin: line so a field log carries the build
// version that produced it. The prior shape's begin:
// line had no version — field logs couldn't prove
// which build produced a death-spiral, and rolling
// back to investigate meant re-flashing and re-running
// the field case. Source-level pin because EspHal.cpp
// is `#ifdef ARDUINO` and can't run on host.
static void test_esphal_begin_logs_autolink_version() {
    std::cout << "\n=== Pin 7: EspHal::begin() logs "
                 "AUTOLINK_VERSION in the begin: line "
                 "(source-grep) ==="
              << std::endl;
    auto cpp = readFile(autolink::testRepoPath("src/al/hal/EspHal.cpp").c_str());
    assert(!cpp.empty());
    // The begin: log line must include `version=`.
    // AUTOLINK_VERSION is the build's version macro
    // (read from AUTOLINK_VERSION in include/AutoLink.h);
    // stringifying the macro into the log line
    // produces a literal at compile time, which is
    // what we want in a field log.
    auto versionLog = cpp.find("begin: version=");
    assert(versionLog != std::string::npos &&
           "Pin 7: EspHal::begin() must log "
           "`begin: version=` so field logs carry "
           "the build version. Without this, "
           "investigating a death-spiral in the "
           "field requires re-flashing and re-running "
           "the case to find the build version that "
           "produced the log.");
    // AUTOLINK_VERSION must be referenced in the
    // log line (the macro, not a literal string).
    auto macro = cpp.find("AUTOLINK_VERSION", versionLog);
    assert(macro != std::string::npos &&
           "Pin 7: the version= log line must "
           "reference the AUTOLINK_VERSION macro "
           "(from include/AutoLink.h), not a literal "
           "string. A literal string would be a "
           "version anchor in source, which rule 12 "
           "forbids; the macro is the legitimate "
           "path.");
    // include/AutoLink.h must define AUTOLINK_VERSION
    // (it always has, but the test pins the contract).
    auto h = readFile(autolink::testRepoPath("include/AutoLink.h").c_str());
    assert(!h.empty());
    auto macroDef = h.find("#define AUTOLINK_VERSION");
    assert(macroDef != std::string::npos &&
           "Pin 7: include/AutoLink.h must define "
           "AUTOLINK_VERSION (the contract that "
           "EspHal::begin() reads).");
    std::cout << "  PASS (EspHal::begin() logs "
                 "AUTOLINK_VERSION; macro is the "
                 "legitimate non-anchored path)"
              << std::endl;
}
...
int main() {
    ...
    test_esphal_begin_logs_autolink_version();
