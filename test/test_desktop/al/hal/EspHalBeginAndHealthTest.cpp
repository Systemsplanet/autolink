...
// version that produced it. Source-level pin because
// EspHal.cpp is `#ifdef ARDUINO` and can't run on host.
static void test_esphal_begin_logs_autolink_version() {
    std::cout << "\n=== Pin 7: EspHal::begin() logs "
              ...
              << std::endl;
    auto cpp =
        readFile(autolink::testRepoPath("src/al/hal/EspHal.cpp").c_str());
    assert(!cpp.empty());
