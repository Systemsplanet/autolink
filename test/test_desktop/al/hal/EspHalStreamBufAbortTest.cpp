...
void test_pingpongbase_halts_on_unhealthy() {
    ...
    // Visible halt: blink the LED every 300 ms in an
    // infinite loop so the operator sees the failure
    // without watching the debug Serial. The previous
    // shape was `delay(1000)` — silent on the bench.
    assert(slice.find("blinkWait(1, 300, 300, 0)") != std::string::npos &&
           "halt loop must blink the LED at 300 ms cadence");
    // The old silent-halt shape must be gone.
