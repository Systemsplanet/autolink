// Closed-loop WireSim end-to-end: link-up + ARQ
// delivery across hostile drop model.
...
void test_closed_loop_with_forced_drops()
{
    std::cout << "\n=== Test: 5000-cycle closed loop "
                 "with 2% drop + forced drops every "
                 "800 (killer) ==="
              << std::endl;
    ...
    for (int warmup = 0; warmup < 1500; warmup++) {
        ...
        if (f.getStateA() == State::OK &&
            f.getStateB() == State::OK)
            break;
    }
    if (f.getStateA() != State::OK ||
        f.getStateB() != State::OK) {
        std::cerr
            << "\nFAIL: link never reached OK (A="
            << (int)f.getStateA()
            << ", B=" << (int)f.getStateB() << ")"
            << std::endl;
        assert(false);
    ...
    std::cout << "  5000 cycles, "
              << sim.dropsInjected() << " drops, "
              << sim.framesDropped()
              << " wire bytes dropped, "
              << "total bytes A->B: "
              << f.totalBytesAtoB() << std::endl;
    if (f.totalBytesAtoB() < 100) {
        std::cerr
            << "\nFAIL: too few bytes transferred ("
            << f.totalBytesAtoB() << "), link is dead"
            << std::endl;
        assert(false);
    }
...
void test_closed_loop_saturates_after_many_drops()
{
    ...
    std::cout << "  3 drops, cache clean, link back "
                 "to OK, bytes resumed (cycle="
              << sim.dropsInjected() << " drops total)"
              << std::endl;
    std::cout << "PASS" << std::endl;
}
...
void test_closed_loop_single_drop_recovers()
{
    std::cout << "\n=== Test: single forced drop, "
                 "cache stays clean (regression) ==="
              << std::endl;
    Log::log().setLevel(Log::Level::WARNING);
...
int main()
{
    std::cout << "=== Closed-Loop Two-Node AutoLink "
                 "Tests (WireSim) ==="
              << std::endl;
...
{
    std::cout
        << "\n=== Test: gap heals without re-sweep at "
           "5 fill levels (killer) ==="
        << std::endl;
    Log::log().setLevel(Log::Level::WARNING);
    ...
    for (int fi = 0; fi < 5; fi++) {
        ...
        if (f.getStateA() != State::OK ||
            f.getStateB() != State::OK) {
            std::cerr << "FAIL: link never reached OK "
                         "at fill="
                      << targetFill << std::endl;
            assert(false);
        }
        ...
        uint64_t bytesBefore =
            sim.bytesTransferredAtoB() +
            sim.bytesTransferredBtoA();
        ...
        if (f.getStateA() != State::OK ||
            f.getStateB() != State::OK) {
            std::cerr
                << "FAIL fill=" << targetFill
                << ": link re-swept (stateBefore=OK, "
                   "stateAfter A="
                << (int)f.getStateA()
                << " B=" << (int)f.getStateB() << ")"
                << std::endl;
        ...
        if (sim.dropsInjected() > 0) {
            std::cerr << "FAIL fill=" << targetFill
                      << ": forced link drop fired "
                         "unexpectedly (dropsInjected="
                      << sim.dropsInjected() << ")"
                      << std::endl;
            assert(false);
        }
        std::cout << "PASS (bytesDelta="
                  << (bytesAfter - bytesBefore) << ")"
                  << std::endl;
    }
    std::cout << "  All fill levels: gap heals "
                 "without re-sweep PASS"
              << std::endl;
}
