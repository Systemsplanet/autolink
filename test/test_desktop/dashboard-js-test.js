...
async function test_pause_toggle() {
    ...
    const dom = await setup();
    // : toggleMsgPause no-ops when role isn't Ping. Force
...
// wraps the body in try/finally to guarantee this.
...
// Save button: present in the log controls, downloads the log
// content as ping.txt (or pong.txt) on click. We pin the element's
// existence + the saveLog function path by setting a known
// deviceRole and inspecting the chosen filename. The actual Blob
// download is hard to verify in jsdom (no native <a download>
// behavior); we check the element + function exist and the
...
async function test_save_log_button_present() {
    ...
    truthy(sbtn, 'sbtn (Save) should exist');
    // Pin: it's labeled "Save", not "Download" or anything else.
    const text = sbtn.textContent.trim();
    eq(text, 'Save', 'button label should be "Save"');
    // Pin: it has the click handler bound.
    assert(typeof dom.window.saveLog === 'function',
...
async function test_save_log_filename_by_role() {
    ...
    const dom = await setup();
    // We can't easily spy on the <a download> attribute, but we
    // can verify the role-based filename selection by reading
    // the source. The function is short enough to grep:
    //   var name=(deviceRole==='Pong'?'pong.txt':
    //             (deviceRole==='Ping'?'ping.txt':'autolink.txt'));
    const src = fs.readFileSync(
        path.join(__dirname, '../../src/al/web/AutoLinkWebHtml.h'),
        'utf8'
    );
    truthy(src.indexOf("'ping.txt'") !== -1,
        "source should reference 'ping.txt' for the Ping role");
    truthy(src.indexOf("'pong.txt'") !== -1,
        "source should reference 'pong.txt' for the Pong role");
    truthy(src.indexOf('a.download=name') !== -1,
...
async function test_pause_logs_state_change() {
    ...
    const dom = await setup();
    // : gating requires role to be Ping before any toggle fires.
    dom.window.deviceRole = 'Ping';
    const topPbtn = dom.window.document.getElementById('topPbtn');
    truthy(topPbtn, 'topPbtn should exist');
...
// : toggleMsgPause must POST /pausemsg to the device, not just
...
async function test_pause_toggle_posts_to_pausemsg_endpoint() {
    ...
    const dom = await setup();
    // : toggleMsgPause no-ops when role isn't Ping. Set up
...
// : pause toggle must be a no-op when role is not Ping.
...
// : before the first /stats response lands, deviceRole is
...
// : /stats.msgPaused is reconciled into the UI on every poll.
...
async function test_pause_label_reconciles_with_stats_msgpaused() {
    ...
    const dom = await setup();
    // : priming poll sets the role. But the test sets up the
...
// : the user reported "reset button doesn't reset counters".
...
// ---- Copy button: regression for the "stuck on Copied" bug ---------------
// The bug: when navigator.clipboard.writeText() failed and copyLog()
// fell through to fallbackCopy(), fallbackCopy set the button text
// to "✓ Copied" but never reverted it. The button was stuck on
// "Copied" until the next page load. The modern clipboard path
// (writeText success) already had a setTimeout revert; the fallback
// path did not. These tests pin both paths revert the label back to
// "Copy" after ~1200 ms.
...
async function test_copy_modern_path_reverts_button_label() {
    ...
    // navigator.clipboard is jsdom-default: a writable stub. Replace
    // writeText with a resolved Promise so the modern path runs.
    ...
    truthy(btn, 'cbtn must exist');
    truthy(btn.textContent === 'Copy',
        'cbtn initial text should be "Copy", got: "' + btn.textContent + '"');
    win.copyLog();
    // The clipboard.writeText is awaited inside a .then() handler
    // that flips the label to "Copied". One microtask flush should
    // land us in the .then().
    await new Promise((r) => setTimeout(r, 50));
    truthy(writeTextCalls === 1,
        'copyLog() should call navigator.clipboard.writeText once, got: ' + writeTextCalls);
    truthy(btn.textContent.indexOf('Copied') !== -1,
        'after copyLog(), cbtn should show "Copied", got: "' + btn.textContent + '"');
    // Wait for the 1200 ms revert setTimeout to fire.
...
async function test_copy_fallback_path_reverts_button_label() {
    ...
    const dom = await setup();
    const win = dom.window;
    // Force the fallback path: navigator.clipboard must be falsy
    // so copyLog() takes the else branch into fallbackCopy().
    Object.defineProperty(win.navigator, 'clipboard', {
        configurable: true, get: function() { return undefined; }
    });
    // jsdom doesn't implement document.execCommand by default;
    // stub it to return true so fallbackCopy's success branch
    // runs (sets label to "Copied").
    win.document.execCommand = function(cmd) {
        truthy(cmd === 'copy', 'fallback should call execCommand("copy")');
        return true;
    };
    const btn = win.document.getElementById('cbtn');
    truthy(btn, 'cbtn must exist');
    truthy(btn.textContent === 'Copy',
        'cbtn initial text should be "Copy", got: "' + btn.textContent + '"');
    win.copyLog();
    // fallbackCopy is synchronous, so the label flip happens on
    // the same tick.
    truthy(btn.textContent.indexOf('Copied') !== -1,
...
async function test_fallbackCopy_sets_copied_label_on_success() {
    ...
    const dom = await setup();
    const win = dom.window;
    // Set up a button the test can pass to fallbackCopy directly.
    ...
    // The temporary textarea must be removed from the DOM after the copy.
    const stray = win.document.querySelectorAll('textarea');
    truthy(stray.length === 0,
...
async function test_fallbackCopy_sets_failed_label_on_throw() {
    ...
    win.fallbackCopy('hello world', btn);
    truthy(btn.textContent.indexOf('Failed') !== -1,
