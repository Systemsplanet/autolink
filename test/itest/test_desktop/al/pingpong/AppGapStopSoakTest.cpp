...
// Returns worst gap-stop duration observed on either side, in ms
// (including any episode still open when the sim ends).
uint32_t runCell(const char *name, AutoLinkConfig::Mode mode, bool randomFill,
                 int lossPermille, uint32_t simMs) {
...
int main() {
    ...
    const Cell cells[] = {
        { "ASYNC/serial", AutoLinkConfig::Mode::ASYNC, false },
        { "ASYNC/random", AutoLinkConfig::Mode::ASYNC, true },
        { "SYNC/serial", AutoLinkConfig::Mode::SYNC, false },
        { "SYNC/random", AutoLinkConfig::Mode::SYNC, true },
    };
    ...
    for (const Cell &c : cells) {
        uint32_t worst = runCell(c.name, c.mode, c.randomFill, kLossPermille,
                                 kSimMs);
        if (worst > kMaxAcceptableStallMs) {
            printf("  FAIL: %s worst gap-stop %u ms exceeds %u ms\n",
                  c.name, worst, kMaxAcceptableStallMs);
            allOk = false;
        }
    }
