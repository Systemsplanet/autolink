// Fix 4: a receive-only peer (the field's Pong role — its
// application loop only calls recvMsg, never sendMsg) never
// receives an ACK frame of its own, so onAck's recentDiscs_ = 0
// never executes. recentDiscs_ had exactly one clear site
// (onAck), so a receive-only peer had no path to it at all —
// three disconnects inside 10 s tripped its own
// DISC_STORM_THRESHOLD veto and forced a full P1 walk down to the
// slowest baud, at the exact moment the sender fast-pathed P3 back
// to the preserved (fast) baud: a guaranteed baud mismatch.
//
// Fix: a CRC-valid recvMsg delivery is the same "the locked baud
// is actually good" evidence as an ACK, so it clears recentDiscs_
// too (LinkApi.cpp), with the same clear added at the valid-NAK
// site (LinkRx.cpp) for symmetry.
//
// Pin: two real nodes, sender + receive-only peer, locked once via
// a real negotiation. Three HealthWatchdog-reason disconnects
// inside 10 s (simulated time), each followed by a real
// reconvergence and one real sendMsg/recvMsg round trip (the
// receive-only peer's only path to clear its own counter). Every
// disconnect must still take the preserved-baud PHASE3 fast path
// and relock at the original (fast) baud index — never fall back
// to the slow baud. Revert either recentDiscs_ = 0 insertion ->
// red: the receive-only peer's recentDiscs_ reaches
// DISC_STORM_THRESHOLD on the third disconnect and it walks away
// from the fast baud.
#include <iostream>
#include <cassert>
#include "MockHal.h"
...
bool pumpUntilOk(Link &a, Link &b, MockHal &mHal, MockHal &sHal,
                 int maxIters) {
...
void test_receive_only_peer_fast_paths_through_repeated_disconnects() {
    ...
    std::cout << "  initial lock: spdI=" << fastSpdI << " (baud="
              << kBauds[fastSpdI] << ")" << std::endl;
    ...
    for (int cycle = 1; cycle <= 3; cycle++) {
        ...
        accSender.resetLink(true, /*preserve=*/true, ResetReason::HealthWatchdog);
