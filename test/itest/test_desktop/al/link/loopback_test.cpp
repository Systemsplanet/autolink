...
// Minimal ARQ cache for the itest harness. Uses the production
// ArqCache directly, so the link's retx path has somewhere to
// re-send from.
...
static int run_loopback(int seconds, bool verbose, int mode, bool reliable,
    ...
    for (;;) {
        ...
        auto deltaMs = std::chrono::duration_cast<std::chrono::milliseconds>(
                           now - lastTick)
                           .count();
        // Carry the sub-ms remainder. The buggy-original
        // shape assigned lastTick = now and
        // discarded the fractional ms every
        // iteration; with sub-ms per-iteration
        // wall time the sim clock crawled (an
        // instrumented run reached sim now=1ms
        // after 40s of wall time). That
        // settle gate is the first post-lock
        // feature that needs sim time to advance
        // by an absolute amount, so the gate
        // never closed and every frame was
        // dropped forever. Add the integer ms
        // delta to lastTick (not assign) to
        // preserve the remainder for the next
        // iteration. Pinned by
        // LoopbackHarnessClockTracksWallTest.
        lastTick += std::chrono::milliseconds(deltaMs);
        if (deltaMs < 0)
            deltaMs = 0;
        if (deltaMs > 100)
            deltaMs = 100;
        ...
        // Drain the in-process log queue every
        // iteration. per-subsystem
        // debug-rate logging floods the bounded
        // host log queue; without a per-iter
        // drain, the queue-overflow warning
        // spammed the console and destroyed
        // every settled / state-change line the
        // operator would otherwise have read
        // (the wedge-diagnosis evidence was
        // buried under 1000s of "queue
        // overflow" lines in the prior instrumentation
    ...
    if (sawOk && okHoldMs >= 1000 && rxCount > 0) {
        // Sanity check: the harness's sim clock
        // must track wall time. The buggy-original
        // shape's `lastTick = now;` discarded
        // the sub-ms remainder every iteration;
        // with sub-ms per-iter wall time the sim
        // clock crawled (an instrumented run
        // reached sim now=1ms after 40s of wall
        // time). The settle gate is the
        // first post-lock feature that needs sim
        // time to advance by an absolute amount,
        // so a crawling sim clock wedged every
        // loopback. Assert the sim clock is
        // within 25% of the wall time over the
        // test run. 25% tolerance absorbs the
        // deltaMs-clamp-at-100 and the per-iter
        // std::this_thread::sleep_for overhead.
        // Pinned by this assertion; a future
        // regression to `lastTick = now;` would
        // drive sim now toward zero and trip the
        // upper-bound.
        uint32_t simNow = g_pingHal->now;
        uint32_t wallMsTotal =
            (uint32_t)(std::chrono::duration_cast<std::chrono::milliseconds>(
                           clk::now() - t0)
                           .count());
        if (simNow < (wallMsTotal * 3) / 4 || simNow > wallMsTotal + 200) {
            printf("\nFAIL: sim clock drift. wallMs=%u simNow=%u (must "
                   "track within 25%% / +200ms)\n",
                   wallMsTotal, simNow);
            return 3;
        }
        printf("\nPASS: link reached OK, held >= 1s,  received data; "
               "sim clock tracks wall (wall=%ums sim=%ums)\n",
               wallMsTotal, simNow);
        return 0;
    }
    if (sawOk) {
