// Regression: random-content payloads must not trigger cascading
// SYNC resyncs. The OK-state CTRL-scan must validate the CRC before
// claiming a `0xAA 0x55 X Y` 5-byte run as a CTRL frame candidate —
// random payload content collides with the sentinel at ~1/65536 per
// byte (every multi-chunk Ping payload hits ~1% chance per chunk),
// and a CRC8 fail on a non-CTRL collision calls err_unlocked() ->
// frameErrs++ -> errThreshold drop. A CTRL-scan with no CRC
// pre-check is safe only for low-entropy sequential fill.
//
...
static void pump_thread() {
    ...
    while (!g_pumpStop.load()) {
        ...
        if (deltaMs > 50)
            deltaMs = 50;
        // Carry the sub-ms remainder. See
        // loopback_test.cpp for the rationale
        // (sim-clock-crawl bug).
...
static int run_loopback_random_fill(int seconds) {
    NullArqCache cache;
    AutoLinkConfig cfg;
    // A deterministic config: maxMsg stays at the AutoLinkConfig
    // default; streamBufferSize uses the host-test MockHal
    // (unbounded app buf) so this test isolates the link layer from
    // app-side buffering concerns.
    cfg.maxMsg = 5120;
    ...
    while (true) {
        ...
        // Yield between idle iterations. Without this the outer loop
        // spins the CPU with no sleep whenever it isn't inside the
        // ack-wait's own 500 us sleeps, and on a single-core host that
        // starves pump_thread's 2 ms cadence — pumpClock/drainTx never
        // get a scheduling slice, so a later message's ACK genuinely
        // never arrives. Not a protocol bug: the driver thread was
    ...
    if (steadyDropsA + steadyDropsB > 0) {
        std::cerr << "FAIL: " << (steadyDropsA + steadyDropsB)
                  ...
                  << " (steady-state random-fill wedge)" << std::endl;
        return 1;
    }
