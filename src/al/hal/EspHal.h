...
class EspHal : public IHal {
    ...
    // Derived from cfg.mode in setMode (pre-
    // begin) or in begin (if no setMode ran
    // first); the driver-install path reads
    // these, not cfg.rxBufferSize /
    // cfg.txBufferSize directly. A caller-
    // set larger value still wins because
    // uartRxBufferFloor / uartTxBufferFloor
    // return the max of (cfg.rxBufferSize,
    // floor). Pre-this-release the UART
    // driver was always sized off the
    // SYNC-shaped cfg defaults, which
    // caused UART overruns on a multi-
    // chunk ASYNC flood.
...
public:
    ...
    // Update the HAL's mode copy. Pre-begin()
    // re-derives UART and stream-buffer sizes
    // from the new mode; post-begin() only
    // updates cfg.mode (the live UART/stream
    // buffers can't be resized in flight —
    // mode switches go through the NVS+reboot
    // path, see AutoLinkWeb::handleMode).
    void setMode(AutoLinkConfig::Mode m) override {
        if (running) {
            // Post-begin: the wire buffers are
            // committed. cfg.mode is updated
            // only so the boot log and any
            // future helper that reads the HAL
            // copy sees the right mode; the
            // UART/stream buffers stay at
            // their begin()-time shape.
            cfg.mode = m;
            return;
        }
        cfg.mode = m;
        // Pre-begin: re-derive the buffer
        // floors so a NVS-restored mode
        // applied before begin() sizes the
        // buffers for that mode. The
        // pre-fix shape held a stale cfg
        // copy and sized for SYNC even
        // when cfg.mode was ASYNC — the
        // UART rx underran on a multi-
        // chunk ASYNC flood.
    ...
    void begin() override {
        ...
        // Pre-begin setMode() may have
        // already derived these; if begin()
        // is reached without a setMode call,
        // derive from the ctor's cfg copy.
