    ...
    while (hal->running) {
        if (xQueueReceive(hal->uart_queue, (void *)&event,
            ...
            if (event.type == UART_DATA) {
                ...
                // onBreak() returns early in any non-OK state (see
                // LinkTimerBreak.cpp), so a delivered BREAK during
                // SWP has no compensating recovery logic — flushing
                // RX here destroys in-flight sweep frames for a
                // BREAK glitch whose source is the peer's own
                // setSpd transition, not a real link drop. Only an
                // OK-state BREAK is a genuine peer-drop signal
                // worth flushing stale RX for. POST_SETSPD_BREAK_
                // GUARD_MS above only guards *local* setSpd
                // glitches; this guards the peer's. Pinned by
                // EspHalBreakFlushGuardTest.
                if (hal->okState)
