...
struct AutoLinkConfig {
    ...
    // Free-heap margin begin() must leave
    // untouched after allocating the UART +
    // stream buffers (WiFi/httpd/OTA need it).
    // capFloorByHeap() shrinks the ASYNC
    // floors toward their minimums when the
    // ask would eat into this reserve; 0
    // disables the cap.
    ...
    // Default 0 = full line rate; callers who need a
    // throttled bench run set this (or the dashboard
    // dropdown) themselves.
    int txDelayMs = 0;
};
...
// Heap-aware cap for one buffer ask. `want` is
// the derived floor, `minFloor` the smallest
// still-functional size (one full coalesced
// message for the stream buffer, the SYNC
// baseline for the UART rx buffer). Grants
// `want` when free heap covers it plus the
// reserve; otherwise grants what fits above
// the reserve, never below `minFloor` (the
// link cannot run below it — better to eat
// the reserve than come up broken). Callers
// apply this per buffer, largest first,
// subtracting each grant from freeHeap.
// Pure arithmetic — host-pinned by
// HeapCapFloorTest.
inline size_t capFloorByHeap(size_t want, size_t minFloor, size_t freeHeap,
    ...
    if (reserve == 0 || freeHeap == 0)
        return want; // 0 = cap disabled / heap unknown (host stub)
