...
void Link::reset_unlocked(bool count) {
    if (count && state == State::OK) {
        discCount++;
#ifdef ARDUINO
        // Triage aid: log free heap at each
        // disc-rate-triggered resweep so a
        // future bench run can confirm
        // whether the disc-counter climb is
        // heap-pressure-driven (POOL_SIZE
        // starvation during uart_driver_install)
        // or a pure state-machine race (Fixes
        // 1-3). On the FireBeetle32 a stable
        // free-heap across cycles is the
        // expected outcome after Fix 1-3; a
        // monotonic drop would point back at
        // the static .bss buffer pressure.
