...
public:
    ...
    //
    // Open 6: a gap held past reorderHoldMs
    // whose missing cobsSeq never arrives must
    // be abandoned — advance rxSeq past it and
    // count lostMsgs — instead of pinning the
    // window open. The contiguous-resume logic
    // in flushContiguous looks up slots_[exp]
    // and breaks on in_use==false; the only way
    // to resume after a never-arriving gap is
    // to advance rxSeq so the next exp slot is
    // the one AFTER the dropped one. Without
    // this, a single lost chunk pins the rx
    // window open forever, the receiver's
    // gap-stop NAKs the same missing seq
    // indefinitely, and a clean retransmit that
    // arrives after reorderHoldMs is silently
    // rejected as stale (the rx seq never moved).
    //
    // Implementation: dropExpired records the
    // set of dropped seqs and returns them via
    // the out-array; the Link caller advances
    // rxSeq across each one. (dropExpired itself
    // doesn't have a handle on the Link's rxSeq
    // — it would need a LinkContext&, which the
    // existing flushContiguous signature already
    // carries but dropExpired doesn't.)
    int dropExpired(uint32_t nowMs, int holdMs, uint8_t *droppedSeqs,
                    int maxDropped);
