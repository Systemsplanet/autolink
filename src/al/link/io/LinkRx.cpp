...
bool Link::onAck(uint8_t ackedCobsSeq, uint16_t bytesRecvd) {
    ...
    for (;;) {
        ...
        // A cumulative ACK proves every seq in the walked range
        // landed, not just ackedCobsSeq — release the NAK latch
        // for any of them, matching the single-seq clear above.
        if (lastNakSeq_ == s)
            lastNakSeq_ = 0xFF;
        if (s == ackedCobsSeq)
            break;
