...
int Link::okTickMs() const {
    ...
    // ASYNC invariant: tick <= reorderHoldMs/2,
    // or the receiver expires held frames before
    // the timer-driven retx can close the gap and
    // a single chunk loss is unrecoverable. SYNC
    // never walks the ARQ table; no clamp.
