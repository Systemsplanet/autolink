...
int Link::okTickMs() const {
    ...
    // ASYNC coupling: the receiver's reorder
    // buffer expires an out-of-order frame
    // after reorderHoldMs and bumps lostMsgs.
    // The sender's OK-state timer tick is the
    // only thing that drives a retransmit
    // (see LinkArq::decideSlot walking the
    // 256-slot table). If the tick period
    // exceeds reorderHoldMs, a single ARQ
    // loss becomes unrecoverable: the
    // receiver expires the held frames
    // before the sender gets a chance to
    // resend the missing seq, and the tail
    // gets force-dropped on a forced
    // resweep. The pre-this-release shape
    // capped the tick at idleTimeoutMs/3
    // (=3333 ms at the default) while
    // reorderHoldMs=1500 ms — a guaranteed
    // unrecoverable loss on a single
    // ASYNC chunk drop. The fix clamps the
    // tick at reorderHoldMs/2 so the
    // sender retx always lands ahead of the
    // receiver's expiry. SYNC mode never
    // walks the ARQ table so the clamp
    // doesn't apply.
    if (cfg.mode == AutoLinkConfig::Mode::ASYNC) {
        int halfReorder = cfg.reorderHoldMs / 2;
        if (halfReorder > 0 && k > halfReorder)
            k = halfReorder;
        if (k < 50)
            k = 50;
    }
