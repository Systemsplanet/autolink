...
void LinkArq::clearAll() {
    ...
    memset(nakCount_, 0, sizeof(nakCount_));
    memset(sentAtMs_, 0, sizeof(sentAtMs_));
    memset(baseSeq_, 0, sizeof(baseSeq_));
    // D10: clear the lap stamps so a
    // bytesForMessage walk after clearAll doesn't
    // find a stale lap from a prior session.
    // The clearAllEpoch_++ below means a
    // waitForAck in flight on the prior session
    // sees its genAtUnlock mismatch and bails.
    // Pinned by ClearAllWipesLapStampsTest.
    memset(baseLap_, 0, sizeof(baseLap_));
    gbnBase_ = 0;
    gbnActive_ = false;
    clearAllEpoch_++;
    // ARQ state reset: every pending slot, base, and retx counter
    // is wiped. The clearAllEpoch_ bump invalidates any caller
    // that unlocked around waitForAck — that caller now sees
    // its answer as "ack arrived, but a reset happened in
    // between" (the false return path) so it can re-evaluate
    // against the fresh session. Info level: this fires on
    // every link reset and is logged as the start of every
    // new session's ARQ bookkeeping.
    Log::log().info(TAG, "ARQ clearAll (epoch=%lu)",
                    (unsigned long)clearAllEpoch_);
#ifdef ARDUINO
...
void LinkArq::onSent(uint8_t seq, uint8_t baseSeq, uint32_t nowMs,
    ...
    // E8: stamp the lap. The lap is the
    // sender-side cobsSeq wrap count (not the
    // session clearAllEpoch_ — the per-message
    // qualifier is the seq-namespace wrap, not
    // the session's life). onSent's baseLap
    // argument is the tx-side lap at the
    // moment of stamping; the byte-rotation
    // from the same seq 254 chunks later
    // carries a different lap, so the
    // bytesForMessage walk won't alias the
    // two messages. Pinned by
    ...
    Log::log().verbose(TAG, "ARQ onSent seq=%u base=%u epoch=%lu (pending=%d)",
                       (unsigned)seq, (unsigned)baseSeq_[bi],
                       (unsigned long)clearAllEpoch_, pendingCount());
#endif
}
...
void LinkArq::rearmSlot(uint8_t seq, uint32_t nowMs) {
    ...
    int bi = budgetIdx(seq);
    retxCount_[bi] = 0;
    // NAK count survives rearm — a stuck base the
    // honest-drop policy kept alive may have seen NAKs;
    // the next rearm reopens the window but the
    // accumulated NAK evidence is part of the
    // "alive peer" signal that justified the keep.
    sentAtMs_[bi] = nowMs;
...
void LinkArq::onAcked(uint8_t seq, uint16_t bytesRecvd) {
    ...
    retxCount_[bi] = 0;
    // NAK count resets with the ACK — the chunk is
    // delivered, the peer's loss signal is now
    // historical evidence, not a live fact.
    nakCount_[bi] = 0;
    // baseSeq_[bi] is set to a sentinel (0xFF, an
    // impossible real cobsSeq since the wire's range is
    // 0..0xFD) so a subsequent bytesForMessage() walk
    // cannot match this slot's prior-lap baseSeq.
    // The pre-shape left baseSeq_=baseSeq
    // here so the per-message sum walk could find it
    // after a cumulative ACK advanced gbnBase_ — but
    // that also let a prior-lap stale slot alias a
    // new message's baseSeq if the seqs wrapped. The
    // sentinel makes the prior-lap case impossible
    // while the onSent path still seeds baseSeq_ on
    // the new message's chunks. Pinned by
    // BytesForMessageLiveWindowTest.
...
void LinkArq::noteSuppressedNak(uint8_t missingCobsSeq) {
    // AL90-9 / AL92-6 (renamed from
    // onNakOnlyForTest, which was a
    // production API named "ForTest"):
    // bump nakCount_ only, do NOT touch
    // sentAtMs_. Used by the LinkRx path
    // for a NAK that is suppressed
    // (base-stuck or same-event dedup'd —
    // no resend goes out). The counter
    // must still advance so a sustained
    // stuck base eventually trips the
    // watchdog, but the RTO clock must
    // not be reseated by a NAK that
    // produced no action. A NAK that DOES
    // produce a resend calls onNaked()
    // instead — see AL92-17.
    if (!isPending(missingCobsSeq))
        return;
    int bi = budgetIdx(missingCobsSeq);
    if (nakCount_[bi] < 0xFF)
        nakCount_[bi]++;
}
...
void LinkArq::onNaked(uint8_t missingCobsSeq, uint32_t nowMs) {
    ...
    int bi = budgetIdx(missingCobsSeq);
    sentAtMs_[bi] = nowMs;
    // NAK-driven resend counter, separate from retxCount_ so the
    // honest-drop gate (which reads retxCountFor()) cannot be
    // satisfied by NAKs. A NAK is the peer's request for an
    // immediate resend, not a proof of a failed RTO. Capped at
    // 0xFF for the same reason retxCount_ is — a single NAK
    // storm that hits 256 times for the same seq must not
    // wrap to 0 mid-storm. Pinned by NakCountSplitTest.
    if (nakCount_[bi] < 0xFF)
        nakCount_[bi]++;
    ...
    Log::log().debug(TAG, "ARQ onNaked seq=%u (RTO reseated, nakCount=%u)",
                     (unsigned)missingCobsSeq, (unsigned)nakCount_[bi]);
}
...
bool LinkArq::waitForAck(IHalCtx &ctx, uint8_t seq, uint32_t timeoutMs) {
    uint32_t genAtUnlock = clearAllEpoch_;
...
uint8_t LinkArq::nakCountFor(uint8_t seq) const {
    return nakCount_[budgetIdx(seq)];
}
...
uint8_t LinkArq::applyRetx(uint8_t seq, uint32_t nowMs, bool rtoDriven) {
    int bi = budgetIdx(seq);
    // D13: retxCount_ is the RTO path's counter —
    // NAK-driven resends don't increment it. The
    // unwalked shape advanced retxCount_ for both
    // paths; a 17-NAK-in-65ms burst would push the
    // counter past 2 and open the storm-stuck
    // verdict's honest-drop gate on evidence that
    // wasn't an RTO-elapsed failure. The NAK
    // path's own counter is nakCount_ (separated
    // earlier in the field-wedge batch); the storm-
    // stuck verdict reads retxCountFor(base) only.
    if (rtoDriven) {
        // Cap the increment at 0xFF. Earlier shape
        // was a bare `retxCount_[bi]++` on a
        // uint8_t; the field probe showed 300
        // applyRetx calls read back as 44,
        // mid-storm. The wrap hid honest-drop
        // evidence — the storm-stuck verdict's
        // `retxCountFor(base) >= 2` gate is
        // satisfied by wrap-back-to-1 in 256-call
        // windows, far below the real attempt
        // count. Pinned by RetxCountCapTest.
        if (retxCount_[bi] < 0xFF)
            retxCount_[bi]++;
    }
    // E7: sentAtMs_ reseating is also gated on rtoDriven. The escape depended
    // on the consecutiveAppBacklog_ cap (D11) — a Ping/Pong link with a
    // continuously backlogged app buffer would still re-arm the storm clock
    // and never hit honest-drop. Move the reseat inside the guard so NAK
    // resends don't reset the RTO clock.
    if (rtoDriven) {
        sentAtMs_[bi] = nowMs;
    }
