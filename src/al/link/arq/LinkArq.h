...
public:
    ...
    // waitForAck uses this to detect a
    // clearAll() that fires between unlock
    // and relock — ABA hazard if the spin
    // only watched ackedPending_[seq].
    ...
    // baseSeq of the message this chunk belongs to. Populated
    // by onSent; cleared by onAcked / clearAll. The sender
    // stamps NO_BASE (self-baseSeq) on the merged single-chunk
    // path and the hdr-only frame of a multi-chunk message;
    // data chunks of a multi-chunk message get the hdr frame's
    // seq. Walked by Link::bytesRecvdForMessage to sum a
    // multi-chunk message's per-chunk bytes-recvd into a
    // single per-message total.
    uint8_t baseSeqFor(uint8_t seq) const { return baseSeq_[seq]; }
    ...
    bool waitForAck(LinkContext &ctx, uint8_t seq, uint32_t timeoutMs);
