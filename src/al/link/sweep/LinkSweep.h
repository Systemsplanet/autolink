...
public:
    ...
    // Elapsed time within the master's current PHASE2 baud step,
    // driving a ~250 ms sub-tick PING_CMD resend cadence instead
    // of once per ~1650 ms dwell — one corrupted PING no longer
    // forfeits the whole dwell. Reset on enterPhase2 and on every
    // advance to the next baud.
    int phase2ElapsedMs() const { return phase2ElapsedMs_; }
    void setPhase2ElapsedMs(int v) { phase2ElapsedMs_ = v; }
    void addPhase2ElapsedMs(int v) { phase2ElapsedMs_ += v; }
