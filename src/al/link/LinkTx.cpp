...
void Link::sendFrame_unlocked(uint8_t payload) {
    ...
    // Wire-level byte count includes the
    // CTRL frame so the dashboard's tx
    // rate reflects the actual wire
    // activity. Pre-this-release Pong's
    // /stats tx stayed at 0 B/sec because
    // Pong's wire output is almost
    // entirely ACK/NAK/PONG, none of
    // which the buildAndSendMsg_unlocked
    // path counted.
...
void Link::resendCobsFrame_unlocked(uint8_t seq, const uint8_t *b, int n) {
    buildAndTxCobsFrame_unlocked(seq, b, n);
    // Retx still counts as wire
    // throughput — the dashboard's tx
    // rate should reflect the bytes the
    // sender actually put on the wire,
    // not just first-attempt bytes.
    if (n > 0)
        txBytes += (uint64_t)n;
    lastTxMs = hw.nowMs();
}
...
void Link::sendCtrlCobsFrame_unlocked(uint8_t type, uint8_t seq) {
    ...
    // Wire-level byte count for NAKs
    // (the only caller of this helper
    // outside of sendAckFrame_unlocked,
    // which has its own accounting).
    // Pong's wire output is dominated by
    // ACKs and NAKs; without this, the
    // dashboard's tx rate would under-
    // report whenever a gap fires.
...
void Link::sendAckFrame_unlocked(uint8_t ackedCobsSeq, uint16_t bytesRecvd) {
    ...
    frame[1 + el] = 0x00;
    hw.tx(frame, (int)(el + 2));
    // Wire-level byte count: the ACK
    // overhed bytes (COBS-encoded 5-byte
    // raw + 2 framing bytes) so the
    // dashboard's tx rate is honest on
    // Pong. The 2 framing bytes are
    // counted in el+2 but the ACK-type
    // frame doesn't pass through
    // buildAndSendMsg_unlocked so it
    // would otherwise not contribute.
    txBytes += (uint64_t)(el + 2);
    lastTxMs = hw.nowMs();
}
