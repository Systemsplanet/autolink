...
bool Link::processCtrlFrame_unlocked(State cur) {
    ...
    if (cur == State::OK) {
        // OK-state PING gets a wire-level PONG reply
        // (legacy-peer compat); no peer-miss bookkeeping.
        if (pl == PING_CMD)
...
bool Link::onPayload(uint8_t cobsSeq, const uint8_t *b, int n) {
    ...
    // Drop expired gaps and advance rxSeq past them
    // so the contiguous-resume logic in
    // flushContiguous can move on. Open 6: a gap
    // held past reorderHoldMs whose missing cobsSeq
    // never arrives must be abandoned — pinning the
    // window open forever and NAK-ing the same
    // missing seq indefinitely is worse than
    // counting the gap as lost and resuming.
    //
    // For multiple contiguous dropped seqs, the
    // max-dropped array carries them in the order
    // dropExpired found them (the per-slot loop is
    // monotonic). Advancing rxSeq once to the
    // highest dropped seq in the contiguous run is
    // enough — rxSeq is the next-expected seq, and
    // any seq in between is also missing (it was
    // never received), so stepping past one is the
    // same as stepping past all of them. lostMsgs
    // bumps per dropped slot to keep the per-gap
    // accounting honest.
    uint8_t droppedSeqs[16];
    int dropped =
        reorder_.dropExpired(hw.nowMs(), cfg.reorderHoldMs, droppedSeqs, 16);
    if (dropped > 0) {
        lostMsgs += (uint64_t)dropped;
        for (int i = 0; i < dropped; i++) {
            uint8_t gap = droppedSeqs[i];
            // rxSeq = gap means the next-expected
            // seq is gap (the slot we just dropped);
            // the +1 in reorderExpectedSeq will
            // then be gap+1 on the next frame.
            // reorderAdvanceRxSeq sets rxSeqSet=true
            // implicitly (LinkCore.cpp) so the
            // classifyGap call below treats gap as
            // the next-expected, not as a stale
            // pre-init seq.
            reorderAdvanceRxSeq(gap);
        }
    }
    int diff = 0;
    ...
    if (decideAppBuf(acc, n) == AppBufAction::HoldAck) {
        // App buf full: NAK (never a silent drop) so the
        // sender's ARQ restamps and retransmits once the
        // app drains. Pinned by LinkAckBytesTest pin 4.
        Log::log().warning(TAG,
                           "seq=%u app buf full "
...
bool Link::onAck(uint8_t ackedCobsSeq, uint16_t bytesRecvd) {
    ...
    // Wire-level ACKs count into rxBytes (pinned by
    // RxBytesWireAckTest): a Ping that never receives
    // app payload must still show a live RX rate.
...
bool Link::onNak(uint8_t missingCobsSeq) {
    ...
    // NAK-driven fast retransmit: resend inline under
    // the RX lock; deferring to the timer throttles a
    // NAK burst to one resend per tick and the gap
    // never closes. Pinned by LinkFastRetxTest.
    retxSeq_unlocked(missingCobsSeq);
    // Mirror of the ACK count above.
