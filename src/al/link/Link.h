...
class Link : private UtilFrameRx::Listener,
    ...
    // pendingBytes * 10 / baud + 1 RTT, so a 32-chunk
    ...
    // locked baud. pendingBytes * 10 / baud gives the
    ...
    uint32_t baudAwareStuckThresholdMs_unlocked() const {
        ...
        // Drain time = bytes * 10 / baud (ms), one RTT for ACK
        // turnaround. Floor at syncAckTimeoutMs so a quiet
        // single-chunk link still has a reasonable window.
        uint32_t drain = (uint32_t)pendingBytes * 10u / baud;
