...
// MSG_HDR is declared in LinkContext.h;
// re-exported here so existing call sites
// that include Link.h keep compiling.
...
class Link : private UtilFrameRx::Listener,
    ...
    // No link-level heartbeat: dead-peer detection is
    // the Ping role's consecutive-send-failure counter.
    ...
    // TX-backpressure stall stamps: first / latest
    // nowMs of the current consecutive sendMsg
    // rejection streak (ASYNC only); 0 = no streak.
    // A sender wedged upstream of the ARQ pool
    // rejects with pendingCount()==0 and
    // frameErrs==0 — invisible to both the
    // asymmetric and symmetric idle checks. The
    // OK-state timer drops + resweeps when the
    // streak outlives idleTimeoutMs and is still
    // live. Pin: LinkTxStallWatchdogTest.
    ...
    // Test-only toggle (LinkTestAccessor): forward-
    // resync on a rx gap instead of reorder-holding.
    ...
    // Decision -> side-effect translators for
    // handleSwp_unlocked. Pure decision functions
    // live in LinkDecision.h; the apply functions
    // own the I/O (setSpd / sendFrame / startTimer
    // / lockOk). Split keeps the decision functions
    // truth-table testable while the side effects
...
public:
    ...
    // Number of un-ACKed chunks sitting in the
    // sender-side ARQ cache. App layers (Ping's
    // backpressure diagnostic) read this through
    // the facade so a saturated cache is reported
    // as a saturated cache, not the app's own
    // local echo queue. No lock: pendingCount_ is
    // a single uint32 stamped inside the link
    // task and read from the app context; the
    // read window matches bytesRecvdFor's
    // lock-free contract.
    ...
    // Sum of bytes-recvd across every chunk that belongs
    // to the message whose first chunk's cobsSeq is
    // `baseSeq`. For a short ASYNC message (one merged
    // chunk) or a single-chunk SYNC message, this returns
    // the same value as bytesRecvdFor(baseSeq). For a
    // multi-chunk ASYNC message (1 hdr-only + N data
    // chunks all sharing baseSeq), the sum equals the
    // total bytes the peer pushed to its app buffer
    // (= MSG_HDR + payload length). Walks 256 entries;
    // no lock (matches bytesRecvdFor's no-lock contract
    // — both fields are stamped by the link task and
    // read from the app context; the per-chunk read
    // window is the same as the existing accessor).
    uint16_t bytesRecvdForMessage(uint8_t baseSeq) const {
        uint16_t total = 0;
        for (int i = 0; i < 256; i++) {
            if (arq_.baseSeqFor((uint8_t)i) == baseSeq)
                total += bytesRecvd_[i];
        }
        return total;
    }
