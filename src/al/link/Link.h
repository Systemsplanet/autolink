...
class Link : private UtilFrameRx::Listener,
    ...
    // Test-only: re-create the pre-fix
    // forward-resync-on-gap behaviour
    // without leaving a public config
    // flag. Set via LinkTestAccessor;
    // production code cannot reach it.
    // Off by default; the link holds
    // out-of-order frames in the reorder
    // buffer and waits for the gap to
    // fill.
