...
void Link::onTimerSwp_unlocked() {
    if (isMaster) {
        ...
        if (sweep_.phase() == SweepPhase::PHASE3) {
            ...
            if (resweepPrefPending_) {
                ...
                bool campOpen = resweepPrefDeadlineMs_ != 0 &&
                ...
                if (campOpen) {
                    ...
                    // is a no-op if the peer missed earlier
    ...
    emptySweeps++;
    // Threshold bumped from 11 ticks (~44 s at the ~4 s/tick
    // sweep cadence) to 15 (~60 s). The field capture's slave
    // fired this diagnostic 47 s before the master's begin() —
    // a staggered boot where the master's own power-on/WiFi
    // bring-up landed after the slave's 11-tick window had
    // already elapsed, producing a false "check your wiring"
    // on a link that was about to lock normally. 15 ticks covers
    // a full minute of staggered cold-boot skew; the !wasEverOk_
    // gate alongside it is unchanged — a link that locked once
    // and later fell back into SWP never re-fires this. Pinned
    // by WiringStaggeredBootThresholdTest.
    if (emptySweeps == 15) {
        if (!wasEverOk_) {
            Log::log().error(TAG,
    ...
    //
    // AL89-8: the re-arm counter now charges against
    // the total camp budget, not per-re-arm. The
    // field capture camped 10.7 s (two re-arms × 5 s)
    // while the master was walking P1 from a
    // baud-mismatch escalation; the attempt counter
    // alone let the re-arm dwell double the budget
    // before the ceiling fired. Use
    // resweepPrefDeadlineMs_ (the wall-clock budget
    // the master path already arms) as the single
    // source of truth and unconditionally fall to P1
    // when the deadline has elapsed, regardless of
    // the attempt count. Pinned by
    // P3SlaveCampBudgetCapTest.
    bool deadlineElapsed = resweepPrefDeadlineMs_ != 0 &&
        (int32_t)(hw.nowMs() - resweepPrefDeadlineMs_) >= 0;
    if (!deadlineElapsed && resweepPrefAttempts_ < RESWEEP_PREF_MAX_ATTEMPTS) {
        resweepPrefAttempts_++;
        Log::log().info(
            TAG,
            "P3 slave camp re-arming at preserved baud "
            "(attempt %d/%d, %lu ms of camp left)",
            resweepPrefAttempts_, RESWEEP_PREF_MAX_ATTEMPTS,
            (unsigned long)(resweepPrefDeadlineMs_ == 0
                                ? 0
                                : resweepPrefDeadlineMs_ - hw.nowMs()));
        int rearmMs = cfg.idleTimeoutMs / 2;
        if (rearmMs < 1000)
            rearmMs = 1000;
        // AL90-10: clamp the re-arm
        // dwell to the remaining budget.
        // The wall-clock budget
        // (resweepPrefDeadlineMs_) is the
        // single source of truth, but
        // rearmMs = idleTimeoutMs/2 can
        // exceed the time left: a 4 s
        // budget with 1 s left would
        // re-arm a 5 s dwell, pushing
        // the camp past its deadline by
        // 4 s. The field capture replayed
        // 5.75 s past a 4 s budget from
        // this exact shape. Clamp to the
        // remaining budget with a 250 ms
        // floor (so a sub-floor-budget
        // re-arm still gets a chance to
        // arm). Pinned by
        // P3SlaveCampBudgetClampedToRemainingTest
        // (unverified — see docs/Version.md).
        if (resweepPrefDeadlineMs_ != 0) {
            int32_t remainingMs =
                (int32_t)resweepPrefDeadlineMs_ - (int32_t)hw.nowMs();
            if (remainingMs > 0) {
                int budgetMs = remainingMs - 100; // small margin
                if (budgetMs < 250)
                    budgetMs = 250;
                if (rearmMs > budgetMs)
                    rearmMs = budgetMs;
