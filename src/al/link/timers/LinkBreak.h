...
struct LinkBreakConsts {
    ...
    // Bounded wait for onBreak() / onBreakStorm() taking the link
    // lock from the UART event task. sendMsg can hold the lock
    // across a hw.tx() write; the old unconditional lock() there
    // parks the event task — and with it the RX drainer both
    // callbacks depend on — behind whatever's holding it. Short
    // enough that a miss just waits for the next BREAK / storm
    // window to retry. Pinned by EventTaskBoundedLockTest.
    static constexpr int EVENT_TASK_LOCK_TIMEOUT_MS = 5;
};
