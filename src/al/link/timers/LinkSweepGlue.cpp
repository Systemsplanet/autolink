...
void Link::lockOk_unlocked(int idx, const char *tag) {
    ...
    if (bestProvenBaud_ != NO_PREFERRED_BAUD &&
        (uint8_t)idx > bestProvenBaud_ &&
    ...
    // AL89-9: re-arm the first-TX marker on
    // every lock. Sentinel = lockedAtMs_, so
    // a session that re-locks on the same
    // hw.nowMs() still resets the gate (a
    // tick-granular race that the
    // equality-vs-inequality check above
    // already handles).
    postLockFirstTxDone_ = lockedAtMs_;
    // AL90-2: clear the peer-response
    // marker too, otherwise the first
    // re-lock after one ACK latches the
    // gate open for the process lifetime.
