...
void Link::dropLink() {
    hw.lock();
    // Late drop on a link that already left OK
    // (sweep, lc, etc) would reset_unlocked
    // twice and the second sendBreak would
    // restart the handshake from a fresh P1.
    // Once sweep is running, let it run.
    if (state != State::OK) {
        hw.unlock();
        return;
    }
...
bool Link::sendMsg(const uint8_t *b, int len, uint8_t *outBaseSeq) {
    ...
    if (sync) {
        ...
        if (len + MSG_HDR <= MAX_CHUNK) {
            ...
            while (offset < len && ok) {
                ...
                // Per-chunk retx-slot reservation.
                // The message-level hasRoom() gate at
                // the top of sendMsg only checks one
                // slot free; a multi-chunk message
                // (hdr frame above + ceil(len/MAX_CHUNK)
                // data frames here) can still hit the
                // pool floor partway through. A chunk
                // that goes on the wire without a
                // retx-slot reservation is unrecoverable
                // on an early loss — the link's NAK
                // handler does a cache lookup and finds
                // nothing. Stop emitting before the
                // pool is full so the partial send
                // surfaces as a sendMsg-failure that
                // the caller's backpressure cooldown
                // can absorb, instead of a silent
                // wire-corruption hazard.
    ...
    if (ok)
        ...
        // SYNC has no ARQ sweep and no per-message
        // NAK path; a waitForAck timeout just fails
        // the send. Feed the same tx-reject streak
        // the backpressure watchdog reads so a
        // persistently un-ACKed SYNC sender (peer
        // desynced by a lost mid-message ACK) trips
        // the drop + BREAK + resweep instead of
        // wedging forever with pendingCount()==0.
        noteTxReject_unlocked();
    hw.unlock();
    if (outBaseSeq)
        *outBaseSeq = baseSeq;
