...
esp_err_t AutoLinkWeb::handleMode(httpd_req_t *req) {
    ...
    // running the new mode. The 5.3.x /mode?m=seq|rand
    // fill-mode route is renamed to /fillmode so the
    // two don't collide on the same query key.
    char query[48] = {};
    char val[12] = {};
    if (httpd_req_get_url_query_str(req, query, sizeof(query)) != ESP_OK ||
...
// OTA stubs. Slots are reserved so httpd's handler table
// has a fixed shape; the stream-to-flash + LittleFS write
// paths land in a follow-up release.
esp_err_t AutoLinkWeb::handleOtaFw(httpd_req_t *req) {
    Log::log().warning("ALinkWeb",
                       "POST /ota/fw received but OTA is not wired up yet");
    httpd_resp_set_status(req, "503 Service Unavailable");
    httpd_resp_set_type(req, "text/plain");
    httpd_resp_set_hdr(req, "Connection", "close");
    httpd_resp_send(req, "OTA firmware upload not yet implemented", 36);
    return ESP_OK;
}

esp_err_t AutoLinkWeb::handleOtaGui(httpd_req_t *req) {
    Log::log().warning("ALinkWeb",
                       "POST /ota/gui received but OTA is not wired up yet");
    httpd_resp_set_status(req, "503 Service Unavailable");
    httpd_resp_set_type(req, "text/plain");
    httpd_resp_set_hdr(req, "Connection", "close");
    httpd_resp_send(req, "OTA gui upload not yet implemented", 33);
    return ESP_OK;
}
