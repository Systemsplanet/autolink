// AutoLink web monitor -- HTTP handlers. The twelve
...
// handleDelay, handleReboot, handleOtaFw,
// handleOtaGui) all live here. They
...
esp_err_t AutoLinkWeb::handleRoot(httpd_req_t *req) {
    ...
    // GUI-over-OTA: an uploaded dashboard wins over the
    // baked-in one. See handleOtaGui.
...
esp_err_t AutoLinkWeb::handleMode(httpd_req_t *req) {
    // /mode?m=SYNC|ASYNC persists the link mode to NVS
    // only; a reboot is required for it to take effect
    // on the wire (mode changes the buffer floor and
    // SYNC wait logic, both init-time). The JS radio
    // handler kicks the reboot after a successful POST
    // so the user's click lands on a device already
    // running the new mode. The legacy /mode?m=seq|rand
    // fill-mode route lives at /fillmode so the two
    // don't collide on the same query key.
    char query[48] = {};
    char val[12] = {};
    if (httpd_req_get_url_query_str(req, query, sizeof(query)) != ESP_OK ||
...
// /fillmode?m=seq|rand sets the Ping fill mode
// (/mode is the link-mode toggle).
...
// OTA upload paths. Decision logic (slot policy, chunk
// sizing, zip parse, name sanitizing) is host-pinned in
// OtaCore; this TU owns the httpd/esp_ota/LittleFS I/O,
// which only the cross-compile exercises. Requires a
// partition table with two app slots (ota_0/ota_1) plus
// a LittleFS/SPIFFS data partition — see
// docs/WebMonitor.md.

void AutoLinkWeb::drainBody_(httpd_req_t *req, size_t remaining) {
    // A 4xx/5xx sent with body bytes still unread leaves
    // the socket half-read (the pre-wire-up 503 stub did
    // exactly that); eat the rest first.
    char sink[256];
    while (remaining > 0) {
...
namespace {
// LittleFS sink for the streaming zip parser.
struct GuiSink {
    File f;
    bool wrote = false;
};
bool guiEntry(void *c, const char *name, uint32_t) {
    ...
    if (!otaSafeEntryName(name, path, sizeof(path)))
        return false; // skip unsafe entries; parse continues
    // Ensure parent dirs exist (LittleFS wants them).
...
bool guiData(void *c, const uint8_t *b, size_t n) {
    GuiSink *s = (GuiSink *)c;
    return s->f.write(b, n) == n; // short write = FS full -> abort
}
bool guiEnd(void *c) {
    GuiSink *s = (GuiSink *)c;
    s->f.close();
