...
private:
    ...
    // LittleFS mounted (GUI-over-OTA storage). Set once
    // in setupHttpAndLogging_; handleRoot serves
    // /web/index.html from FS when present, else the
    // baked-in DASHBOARD_HTML.
    bool fsOk_ = false;
    bool mountFs_();
    // Drain and discard any unread request body so an
    // error response doesn't leave a half-read socket.
