...
int formatStatsJson(const WebSnapshot *s, int logLevel, const char *version,
    ...
    return snprintf(
        out, (size_t)outLen,
        "{\"state\":\"%s\",\"errCount\":%lu, \"errTotal\":%llu, \"lostMsgs\":%llu, \"quietDrops\":%llu, \"rateDrops\":%llu, \"ringStallDrops\":%llu, \"logDrops\":%llu, \"txBps\":%lu,\"rxBps\":%lu, \"txTotal\":%llu,\"rxTotal\":%llu, \"rssi\":%d,\"freeHeap\":%lu,\"uptimeS\":%lu, \"baudRate\":%lu,\"lvl\":%d,\"mode\":%u, \"msgPaused\":%u, \"linkMode\":%u, \"linkModeLabel\":\"%s\", \"txDelayMs\":%d, \"role\":\"%s\", \"webLogDropped\":%lu, \"version\":\"%s\"}",
        ...
        // AL90-7: log ring drops surfaced
        // in the dashboard JSON so the
        // operator sees the saturated
        // ring without timestamp-grepping
        // the device log. Pinned by
        // LogDropsSurfaceTest.
        (unsigned long long)s->logDrops, (unsigned long)s->txBps,
        ...
        (int)s->txDelayMs, s->role, (unsigned long)s->webLogDropped,
        version ? version : "");
}
...
int formatLogsJson(const WebLogEntry *ring, uint32_t head, uint32_t since,
    ...
    if (!ring || !out || outLen <= 0)
        return 0;
    // AL87-01: the ring is fixed-capacity (WEB_RING_CAP) and
    // overwrites oldest-first. If the caller's `since` is older
    // than what the ring can still hold, everything between
    // `since` and the true window start is gone and was
    // previously served up with zero indication — a client
    // polling slower than the ring fills (or a client that missed
    // one poll under load) got a seamless-looking but silently
    // incomplete log, which is exactly what made every field
    // capture in this project's history undiagnosable. Surface
    // the loss count in the response instead of only in the
    // internal `start` clamp. Pinned by WebLogDropVisibleTest.
    uint32_t ringStart =
    ...
    int len = snprintf(out, (size_t)outLen,
                       "{\"head\":%lu,\"dropped\":%lu,\"lines\":[",
                       (unsigned long)head, (unsigned long)dropped);
    if (len < 0 || len >= outLen)
        return len;
