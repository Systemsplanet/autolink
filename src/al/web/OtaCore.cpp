...
namespace autolink {
static constexpr const char *TAG = "OtaCore";
...
bool otaSafeEntryName(const char *name, char *out, size_t outLen) {
    ...
    if (n == 0 || n > OtaZipStream::kNameMax) {
        Log::log().warning(TAG,
                           "otaSafeEntryName rejected: name len=%zu "
                           "(max %u)",
                           n, (unsigned)OtaZipStream::kNameMax);
        return false;
    }
    if (name[0] == '/' || name[n - 1] == '/') {
        Log::log().warning(TAG, "otaSafeEntryName rejected: '%s' (slashes)",
                           name);
        return false;
    }
    if (strstr(name, "..") || strchr(name, '\\')) {
        Log::log().warning(
            TAG, "otaSafeEntryName rejected: '%s' (path traversal)", name);
        return false;
    }
    constexpr const char *kPrefix = "/web/";
    if (outLen < strlen(kPrefix) + n + 1) {
        Log::log().warning(
            TAG, "otaSafeEntryName rejected: '%s' (out too small)", name);
        return false;
    }
    strcpy(out, kPrefix);
    strcat(out, name);
    ...
    while (n > 0 || (st_ == St::DATA && dataLeft_ == 0)) {
        ...
        case St::SIG: {
            ...
            if (sig == 0x02014b50u || sig == 0x06054b50u) {
                Log::log().info(TAG,
                                "OTA zip: end-of-archive sig=0x%08X "
                                "(0x02014b50=central dir, 0x06054b50=EOCD)",
                                (unsigned)sig);
                st_ = St::DONE;
                return true;
            }
            ...
            Log::log().debug(TAG, "OTA zip: local file header sig=0x%08X",
                             (unsigned)sig);
            st_ = St::HDR;
            got_ = 0;
            need_ = 26;
        ...
        case St::HDR: {
            ...
            Log::log().debug(TAG,
                             "OTA zip: entry hdr method=%u flags=0x%X "
                             "dataLen=%u nameLen=%u extraLen=%u",
                             (unsigned)method, (unsigned)flags,
                             (unsigned)dataLeft_, (unsigned)nameLen_,
                             (unsigned)extraLen_);
            st_ = St::NAME;
            got_ = 0;
            break;
        ...
        case St::NAME: {
            ...
            if (got_ < nameLen_)
                return true;
            name_[nameLen_] = '\0';
            Log::log().debug(TAG, "OTA zip: entry name='%s'", name_);
            st_ = St::EXTRA;
            got_ = 0;
            break;
        }
        case St::EXTRA: {
            ...
            if (got_ < extraLen_)
                return true;
            uint32_t sz = rd32(hdr_ + 18);
            deliver_ = cb_.entry ? cb_.entry(cb_.ctx, name_, sz) : false;
            Log::log().info(TAG, "OTA zip: entry '%s' size=%u deliver=%s",
                            name_, (unsigned)sz, deliver_ ? "yes" : "no");
        ...
        case St::DONE:
            return true;
        case St::ERR:
            // Reached only via the fail() helper, which logs
            // the reason inline. No additional log here so
            // the message in the wire is the single source
