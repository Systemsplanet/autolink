...
bool AutoLinkWeb::setupHttpAndLogging_() {
    ...
    // GUI-over-OTA storage. Non-fatal: without FS the
    // dashboard serves from the baked-in AutoLinkWebHtml.h.
    mountFs_();
    {
        // First boot after a firmware OTA: mark the image
        // valid so bootloader rollback (when enabled)
        // doesn't revert it on the next reset.
...

    {
        ...
        // OTA reserves r10/r11 below. The cap has to be at
        // least N+2 so the registration loop doesn't trip
        // HANDLERS_FULL on the new routes.
...
bool AutoLinkWeb::mountFs_() {
    if (fsOk_)
        return true;
    fsOk_ = LittleFS.begin(true /* format on first mount */);
    if (!fsOk_)
        Log::log().warning(TAG,
