...
// First decoded byte of an ACK frame. RESERVED value — never appears
// as a data frame's cobsSeq (sender skips 0xFF in its counter). This
// is what makes ACK/data discrimination work: a single-byte check
// (`decoded[0] == ACK_TYPE`) is only safe if the discriminator value
// is structurally impossible in the data path.
//
// v5.1.45 (wire format break): ACK_TYPE moved from 0x33 to 0xFF and
// the sender's cobsSeq counter skips 0xFF. The previous design
// "checks first byte == ACK_TYPE BEFORE interpreting as cobsSeq"
// was true at the code level but the discrimination was
// structurally impossible when ACK_TYPE overlapped the cobsSeq
// range (0..255). A data frame with cobsSeq=0x33 was misidentified
// by the receiver as an ACK frame and discarded — see
// test_throughput_and_sizes failing at sz=8000 when cobsSeq=51=0x33
// hit the wire.
constexpr uint8_t ACK_TYPE = 0xFF;
// Highest cobsSeq the counter emits before wrapping to 0.
// 0xFF is reserved for ACK_TYPE (the counter skips it).
constexpr uint8_t COBS_SEQ_MAX = 0xFE;
