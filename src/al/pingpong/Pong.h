...
public:
    ...
    void loop() {
        ...
        if (!base_.wasReady_) {
            // Don't drain yet: post-switch baud garbage
            // arrives AFTER this point. Kick the settling
            // indicator; next iteration drains.
        ...
        if (tReady_ != 0) {
            // Post-settle drain. The 4-blink
            // settles at ~480 ms (4 × 120 ms
            // on/off); the next loop iteration
            // is well after the baud switch is
            // stable. Drain any garbage the
            // baud transition pushed onto the
            // rx stream so the post-recovery
            // recv path starts on a clean
            // buffer.
        ...
        if (n < 0) {
            // Pong's recv-rejected log line includes
            // the link-layer's frameErrs counter so
            // operators can correlate the app-level
            // reject (one per recv() that returned
            // negative) with the wire-level error
            // count (one per actual CRC failure in
            // the framer). The two counters are
            // different by construction: a single
            // CRC failure can produce multiple
            // recv-rejected log lines (each
            // subsequent recv() on a still-corrupt
            // stream returns negative), but only one
            // frameErrs bump. Logging them together
            // resolves the 16-vs-3 mismatch the
            // bench log exposed — operators see
            // exactly how the two relate on their
            // own link.
            Stats s;
...
private:
    // Post-OK settling delay before draining stale
    // bytes: must outlast the ~480 ms blink indicator
    // so line garbage from the baud switch lands
