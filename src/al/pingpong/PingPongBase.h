...
// silent wire. The halt is a 300 ms
// LED blink so an operator at the
// bench sees the failure without
// watching the debug Serial.
inline void bringUpLink(Log &log, AutoLink &comm, bool prePaused = false) {
    // First thing the link layer emits: the
    // version. Operators at the bench
    // correlate a stale build against the
    // running wire; the version line must
    // land before any "calling begin()"
    // diagnostic so the first log line
    // answers "what is this firmware?".
    // The previous shape placed the
    // version line AFTER begin() — a
    // pre-fix bogus check (since removed
    // in 6.0.9) made the wrong line the
    // first line in the live log, and
    // even on a clean boot the order
    // forced the operator to read past
    // begin() diagnostics before the
    // version.
    ...
    comm.begin();
    if (!comm.isHealthy()) {
        // Halt visibly: blink the LED every 300 ms in an
        // infinite loop so an operator at the bench
        // sees the failure without watching the serial
        // log. The previous shape was a silent infinite
        // delay loop — invisible on the bench unless the
        // operator was staring at the debug Serial.
        // 300 ms on/off is the "trouble" cadence — slow
        // enough to distinguish from normal wire-activity
        // blinks, fast enough to be impossible to miss.
