...
//   - Random mode: random size 1..maxMsg with random
...
public:
    ...
    void loop() {
        ...
        if (!base_.wasReady_) {
            // A half-received frame from before
            // the drop (header read but payload
            // truncated by the link reset) lands
            // in the rx stream at the next
            // recvMsg call and fails the CRC.
            // Flush the raw rx path before any
            // recv; mirrors Pong's pre-blink drain.
            base_.comm_.flushRx();
            int drained = 0;
                ...
                if (count_ > 0) {
                    while (count_ > 0 &&
                        ...
                        base_.log_.debug("Ping", "echo %u %u %d",
                                         ...
                                         count_ - 1);
                        // Wire-ACK bytes-recvd sum across
                        // every chunk sharing this baseSeq.
                        // For a single-chunk send equals the
                        // merged-chunk length (= MSG_HDR +
                        // payload); for a multi-chunk ASYNC
                        // message equals MSG_HDR + payload
                        // across all chunks. Reads through
                        // the AutoLink facade so the same
                        // accessor serves the dashboard JSON.
                        base_.log_.debug(
                            "Ping", "wire %u %u", (unsigned)queue_[head_].seq,
                            (unsigned)base_.comm_.bytesRecvdForMessage(
                                queue_[head_].seq));
        ...
        while (count_ < WINDOW && sentThisLoop < maxTx) {
            // txDelayMs paces sends when the user
            // asked for it. backpressureCoolUntilMs_
            // is the ASYNC-only emergency brake
            // (set on a sendMsg-failure branch
            // below) that fires regardless of
            // txDelayMs — the gate at line ~284 in
            // the prior shape only honored
            // tNextSendMs_ when txDelayMs > 0, so
            // ASYNC flood mode (txDelayMs=0, the
            // exact scenario that produced the
            // consec=1378 storm) skipped the
            // throttle entirely. Keep the two
            // gates orthogonal so the cooldown
            // always wins on a real backpressure
            // hit.
            ...
            if (!base_.comm_.sendMsg(sendBuf_, n, &seq)) {
                // sendMsg returns false for two
                // distinct reasons:
                //   1. Link not OK (state == SWP/LCK).
                //      The link is self-recovering;
                //      counting it as a send failure
                //      makes the consecSendFail_
                //      counter escalate to dropLink on
                //      a link that's already resweeping,
                //      doubling the resweep cycle and
                //      starving wire bandwidth.
                //   2. Real backpressure (ARQ cache
                //      full / seq-space exhausted).
                //      This is what MAX_SEND_FAIL is
                //      for.
                // Only count (2) toward the threshold.
                if (!base_.comm_.ready()) {
                    break;
                }
                consecSendFail_++;
                ...
                // ASYNC backpressure cooldown: the
                // link's ARQ retransmit loop is the
                // only path that drains the cache;
                // hammering send() on a full cache
                // burns wall-clock on log lines that
                // tell the operator nothing new and
                // steals cycles from the link task
                // that's actually doing the work.
                // Wait BACKPRESSURE_COOLDOWN_MS before
                // the next attempt so the link gets a
                // chance to make room. The throttle
                // is wall-clock, not loop-iteration
                // count, so it scales with baud.
                // SYNC mode never trips this branch
                // — the sender blocks inline for the
                // receiver ACK — so the cooldown is
                // ASYNC-only.
                //
                // Use backpressureCoolUntilMs_, not
                // tNextSendMs_: the send-loop's
                // txDelayMs gate only honors
                // tNextSendMs_ when txDelayMs > 0,
                // so setting it here was a no-op in
                // ASYNC flood mode (txDelayMs=0).
        ...
        if (count_ > 0) {
            while (count_ > 0 && base_.comm_.isAcked(queue_[head_].seq)) {
                ...
                // the AutoLink facade so the same
                // accessor serves the dashboard JSON.
                base_.log_.debug("Ping", "wire %u %u",
                                 (unsigned)queue_[head_].seq,
                                 (unsigned)base_.comm_.bytesRecvdForMessage(
                                     queue_[head_].seq));
                head_ = (head_ + 1) % WINDOW;
...
private:
    int pickMsgSize_(FillMode m) {
        ...
        // Random: 1..maxSeqSize_. The pre-this release
        // shape used RANDOM_MIN_BYTES = 1024 with a
        // hard-coded maxSeqSize_ = 1024; min == max
        // collapsed the random range to a single
        // value. With the default maxSeqSize_ (pulled
        // from AUTOLINK_DEFAULT_MAX_MSG in setup())
        // the random range has the full span needed
        // to exercise the chunk-build path at every
        // size.
    ...
    // Random mode: 1-byte floor. Earlier releases
    // collapsed the random range to a single value
    // when min == maxSeqSize_; floor of 1 keeps the
    // range 1..maxSeqSize_ and exercises the
    // chunk-build path at every size.
    ...
    // ASYNC backpressure cooldown: the wall-clock
    // gap between a sendMsg() that returned false
    // (ARQ cache full or seq-space exhausted) and
    // the next send attempt. The link's ARQ
    // retransmit loop needs time to drain the cache
    // or for the receiver to catch up; retrying
    // immediately just bumps the consecSendFail_
    // counter toward MAX_SEND_FAIL and starves the
    // link task. 1000 ms is the same order as
    // syncAckTimeoutMs — one round trip's worth of
    // time for the cache to make room. Operators
    // who want a tighter or looser retry can set
    // cfg.txDelayMs instead; the cooldown is the
    // ASYNC-only emergency brake on a stalled
    // pipeline.
    ...
    // ASYNC-only backpressure cooldown stamp. Set
    // when sendMsg() trips consecSendFail_++,
    // honored by the send-loop gate regardless of
    // cfg.txDelayMs. Distinct from tNextSendMs_
    // (which the gate only honors when
    // txDelayMs > 0) so ASYNC flood mode still
    // throttles on a real backpressure hit.
    ...
    // in a register across iterations. Pre-setup default
    // tracks the canonical default so a sketch that
    // reads maxSeqSize_ before setup() lands still sees
    // a consistent value.
