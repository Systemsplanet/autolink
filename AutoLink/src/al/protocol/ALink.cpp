      ...
      txBytes(0), rxBytes(0), discCount(0), frameErrs(0)
{
    ...
    hw.bind(this);
    Log::log().info(ALINK_TAG, "Initialized as %s. Reliable Mode: %s. cobsSeq tracking: ON",
...
void ALink::resetSeq_unlocked() {
    ...
    Log::log().debug(ALINK_TAG, "cobsSeq reset (sender=%u, lastRx cleared)", (unsigned)txSeq);
}
...
void ALink::begin() {
    ...
    if (cfg.allowedBaudsCount != 0) {
        Log::log().info(ALINK_TAG, "%s: %d bauds %lu..%lu, %d samples/baud, fastAck=%s",
            isMaster ? "Ping" : "Pong",
    ...
    if (isMaster) {
        ...
        Log::log().info(ALINK_TAG, "SWP Pong testing baud[0]=%lu",
            (unsigned long)cfg.allowedBauds[0]);
...
void ALink::changeState_unlocked(State newState) {
    if (state != newState) {
        ...
        Log::log().debug(ALINK_TAG, "State Transition: %s -> %s",
...
bool ALink::err_unlocked() {
    ...
    if (errs > cfg.errThreshold) {
        Log::log().info(ALINK_TAG, "Error threshold exceeded (%d > %d). Dropping link.",
...
// v5.1.19: lock-free send body. Caller MUST hold hw.lock().
// Body of the old ALink::write() factored out so the ARQ retransmit
// path (which runs inside the OK-state timer callback while hw.lock()
// is already held) can call into the protocol layer without
// re-acquiring the mutex. See ALink.h for the full rationale.
int ALink::sendMsg_unlocked(const uint8_t* b, int len) {
    if (state != State::OK) {
...
void ALink::lockOk_unlocked(int idx, const char* tag) {
    ...
    Log::log().info(ALINK_TAG, "Locked at %lu baud (%s)",
...
bool ALink::onFrameError() {
    Log::log().debug(ALINK_TAG, "frame error (corrupt COBS / bad CRC / oversize)");
    return err_unlocked();
}

void ALink::onBreak() {
    hw.lock();
    Log::log().info(ALINK_TAG, "BREAK received -> re-sweep");
...
void ALink::onTimer() {
    ...
    // v5.1.19: dispatch any deferred ARQ retransmit callback OUTSIDE
    // the link lock. onTimerOk_unlocked() may have set hasPendingRetx_
    // = true when it found an expired slot; the actual resend must run
    // without hw.lock() held because AutoLink::retx_resend ->
    // link->sendMsg() acquires the same lock, and on FreeRTOS with a
    // non-recursive mutex a re-entrant lock attempt deadlocks (and on
    // ESP-IDF v4 the attempt can also crash if the mutex owner is
    // detected as invalid). This is the fix for the user-reported
    // v5.1.17 boot-crash (2026-06-19).
    if (hasPendingRetx_ && arqRetxCallback_) {
        uint8_t base = pendingRetxBase_;
        hasPendingRetx_ = false;
        if (arqRetxCallback_(base, arqCtx_)) {
            // Facade asked us to drop the link (e.g. cache miss).
            // Don't try to acquire hw.lock() here — the callback
            // already ran without it, and the drop path needs the
            // lock. Re-take it and reset.
            hw.lock();
            reset_unlocked(true);
            hw.unlock();
            hw.sendBreak();
...
void ALink::onTimerOk_unlocked() {
    ...
    if (cfg.reliableMode) {
        for (int s = 0; s < 256; s++) {
            ...
            // v5.1.19: defer the facade retransmit callback until
            // AFTER hw.unlock(). Calling it here would re-enter
            // link->sendMsg() -> hw.lock() on a non-recursive mutex
            // (deadlock / crash). The retransmit is dispatched by
            // ALink::onTimer() right after this function unlocks.
            // pre-v5.1.19 this was a direct call to arqRetxCallback_,
            // which is the root cause of the user-reported boot
            // crash on Pong (2026-06-19).
            if (arqRetxCallback_) {
                pendingRetxBase_ = base;
...
void ALink::onTimerSwp_unlocked() {
    ...
    if (scored < needed) {
        ...
        pingSample = 0;
        Log::log().info(ALINK_TAG, "SWP Pong testing baud[%d]=%lu",
            spdI, (unsigned long)cfg.allowedBauds[spdI]);
        hw.setSpd(cfg.allowedBauds[spdI]);
    }
    hw.startTimer(cfg.pingSamplesPerBaud * cfg.delayMs);
