...
void test_level_filtering() {
 std::cout << "\n=== Test: Log Level Filtering (4: NONE..DEBUG) ===" << std::endl;
 Log& L = Log::getLog();
 L.setSink(captureSink);
 g_captured.clear();

 // log line must not produce a sink callback.
 L.setLevel(Log::NONE);
 ...
 L.info ("T", "info3");
 L.debug("T", "dbg3");
 // 2 (INFO round: err+info; dbg filtered at INFO) = 5.
 ...
 L.info ("T", "info4");
 L.debug("T", "dbg4");
 // 5 + 3 (DEBUG round: err+info+dbg) = 8.
 ...
 // (no gate above it — VERBOSE is the new top tier). The 'V'
 // severity letter is the one the host sink / dashboard CSS
 // expect.
 ...
 // semantic (NONE < ERROR < WARNING < INFO < DEBUG) so a piece of
 ...
 // wouldEmit() helper is the public contract the GUI uses to
 // decide which level to highlight on first paint.
 // wouldEmit(X) returns (X <= current_level).
 L.setLevel(Log::WARNING); // level = 2
 assert( L.wouldEmit(Log::ERROR)); // 1 <= 2
 assert( L.wouldEmit(Log::WARNING)); // 2 <= 2
 assert(!L.wouldEmit(Log::INFO)); // 3 > 2
 assert(!L.wouldEmit(Log::DEBUG)); // 4 > 2
 L.setLevel(Log::NONE); // level = 0
 assert(!L.wouldEmit(Log::ERROR));
 assert(!L.wouldEmit(Log::WARNING));
 assert(!L.wouldEmit(Log::INFO));
 assert(!L.wouldEmit(Log::DEBUG));
 L.setLevel(Log::DEBUG); // level = 4
 assert( L.wouldEmit(Log::ERROR));
 assert( L.wouldEmit(Log::WARNING));
 assert( L.wouldEmit(Log::INFO));
 assert( L.wouldEmit(Log::DEBUG));
 ...
 L.setLevel(Log::VERBOSE); // level = 5
 assert( L.wouldEmit(Log::ERROR));
 assert( L.wouldEmit(Log::WARNING));
 assert( L.wouldEmit(Log::INFO));
 assert( L.wouldEmit(Log::DEBUG));
...
void test_long_message_truncated_at_buffer() {
 ...
 L.setSink(captureSink);
 g_captured.clear();
 // long error messages without spilling into the one-shot
 // truncation warning); vsnprintf truncates to fit.
 std::string big(1024, 'A');
 L.info("T", "%s", big.c_str());
 assert(g_captured.size() == 1);
 assert(g_captured[0].msg.size() <= 384); //: was 320; raised to fit WIRING CHECK
 assert(g_captured[0].msg.size() > 320); // and actually filled past the old cap
 L.clearSink();
 std::cout << "PASS" << std::endl;
}
