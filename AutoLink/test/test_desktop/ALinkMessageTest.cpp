...
void test_app_buffer_null_simulates_disconnect() {
 ...
 // production code). The exact count is not asserted because the
 // MockHal pipeline may pre-decode multiple frames depending on
 // how many were queued; we just confirm at least one gap was
 // recorded so an operator can see the counter climbing.
 Diag d; b.getDiag(d);
 assert(d.gaps >= 1);

 std::cout << "PASS (recv returned 0, gaps=" << d.gaps << " as expected)" << std::endl;
}
...
void test_corrupt_msg_header_does_not_clear_buffer() {
 ...
 assert(err == -1);
 assert(b.getErrCount() == errsBefore + 1);
