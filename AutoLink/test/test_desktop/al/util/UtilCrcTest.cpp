// Host-only unit tests for UtilCrc. Arduino/ESP32 builds skip this file.
#ifndef ARDUINO
...
// Bitwise reference implementations, independent of the LUTs under test.
...
void test_known_answers() {
    ...
    assert(UtilCrc::crc8(check, 9) == 0xF4);    // CRC-8 (poly 0x07, init 0)
    assert(UtilCrc::crc16(check, 9) == 0x29B1); // CRC-16/CCITT-FALSE
...
void test_edges() {
    ...
    uint8_t b = 0x00;
    assert(UtilCrc::crc8(&b, 0) == 0x00);       // empty input = init value
...
// LUTs must agree with the bitwise reference across every byte value and a
// long pseudo-random buffer — locks the tables against single-entry typos.
void test_lut_vs_reference() {
    std::cout << "\n=== Test: LUT Agrees With Bitwise Reference ===" << std::endl;
    for (int v = 0; v < 256; v++) {
        uint8_t b = (uint8_t)v;
        assert(UtilCrc::crc8(&b, 1) == ref_crc8(&b, 1));
        assert(UtilCrc::crc16(&b, 1) == ref_crc16(&b, 1));
...
// A single flipped bit anywhere must change both checksums.
