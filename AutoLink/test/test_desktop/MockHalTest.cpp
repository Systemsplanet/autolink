// Host-only unit tests for the MockHal helper (host-side ILink mock).
// MockHal is the seam the entire ALink test suite relies on; these tests
// pin its public behaviour: TX buffer, break delivery, app buffer
// (push/peek/pop/clear), and the injectable clock.
#ifndef ARDUINO
...
void test_send_break_self_delivery_is_noop_without_peer() {
    std::cout << "\n=== Test: sendBreak Without Peer Doesn't Self-Deliver ===" << std::endl;
    MockHal hal;
    hal.bind(new ALink(hal, true, AutoLinkConfig()));
    hal.sendBreak();
    assert(hal.sendBreakCalls == 1);
    // State unchanged -- sendBreak is a wire-emit, not a local drop.
...
void test_send_break_delivers_to_peer() {
    std::cout << "\n=== Test: sendBreak Delivers to Peer MockHal ===" << std::endl;
    MockHal a, b;
    a.peer = &b;
    a.bind(new ALink(a, true, AutoLinkConfig()));
    b.bind(new ALink(b, false, AutoLinkConfig()));
    // Both start in State::OK. a.sendBreak() must transition b -> SWP
    // without touching a's own state.
    assert(a.link->getState() == State::OK);
    assert(b.link->getState() == State::OK);
    a.sendBreak();
    assert(a.sendBreakCalls == 1);
    assert(b.link->getState() == State::SWP);
    assert(a.link->getState() == State::OK);   // sender unaffected
    delete a.link; a.link = nullptr;
    delete b.link; b.link = nullptr;
    std::cout << "PASS" << std::endl;
}
...
void test_lock_unlock_balanced() {
    std::cout << "\n=== Test: MockHal lock / unlock Are Balanced ===" << std::endl;
    MockHal hal;
    hal.lock();
    hal.unlock();
    // std::mutex is non-recursive; a second lock without unlock would
    // deadlock. We assert the contract is "balanced" by acquiring and
    // releasing in pairs, and leaving the mutex unlocked at end-of-test.
    hal.lock();
    hal.unlock();
    std::cout << "PASS" << std::endl;
}
...
void test_app_buffer_push_peek_pop_clear() {
    std::cout << "\n=== Test: MockHal App Buffer Push/Peek/Pop/Clear ===" << std::endl;
    MockHal hal;
    uint8_t pb[] = {0xAA, 0xBB};
    hal.pushAppBuf(pb, 2);
    assert(hal.appBufAvailable() == 2);
    assert(hal.peekAppBuf() == 0xAA);
    assert(hal.appBufAvailable() == 2);   // peek doesn't consume
