...
void test_lck_timeout() {
 ...
 // v5.1.40: pumpClock drives each timer tick deterministically.
 // cfg.delayMs=50; allowedBaudsCount=2 -> LCK retry threshold = 4.
 AutoLinkConfig cfg;
 cfg.allowedBauds[0] = 9600; cfg.allowedBauds[1] = 115200; cfg.allowedBaudsCount = 2; cfg.pingSamplesPerBaud = 1;
 MockHal mHal; // no peer: REQs go nowhere
 ALink ping(mHal, true, cfg);
 ping.begin();
 mHal.pumpClock(50); ping.onTimer(); // PING@9600
 mHal.pumpClock(50); ping.onTimer(); // PING@115200 -> LCK
