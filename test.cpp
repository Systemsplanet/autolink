...
uint8_t calcTestCrc(const uint8_t* data, int len) {
    uint8_t crc = 0;
    // Note: Test needs to use same logic as optimized LUT. We can re-implement LUT locally for test
...
void run_test_reliable_mode() {
    ...
    master.write(data, 2);
    // TX should be 6 bytes: 2 frames of 3 bytes [SYNC, DATA, CRC]
    assert(mHal.txN == 6);
